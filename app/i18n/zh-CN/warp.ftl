...
common-skip = 跳过
common-get-warping = 开始使用 Phosphor
common-try-again = 重试
common-settings = 设置
common-premium = 高级版
...
agent-management-source-local-agent = Phosphor（本地智能体）
agent-management-source-cloud-agent = Phosphor（本地智能体）
...
workspace-menu-update-warp-manually = 手动更新 Phosphor
...
workspace-menu-view-warp-logs = 查看 Phosphor 日志
...
workspace-toast-all-conversations-deleted = 已删除 { $count } 个对话
workspace-toast-no-conversations-to-delete = 没有可删除的对话
...
app-menu-debug = 调试
app-menu-set-default-terminal = 将 Phosphor 设为默认终端
app-menu-file = 文件
app-menu-edit = 编辑
app-menu-use-warp-prompt = 使用 Phosphor 提示符
...
app-menu-show-warpified-ssh-blocks = 显示 Phosphor 化 SSH 块
app-menu-hide-warpified-ssh-blocks = 隐藏 Phosphor 化 SSH 块
...
app-menu-help = 帮助
app-menu-warp-documentation = Phosphor 文档...
app-menu-github-issues = GitHub Issues...
app-menu-warp-slack-community = Phosphor Slack 社区...
workspace-update-and-relaunch-warp = 更新并重启 Phosphor
workspace-updating-to-version = 正在更新到（{ $version }）
workspace-update-warp-manually = 手动更新 Phosphor
...
terminal-banner-completions-not-working-suffix =  中启用 tmux phosphorization 可能会解决此问题。
terminal-banner-shell-config-incompatible = 你的 shell 配置与 Phosphor 不兼容...{"  "}
...
terminal-agent-header-for-parent-named = 用于 { $name }
ssh-remote-choice-title = 为此远程会话选择体验：
ssh-remote-choice-install-extension = 安装 Phosphor 的 SSH 扩展
ssh-remote-choice-install-extension-desc = 安装 Phosphor 的扩展以在此会话中启用文件浏览、代码评审和智能命令补全等智能体功能。
ssh-remote-choice-continue-without-installing = 不安装并继续
ssh-remote-choice-continue-without-installing-desc = 你仍可获得 Phosphor 化体验，但不包含智能体功能。
...
ai-document-copied-as-markdown = 已作为 Markdown 复制到剪贴板
...
ai-conversation-view-in-oz = 在 Phosphor Agent 中查看
ai-conversation-view-in-oz-tooltip = 在 Web 应用中查看此运行
...
ai-footer-enable-notifications-tooltip = 安装通知插件以在 Phosphor 中启用丰富的智能体通知
ai-footer-notifications-setup-instructions = 通知设置说明
ai-footer-install-plugin-instructions-tooltip = 查看安装通知插件的说明
ai-footer-update-warp-plugin = 更新通知插件
ai-footer-plugin-update-available-tooltip = 通知插件有新版本可用
ai-footer-plugin-update-instructions = 插件更新说明
ai-footer-plugin-update-instructions-tooltip = 查看更新通知插件的说明
...
ai-cloud-setup-visit-oz = 访问 Phosphor Agent
...
zap-launch-title = Phosphor 现已开源
zap-launch-description = 你和我们的社区现在可以使用智能体优先的工作流参与构建 Phosphor。
zap-launch-contribute-title = 参与贡献
zap-launch-contribute-description = Phosphor 客户端代码现已开源。你可以先使用 /feedback skill 创建 issue，并阅读这里的贡献指南。
zap-launch-contribute-link-text = 这里
zap-launch-oad-title = 开放自动化开发
zap-launch-oad-description = Phosphor 仓库由智能体优先的本地工作流管理，并由 Phosphor Agent 本地智能体体验提供支持。
...
hoa-finish = 完成
session-config-get-warping = 开始使用 Phosphor
...
terminal-warpify-subshell-tooltip = 在此会话中启用 Phosphor shell 集成
terminal-use-agent = 使用智能体
terminal-use-agent-tooltip = 让 Phosphor 智能体协助
terminal-give-control-back-to-agent = 将控制权交还给智能体
terminal-resume-agent-tooltip = 让 Phosphor 智能体继续
...
terminal-open-in-warp = 在 Phosphor 中打开
terminal-open-conversation-in-warp-tooltip = 在 Phosphor 桌面端打开此对话
...
terminal-input-agent-hint-deploy-react-vercel = 让 Phosphor 处理任何任务，例如将 React 应用部署到 Vercel 并配置环境变量
terminal-input-agent-hint-debug-python-ci = 让 Phosphor 处理任何任务，例如帮我调试 Python 测试在 CI 中失败的原因
terminal-input-agent-hint-setup-microservice = 让 Phosphor 处理任何任务，例如用 Docker 搭建新的微服务并创建部署流水线
terminal-input-agent-hint-fix-node-memory-leak = 让 Phosphor 处理任何任务，例如查找并修复 Node.js 应用中的内存泄漏
terminal-input-agent-hint-backup-postgres = 让 Phosphor 处理任何任务，例如为 PostgreSQL 数据库创建备份脚本并设置定时任务
terminal-input-agent-hint-migrate-mysql-postgres = 让 Phosphor 处理任何任务，例如帮我把数据从 MySQL 迁移到 PostgreSQL
terminal-input-agent-hint-monitor-aws = 让 Phosphor 处理任何任务，例如为 AWS 基础设施设置监控和告警
terminal-input-agent-hint-build-fastapi = 让 Phosphor 处理任何任务，例如用 FastAPI 为移动应用构建 REST API
terminal-input-agent-hint-optimize-sql = 让 Phosphor 处理任何任务，例如帮我优化运行缓慢的 SQL 查询
terminal-input-agent-hint-github-actions = 让 Phosphor 处理任何任务，例如创建 GitHub Actions 工作流，在合并后自动部署
terminal-input-agent-hint-redis-cache = 让 Phosphor 处理任何任务，例如为 Web 应用设置 Redis 缓存
terminal-input-agent-hint-kubernetes-pods = 让 Phosphor 处理任何任务，例如帮我排查 Kubernetes Pod 持续崩溃的原因
terminal-input-agent-hint-bigquery-pipeline = 让 Phosphor 处理任何任务，例如构建数据流水线来处理 CSV 文件并加载到 BigQuery
terminal-input-agent-hint-ssl-https = 让 Phosphor 处理任何任务，例如设置 SSL 证书并为域名配置 HTTPS
terminal-input-agent-hint-refactor-legacy-code = 让 Phosphor 处理任何任务，例如帮我将这段遗留代码重构为现代设计模式
terminal-input-agent-hint-unit-tests = 让 Phosphor 处理任何任务，例如为认证服务创建单元测试
terminal-input-agent-hint-elk-logs = 让 Phosphor 处理任何任务，例如为分布式系统设置 ELK 日志聚合
terminal-input-agent-hint-oauth-express = 让 Phosphor 处理任何任务，例如帮我在 Express.js 应用中实现 OAuth2 认证
terminal-input-agent-hint-optimize-docker = 让 Phosphor 处理任何任务，例如优化 Docker 镜像以减少构建时间和体积
terminal-input-agent-hint-ab-testing = 让 Phosphor 处理任何任务，例如为 Web 应用搭建 A/B 测试基础设施
...
terminal-model-specs-description = Phosphor 对模型在执行器中的表现、额度消耗速率和任务速度的基准测试。
...
onboarding-callout-meet-input-title = 认识 Phosphor 输入框
...
onboarding-callout-nl-support-text-prefix = 默认情况下自然语言输入处于关闭状态。启用后，你可以直接用自然语言输入请求，Phosphor 会为智能体自动识别查询。你也始终可以使用
onboarding-callout-enable-nl-detection = 启用自然语言识别
onboarding-callout-new-agent-title = 认识 Phosphor 全新的智能体体验
...
language-widget-secondary = 重启 Phosphor 以让此更改完全生效。
language-restart-required-title = 语言已切换
language-restart-required-body = Phosphor 的界面语言已更新。部分文字会立即切换，完整生效需要重启 Phosphor。
...
settings-section-warp-agent = Phosphor 智能体
...
# about_page.rs
settings-about-copyright = 版权所有 2026 Phosphor
settings-about-automatic-updates-label = 自动检查更新
settings-about-automatic-updates-description = 开启后，Phosphor 会在后台定期检查是否有新版本;发现新版本后自动下载安装包到本地缓存,等您主动点击「立即安装」时才会启动安装程序,期间不会改动当前正在运行的 Phosphor。
...
settings-about-update-install-hint-macos = 安装包将打开,请把 Phosphor 拖到「应用程序」文件夹完成升级。
settings-about-update-install-hint-windows = 安装向导将启动,按提示完成升级。
settings-about-update-install-hint-linux = AppImage 将就地更新并重启 Phosphor。
...
# ---- list_page.rs ----
settings-mcp-list-description = 添加 MCP 服务器以扩展 Phosphor Agent 的能力。MCP 服务器通过标准化接口向 agent 暴露数据源或工具，本质上类似插件。你可以添加自定义服务器，或使用预设快速开始使用流行的服务器。
...
settings-mcp-list-section-shared-by-warp-and-team = 来自 Phosphor 和 { $name }
settings-mcp-list-section-shared-by-warp-and-other-devices = 由 Phosphor 和其他设备共享
settings-mcp-list-section-shared-from-warp = 来自 Phosphor 的共享
...
settings-platform-description = 创建并管理 API 密钥，允许本地 agent 访问你的 Phosphor 账户。
    了解更多请访问
...
settings-platform-zero-state-description = 创建密钥以管理对 Phosphor 的外部访问
settings-platform-create-api-key-description-personal = 此 API 密钥与你的用户绑定，可用于向你的 Phosphor 账户发起请求。
settings-platform-create-api-key-description-team = 此 API 密钥与你的团队绑定，可用于代表你的团队发起请求。
settings-platform-create-api-key-name-placeholder = Phosphor API 密钥
...
settings-referrals-page-title = 邀请朋友加入 Phosphor
settings-referrals-anonymous-header = 本地 Phosphor 构建不提供推荐计划
...
settings-referrals-reward-intro = 推荐朋友即可获得 Phosphor 专属周边*
...
settings-warpify-description-prefix = 配置 Phosphor 是否尝试对特定 Shell 执行 "Phosphorize"（为其添加命令块、输入模式等支持）。
...
settings-warpify-section-ssh-subtitle = 对交互式 SSH 会话启用 Phosphorize。
...
settings-warpify-enable-ssh = 对 SSH 会话启用 Phosphorize
settings-warpify-install-ssh-extension = 安装 SSH 扩展
settings-warpify-install-ssh-extension-description = 控制远程主机未安装 Phosphor 的 SSH 扩展时的安装行为。
settings-warpify-use-tmux = 使用 Tmux Phosphorize
settings-warpify-tmux-description = tmux ssh 包装器在许多默认方式无效的场景下能正常工作，但可能需要你手动点击按钮才能 Phosphorize。在新标签页中生效。
settings-warpify-ssh-tmux-toggle-binding-label = 用于 Phosphorize 的 SSH 会话检测
...
settings-ai-command-denylist-description = 匹配命令的正则表达式，Phosphor 智能体执行这些命令前必须征得许可。
settings-ai-command-allowlist = 命令允许列表
settings-ai-command-allowlist-description = 匹配命令的正则表达式，Phosphor 智能体可自动执行这些命令。
...
settings-ai-mcp-allowlist-description = 允许 Phosphor 智能体调用这些 MCP 服务器。
settings-ai-mcp-denylist = MCP 拒绝列表
settings-ai-mcp-denylist-description = Phosphor 智能体调用此列表中的任何 MCP 服务器前都必须征得许可。
settings-ai-info-banner-managed-by-workspace = 你的部分权限由工作区管理。

# 模型 / 配置
settings-ai-base-model = 基础模型
settings-ai-base-model-description = 此模型作为 Phosphor 智能体背后的主要引擎，驱动大部分交互，并在需要时调用其他模型完成规划或代码生成等任务。Phosphor 可能根据模型可用性自动切换备用模型，或将其用于会话摘要等辅助任务。
settings-ai-show-model-picker-in-prompt = 在提示中显示模型选择器
settings-ai-codebase-context = 代码库上下文
settings-ai-codebase-context-description = 允许 Phosphor 智能体生成代码库的概要作为上下文。代码从不存储到我们的服务器。
...
settings-ai-auto-approve-bypasses-command-denylist = 允许自动批准绕过命令拒绝列表
settings-ai-auto-approve-bypasses-command-denylist-description = 启用后，快进和自动批准将直接运行拒绝列表中的命令，不再请求确认。
...
settings-ai-rules-description = 规则帮助 Phosphor 智能体遵循你的约定，无论是针对代码库还是特定工作流。
...
settings-ai-prompt-template-dir = System prompt 模板目录
settings-ai-prompt-template-dir-description = 从该目录加载 system prompt 模板与 tool 描述,而非使用内置版本;改完存盘即生效,无需重新编译。留空则用内置版本。缺失或语法错误的文件会逐个回退。环境变量 ZAP_PROMPT_DIR 优先级更高。
settings-ai-prompt-template-dir-placeholder = /path/to/prompts
settings-ai-prompt-template-seed = 导出内置模板
settings-ai-prompt-template-status-inactive = ○ 未启用 —— 使用内置模板。设置此目录前,磁盘上的改动不会生效。
settings-ai-prompt-template-status-active = ● 已启用 —— 已从 { $dir } 加载 { $on_disk }/{ $total } 个文件。保存的改动在下次请求生效。
settings-ai-prompt-template-status-active-env = ● 已通过 ZAP_PROMPT_DIR 启用 —— 已从 { $dir } 加载 { $on_disk }/{ $total } 个文件。保存的改动在下次请求生效。
...
settings-ai-auto-show-rich-input-tooltip = 需要为你的编码智能体安装通知插件
...
settings-ai-cloud-agent-computer-use-description = 在 Phosphor 应用中启动的智能体会话中启用计算机使用。

# AWS Bedrock
settings-ai-aws-bedrock-toggle = 使用 AWS Bedrock 凭证
settings-ai-aws-bedrock-description = Phosphor 加载并发送本地 AWS CLI 凭证以使用 Bedrock 支持的模型。
settings-ai-aws-bedrock-description-managed = Phosphor 加载并发送本地 AWS CLI 凭证以使用 Bedrock 支持的模型。此设置由你的组织管理。
...
settings-features-ssh-wrapper = Phosphor SSH 包装器
settings-features-receive-desktop-notifications = 接收来自 Phosphor 的桌面通知
...
settings-features-make-default-terminal = 将 Phosphor 设为默认终端
...
settings-features-default-terminal-current = Phosphor 已是默认终端
...
settings-features-restart-warp-to-apply = 重启 Phosphor 以使更改生效。
...
settings-footer-alert-fix-with-oz = 用 Phosphor Agent 修复
...
settings-code-codebase-context-desc = 允许 Agent 检索已打开仓库的嵌入索引，从而直接定位相关代码，而不必逐个 grep。代码片段仅发送给你所配置的嵌入服务商。若仓库位于 SSH 主机上，索引会在该主机上构建；开启此项即会把你的嵌入服务商地址和 API 密钥发送给该主机，以便主机自行调用服务商。
...
settings-code-remote-indexed-folders-desc = 已请求远程 Agent 守护进程建立索引的 SSH 主机仓库，以及各主机回报的状态。嵌入计算在主机上进行，不会经过任何 Phosphor 服务器。但只要上方的“代码库索引”处于开启状态，你连接的每一台主机都会收到你的嵌入服务商地址和 API 密钥，以便主机直接调用该服务商来嵌入你的代码。关闭该设置会从已连接的主机撤回密钥，并停止向新主机发送。请仅连接你愿意托付该密钥的主机。
...
settings-exec-profile-editor-section-prompts = 系统提示词
settings-exec-profile-editor-prompts-desc = 为每个模型槽选择系统提示词:Auto 按模型家族自动选内置模板;也可指定某个内置模板,或提示词模板目录下 “custom” 文件夹里的自定义文件。
settings-exec-profile-editor-prompt-auto = 自动
settings-exec-profile-editor-prompt-base = 基础提示词
settings-exec-profile-editor-prompt-coding = 编码提示词
settings-exec-profile-editor-prompt-cli-agent = 完整终端使用提示词
settings-exec-profile-editor-prompt-computer-use = 电脑操作提示词
settings-exec-profile-editor-prompt-title = 标题生成提示词
settings-exec-profile-editor-prompt-suggestions = 提示建议提示词
settings-exec-profile-editor-prompt-nld-predict = 输入补全提示词
settings-exec-profile-editor-prompt-relevant-files = 相关文件提示词
settings-exec-profile-editor-prompt-workflow-metadata = 工作流元数据提示词
settings-exec-profile-editor-prompt-next-command = 下一条命令提示词
settings-exec-profile-editor-prompt-base-desc = 主 Agent 系统提示词,由基础模型用于主对话。
settings-exec-profile-editor-prompt-coding-desc = 编码子 Agent 的系统提示词。
settings-exec-profile-editor-prompt-cli-agent-desc = 完整终端使用(CLI)Agent 的系统提示词。
settings-exec-profile-editor-prompt-computer-use-desc = 电脑操作模型的系统提示词。
settings-exec-profile-editor-prompt-title-desc = 根据首条消息生成会话/标签页标题。
settings-exec-profile-editor-prompt-suggestions-desc = 命令运行后,建议可向 Agent 追问的提示。
settings-exec-profile-editor-prompt-nld-predict-desc = 在 Agent 输入框打字时实时自动补全。
settings-exec-profile-editor-prompt-relevant-files-desc = 筛选与当前查询相关的文件。
settings-exec-profile-editor-prompt-workflow-metadata-desc = 在工作流编辑器中,根据命令自动填充参数化元数据。
settings-exec-profile-editor-prompt-next-command-desc = 建议下一条命令(灰色内联/zero-state 建议)。
settings-exec-profile-editor-prompts-custom-location = 自定义文件:
settings-exec-profile-editor-prompts-builtin-location = 内置模板:
settings-exec-profile-editor-prompts-no-dir = 尚未设置提示词模板目录 —— 请在 设置 → AI 中配置后即可添加自定义提示词文件。
settings-exec-profile-editor-section-permissions = 权限
settings-exec-profile-editor-base-model = 基础模型
settings-exec-profile-editor-base-model-desc = 该模型作为智能体的主要引擎，驱动绝大多数交互，并在需要时调用其他模型完成规划或代码生成等任务。Phosphor 可能基于模型可用性或辅助任务（如对话摘要）自动切换到备选模型。
...
settings-exec-profile-editor-command-allowlist-desc = 用于匹配可被 Phosphor Agent 自动执行的命令的正则表达式。
settings-exec-profile-editor-command-denylist = 命令拒绝列表
settings-exec-profile-editor-command-denylist-desc = 用于匹配 Phosphor Agent 必须每次询问权限才能执行的命令的正则表达式。
settings-exec-profile-editor-mcp-allowlist = MCP 允许列表
settings-exec-profile-editor-mcp-allowlist-desc = 允许被 Phosphor Agent 调用的 MCP 服务器。
settings-exec-profile-editor-mcp-denylist = MCP 拒绝列表
settings-exec-profile-editor-mcp-denylist-desc = 不允许被 Phosphor Agent 调用的 MCP 服务器。
...
settings-show-blocks-confirm-dialog-text = 确定要取消共享此命令块吗？
    ...
    取消后将无法通过链接访问，并将从 Phosphor 服务器永久删除。
...
settings-appearance-custom-icon-restart-warning = 你可能需要重启 Phosphor 才能让 macOS 应用所选图标样式。
...
settings-appearance-input-mode-pinned-bottom = 固定在底部（Phosphor 模式）
...
settings-environments-page-description = 环境定义了你的 ambient agent 在哪里运行。可通过 GitHub（推荐）、Phosphor 辅助配置或手动配置在几分钟内创建一个。
...
settings-environments-section-team-default = 由 Phosphor 和本地设备提供
settings-environments-section-team-named = 由 Phosphor 和 { $team } 共享
...
quit-warning-title-app = 退出 Phosphor？
...
keybinding-desc-workspace-toggle-wire-inspector = 切换 BYOP 线路检查器
...
keybinding-desc-workspace-terminate-app = 退出 Phosphor
...
keybinding-desc-workspace-install-cli = 安装 Phosphor Agent CLI 命令
keybinding-desc-workspace-uninstall-cli = 卸载 Phosphor Agent CLI 命令

# AI 助手 / agent
keybinding-desc-workspace-toggle-ai-assistant = 切换 Phosphor AI
...
keybinding-desc-workspace-show-settings-about-menu = 关于 Phosphor
keybinding-desc-workspace-show-settings-warpify = 打开设置：Phosphorize
keybinding-desc-workspace-show-settings-warpify-menu = 配置 Phosphorize...
...
keybinding-desc-workspace-send-feedback-oz = 用 Phosphor Agent 发送反馈
keybinding-desc-workspace-view-logs = 查看 Phosphor 日志
...
keybinding-desc-workspace-init-project = 为 Phosphor 初始化项目
...
keybinding-desc-workspace-open-oz-launch-modal = [调试] 打开 Phosphor Agent 启动弹窗
keybinding-desc-workspace-reset-oz-launch-modal-state = [调试] 重置 Phosphor Agent 启动弹窗状态
keybinding-desc-workspace-open-zap-launch-modal = [调试] 打开 Phosphor 启动弹窗
keybinding-desc-workspace-reset-zap-launch-modal-state = [调试] 重置 Phosphor 启动弹窗状态
...
# 终端输入相关 binding
keybinding-desc-input-show-network-log = 显示 Phosphor 网络日志
...
keybinding-desc-terminal-warpify-subshell = Phosphorize 子 shell
keybinding-desc-terminal-warpify-ssh-session = Phosphorize SSH 会话
...
keybinding-desc-ai-assistant-close = 关闭 Phosphor AI
keybinding-desc-ai-assistant-focus-terminal-input = 从 Phosphor AI 切回终端输入
keybinding-desc-ai-assistant-restart = 重启 Phosphor AI
...
keybinding-desc-terminal-ask-ai-selection = 就所选内容询问 Phosphor AI
keybinding-desc-terminal-ask-ai-last-block = 就最近的命令块询问 Phosphor AI
keybinding-desc-terminal-ask-ai = 询问 Phosphor AI
...
toggle-enable-auto-approve-bypasses-command-denylist = 允许自动批准绕过命令拒绝列表
toggle-disable-auto-approve-bypasses-command-denylist = 自动批准时仍需确认拒绝列表中的命令
...
toggle-suffix-ssh-wrapper = Phosphor SSH 包装器
...
settings-external-editor-choose-layout = 选择在 Phosphor 中打开文件的布局
settings-external-editor-tabbed-header = 多个文件合并到同一编辑器面板
settings-external-editor-tabbed-desc = 开启后，同一标签页中打开的文件会自动归并到单一编辑器面板。
settings-external-editor-prefer-markdown = 默认用 Phosphor Markdown 查看器打开 Markdown 文件
...
menu-block-open-in-warp = 在 Phosphor 中打开
...
menu-block-ask-warp-ai = 询问 Phosphor AI
...
menu-input-ask-warp-ai = 询问 Phosphor AI
...
slash-cmd-create-environment-desc = 通过引导式流程创建 Phosphor Agent 环境（Docker 镜像 + 仓库）
slash-cmd-create-environment-hint = <可选：仓库路径或 GitHub URL>
slash-cmd-docker-sandbox-desc = 创建新的 Docker 沙盒终端会话
slash-cmd-create-new-project-desc = 由 Phosphor Agent 引导你创建新的代码项目
slash-cmd-create-new-project-hint = <描述你想构建什么>
slash-cmd-open-skill-desc = 在 Phosphor 内置编辑器中打开技能的 markdown 文件
slash-cmd-skills-desc = 调用技能
slash-cmd-add-prompt-desc = 添加新的智能体提示词
slash-cmd-add-rule-desc = 为智能体添加新的全局规则
slash-cmd-open-file-desc = 在 Phosphor 代码编辑器中打开文件
...
prompt-editor-warp-prompt-section = Phosphor 终端提示词
...
welcome-tips-command-palette-description = 无需双手离开键盘，即可轻松发现 Phosphor 的全部功能。
...
welcome-tips-theme-picker-description = 选择内置主题，让 Phosphor 更符合你的风格。也可以创建自己的主题。
...
workflow-ai-assist-tooltip = 使用 Phosphor AI 生成标题、描述或参数
...
workspace-left-panel-wire-inspector = BYOP 线路检查器
...
wasm-nux-open-desktop-title = 在 Phosphor 桌面版中打开？
wasm-nux-open-desktop-detail = 之后的链接会自动在桌面版中打开。
wasm-nux-open-desktop-confirm = 在 Phosphor 中打开
wasm-nux-download-title = 下载 Phosphor 桌面版？
wasm-nux-download-description = Phosphor 是一款内置 AI 和团队知识的智能终端。
wasm-nux-learn-more = 了解更多
wasm-nux-download-confirm = 下载
wasm-nux-object-kind-drive-objects = 库对象
wasm-nux-object-kind-warp-links = Phosphor 链接
...
auth-override-warning-description = 看起来你刚刚通过网页浏览器登录了一个 Phosphor 账号。如果继续，当前匿名会话中的所有个人库对象和偏好都将被永久删除。
...
auth-override-warning-accessibility-help = Phosphor 检测到你通过网页浏览器进行了新的登录。按 Esc 键取消，并继续在未登录状态下使用 Phosphor。
...
auth-needs-sso-link-detail = 点击下方按钮，将你的 Phosphor 账号关联到你的 SSO 提供商。
...
auth-offline-first-use-description = 你当前处于离线状态。首次使用 Phosphor 时需要联网。
auth-offline-first-use-learn-more = 了解更多
auth-offline-overlay-title = 离线使用 Phosphor
auth-offline-overlay-paragraph-1 = Phosphor 的本地功能可以离线使用。
auth-offline-overlay-paragraph-2 = 首次使用时可能需要联网完成依赖初始化，之后本地智能体和终端功能不依赖 Phosphor 云服务。
auth-offline-overlay-paragraph-3 = Phosphor 会把必要状态保存在本机，用于本地会话、设置和智能体功能。
auth-offline-overlay-dismiss = 关闭
auth-privacy-settings-title = 隐私设置
auth-privacy-settings-done = 完成
auth-privacy-settings-help-improve = 帮助改进 Phosphor
auth-privacy-settings-help-improve-description = 高层级的功能使用数据可以帮助 Phosphor 产品团队确定路线图优先级。
auth-privacy-settings-learn-more = 了解更多
auth-privacy-settings-send-crash-reports = 发送崩溃报告
auth-privacy-settings-crash-reports-description = 崩溃报告能帮助 Phosphor 工程团队了解稳定性情况并改进性能。
...
cli-agent-plugin-codex-install-title = 为 Codex 启用 Phosphor 通知
cli-agent-plugin-codex-install-subtitle = 将 Codex 更新到最新版本，然后启用聚焦时通知，这样你工作时 Phosphor 就能显示通知。
...
cli-agent-plugin-deepseek-install-title = 为 DeepSeek 启用 Phosphor 通知
...
cli-agent-plugin-claude-install-title = 安装 Claude Code 的通知插件
cli-agent-plugin-claude-install-subtitle = 确保你的机器已安装 jq，然后运行这些命令。
cli-agent-plugin-claude-add-marketplace-step = 添加插件市场仓库
cli-agent-plugin-install-warp-plugin-step = 安装插件
...
cli-agent-plugin-claude-update-title = 更新 Claude Code 的通知插件
...
cli-agent-plugin-gemini-install-title = 安装 Gemini CLI 的通知插件
cli-agent-plugin-gemini-run-command-restart = 运行以下命令，然后重启 Gemini CLI。
cli-agent-plugin-install-warp-extension-step = 安装扩展
cli-agent-plugin-gemini-restart-note = 重启 Gemini CLI 以启用插件。
cli-agent-plugin-gemini-update-title = 更新 Gemini CLI 的通知插件
cli-agent-plugin-update-warp-extension-step = 更新扩展
cli-agent-plugin-gemini-restart-update-note = 重启 Gemini CLI 以启用更新。
cli-agent-plugin-opencode-install-title = 安装 OpenCode 的通知插件
cli-agent-plugin-opencode-install-subtitle = 将插件加入 OpenCode 配置，然后重启 OpenCode。
...
cli-agent-plugin-opencode-update-title = 更新 OpenCode 的通知插件
...
terminal-bootstrapping-installing-warp-ssh-extension-progress = 正在安装 Phosphor SSH 扩展...（{ $p }%）
terminal-bootstrapping-installing-warp-ssh-extension = 正在安装 Phosphor SSH 扩展...
terminal-bootstrapping-updating-warp-ssh-extension = 正在更新 Phosphor SSH 扩展...
...
agent-tip-command-palette = 按 <keybinding> 打开命令面板，访问 Phosphor 操作和快捷方式。
...
agent-tip-oz-headless = 使用 `oz` 命令以 headless 模式运行 Phosphor Agent，适合远程机器。
...
agent-tip-warpify-ssh = 对远程 SSH 会话执行 Phosphorize，即可在该环境中启用 Phosphor Agent。
...
hoa-welcome-banner-title = 引入通用智能体支持：用 Phosphor 增强任何编码智能体
...
hoa-feature-native-code-review-description = 将 Phosphor 代码审查中的行内评论直接发送到 Claude Code、Codex 或 OpenCode
resource-center-whats-new-section = 新变化
resource-center-getting-started-section = 入门
resource-center-maximize-warp-section = 充分使用 Phosphor
...
resource-center-command-palette-description = 通过键盘访问 Phosphor 的全部功能。
resource-center-set-theme-title = 设置主题
resource-center-set-theme-description = 选择主题，让 Phosphor 符合你的偏好。
resource-center-custom-prompt-title = 使用自定义提示符
resource-center-custom-prompt-description = 设置 Phosphor 以遵循你的 PS1 设置
resource-center-view-documentation = 查看文档
resource-center-integrate-ide-title = 将 Phosphor 与 IDE 集成
resource-center-integrate-ide-description = 配置 Phosphor，从你最常用的开发工具中启动
resource-center-how-warp-uses-warp-title = Phosphor 如何使用 Phosphor
resource-center-how-warp-uses-warp-description = 了解 Phosphor 工程团队如何使用他们最喜欢的功能
...
onboarding-intention-title = 欢迎使用 Phosphor
...
onboarding-ai-feature-warp-agents = Phosphor 智能体
onboarding-ai-feature-oz-cloud-agents-platform = Phosphor Agent 本地智能体平台
...
onboarding-agent-title = 自定义你的 Phosphor 智能体
...
onboarding-agent-disable-warp-agent = 禁用 Phosphor 智能体
...
onboarding-project-subtitle = 设置项目，让 Phosphor 更适合编码。
...
onboarding-customize-title = 自定义你的 Phosphor
...
onboarding-free-user-agent-title = 使用 Phosphor 内置智能体进行开发
onboarding-free-user-agent-description = 使用 Phosphor Agent（Phosphor 内置智能体）迭代、规划和构建。本地运行，不依赖 Phosphor 云服务。
...
onboarding-free-user-subscribe-title = 订阅后即可使用 Phosphor 的智能体开发能力。
...
onboarding-free-user-subscribe-item-drive = 不限量 Phosphor Drive 对象和协作
onboarding-free-user-subscribe-item-support = 私人邮件支持
onboarding-free-user-subscribe-item-cloud-storage = 本地对话存储

auth-opt-out-line-1 = Phosphor 会把引导选择保存在本机。
...
auth-local-privacy-note = Phosphor 会把引导选择保存在本设备。
...
auth-disable-ai-features-confirm-body = 开启 AI 后 Phosphor 会更强大。继续后，你将无法使用以下功能：
...
auth-require-login-ai-collaboration = 本地 AI 功能不需要 Phosphor 账户。
auth-require-login-drive-limit = Phosphor 会将库对象保存在本机。
auth-require-login-share = 本地 Phosphor 构建不提供分享功能。
auth-welcome-title = 欢迎使用 Phosphor！
auth-sign-up-for-warp = 继续使用 Phosphor
...
get-started-welcome-title = 欢迎使用 Phosphor
...
terminal-banner-p10k-supported = Powerlevel10k 现在支持 Phosphor！{"  "}
...
terminal-banner-pure-unsupported = Phosphor 尚不支持 Pure。你可以考虑改用受支持的提示符。{"  "}
...
ai-footer-plugin-installed-restart-session = 通知插件已安装。请重启会话以启用。
ai-footer-installing-warp-plugin = 正在安装通知插件...
ai-footer-failed-install-warp-plugin = 安装通知插件失败
ai-footer-plugin-updated-restart-session = 通知插件已更新。请重启会话以启用。
ai-footer-updating-warp-plugin = 正在更新通知插件...
ai-footer-failed-update-warp-plugin = 更新通知插件失败
...
settings-import-reset-to-warp-defaults = 重置为 Phosphor 默认设置
...
workspace-notification-permission-denied-toast = Phosphor 没有发送桌面通知的权限。
...
workspace-update-warp = 更新 Phosphor
...
workspace-version-deprecation-without-permissions-banner = 若不立即更新，部分 Phosphor 功能可能无法正常工作，但 Phosphor 无法执行更新。
workspace-new-version-unable-to-update-banner = 有新版本可用，但 Phosphor 无法执行更新。
workspace-unable-to-launch-new-installed-version = Phosphor 无法启动新安装的版本。
...
terminal-warpify-without-tmux = 不使用 TMUX 进行 Phosphorize
terminal-continue-without-warpification = 不进行 Phosphorization 继续
terminal-always-install = 始终安装
terminal-never-install = 从不安装
terminal-ssh-report-issue-prefix = 我们正在积极改进 Phosphor 中 SSH 的稳定性。请考虑
...
terminal-agent-mode-setup-title = 为此代码库优化 Phosphor？
...
terminal-shell-premature-subtext = 启动 { $shell_detail } 并对其进行 Phosphorize 时出错，导致进程终止。这里显示了 Phosphorize 脚本输出，可能能帮助定位原因。
...
notifications-banner-permissions-denied-title = Phosphor 被拒绝发送通知的权限。
...
tooltip-secrets-not-sent-to-warp-server = *密钥不会发送到 Phosphor 服务器。
