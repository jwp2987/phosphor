# Phosphor Desktop — English (source-of-truth locale)
# This file is edited in parallel by multiple agents, each maintaining their
# own SECTION; keys are isolated by a surface prefix to avoid conflicts.
# When adding a key, ctrl-F to find the corresponding SECTION header and
# append there; a new surface gets a new SECTION at the end of the file.
#
# Naming convention: kebab-case, prefixed by surface, e.g.
# settings-ai-title / drive-folder-rename-title
# Use Fluent's { $name } syntax for variable interpolation, not string concatenation
...
agent-management-source-local-agent = Phosphor (local agent)
agent-management-source-cloud-agent = Phosphor agent
agent-management-source-oz-web = Web
...
workspace-menu-update-warp-manually = Update Phosphor manually
...
workspace-menu-view-warp-logs = View Phosphor logs
...
workspace-toast-all-conversations-deleted = Deleted { $count } conversations
workspace-toast-no-conversations-to-delete = No conversations to delete
...
app-menu-hide-warpified-ssh-blocks = Hide Phosphorized SSH Blocks
...
app-menu-help = Help
app-menu-warp-documentation = Phosphor Documentation...
app-menu-github-issues = GitHub Issues...
app-menu-warp-slack-community = Phosphor Slack Community...
workspace-update-and-relaunch-warp = Update and relaunch Phosphor
workspace-updating-to-version = Updating to ({ $version })
workspace-update-warp-manually = Update Phosphor manually
...
terminal-banner-completions-not-working-middle = ). Enabling tmux phosphorization in {" "}
terminal-banner-settings = settings
terminal-banner-completions-not-working-suffix =  may resolve this issue.
terminal-banner-shell-config-incompatible = Your shell configuration is incompatible with Phosphor...{"  "}
...
terminal-toast-conversation-only-no-input = This conversation has no running session to send input to
terminal-toast-link-invalid = Blocked link: not a valid URL
terminal-toast-link-blocked-scheme = Blocked link: { $scheme }: links aren't opened from terminal output
...
ssh-remote-choice-continue-without-installing-desc = You'll still get a Phosphorized experience just without the agent features.
...
ai-footer-enable-notifications-tooltip = Install your coding agent's notification plugin to enable rich agent notifications within Phosphor
ai-footer-notifications-setup-instructions = Notifications setup instructions
ai-footer-install-plugin-instructions-tooltip = View instructions to install the notification plugin
ai-footer-update-warp-plugin = Update notification plugin
ai-footer-plugin-update-available-tooltip = A new version of the notification plugin is available
ai-footer-plugin-update-instructions = Plugin update instructions
ai-footer-plugin-update-instructions-tooltip = View instructions to update the notification plugin
...
zap-launch-title = Phosphor is now open-source
zap-launch-description = You, our community, can participate in building Phosphor using an agent-first workflow.
zap-launch-contribute-title = Contribute
zap-launch-contribute-description = Phosphor's client code is now open source. Get started by using the /feedback skill to open an issue, and follow the contribution guidelines here.
zap-launch-contribute-link-text = here
zap-launch-oad-title = Open Automated Development
zap-launch-oad-description = The Phosphor repo is managed by an agent-first local workflow powered by Phosphor Agent.
...
settings-agent-providers-vertex-project-placeholder = my-gcp-project-123
settings-agent-providers-vertex-location-placeholder = us-east5 (or global)
...
terminal-warpify-subshell = Phosphorize subshell
terminal-warpify-subshell-tooltip = Enable Phosphor shell integration in this session
terminal-use-agent = Use agent
terminal-use-agent-tooltip = Ask the Phosphor agent to assist
terminal-give-control-back-to-agent = Give control back to agent
terminal-resume-agent-tooltip = Ask the Phosphor agent to resume
...
terminal-open-in-warp = Open in Phosphor
terminal-open-conversation-in-warp-tooltip = Open this conversation in the Phosphor desktop app
...
terminal-model-specs-description = Phosphor's benchmarks for how well a model performs in our harness, the rate at which it consumes credits, and task speed.
...
code-review-stage-file = Stage file
code-review-stage-remaining = Stage remaining changes
code-review-unstage-file = Unstage file
...
onboarding-callout-meet-input-title = Meet the Phosphor input
...
onboarding-callout-nl-support-text-prefix = Natural language input is off by default. If enabled, you can type requests in plain English and Phosphor will autodetect queries for the agent. You can always override them using
onboarding-callout-enable-nl-detection = Enable Natural Language Detection
onboarding-callout-new-agent-title = Introducing Phosphor's new agent experience
...
language-widget-secondary = Restart Phosphor for the change to fully take effect.
language-restart-required-title = Language changed
language-restart-required-body = Phosphor's UI language has been updated. Some text will switch immediately, but a full restart is required for the change to take effect everywhere.
...
# Phosphor: settings-section-account removed alongside the Account settings page.
...
# Strings for settings_view/about_page.rs + main_page.rs go under this anchor
...
# brand-guard: allow -- AGPL-3.0 §13 attribution; naming upstream here is required to be true.
...
settings-about-update-up-to-date = Phosphor is up to date.
...
settings-about-update-install-hint-macos = The installer will open — drag Phosphor into your Applications folder to finish.
settings-about-update-install-hint-windows = The setup wizard will launch — follow the prompts to finish the upgrade.
settings-about-update-install-hint-linux = The AppImage will be replaced in place and Phosphor will restart.
...
# Phosphor: main_page.rs (Account / version / autoupdate) strings removed alongside
# the Account settings page. The About page now owns version / update CTAs.


# --- ANCHOR-SUB-MCP (agent-settings-mcp) ---
# Strings for settings_view/mcp_servers_page.rs go under this anchor
...
# ---- list_page.rs ----
settings-mcp-list-description = Add MCP servers to extend the Phosphor Agent's capabilities. MCP servers expose data sources or tools to agents through a standardized interface, essentially acting like plugins. Add a custom server, or use the presets to get started with popular servers.
...
settings-mcp-list-section-shared-by-warp-and-team = Available from Phosphor and { $name }
settings-mcp-list-section-shared-by-warp-and-other-devices = Shared by Phosphor and from other devices
settings-mcp-list-section-shared-from-warp = Shared from Phosphor
...
# Strings for settings_view/platform_page.rs go under this anchor
# Naming prefix: settings-platform-*
settings-platform-section-title = Agent API Keys
settings-platform-description = Create and manage API keys to allow local agents to access your Phosphor account.
...
settings-platform-zero-state-description = Create a key to manage external access to Phosphor
settings-platform-create-api-key-description-personal = This API key is tied to your user and can make requests against your Phosphor account.
...
settings-platform-create-api-key-name-placeholder = Phosphor API Key
...
settings-referrals-page-title = Invite a friend to Phosphor
settings-referrals-anonymous-header = Referral program is unavailable in local Phosphor builds
...
settings-referrals-reward-intro = Get exclusive Phosphor goodies when you refer someone*
...
settings-warpify-page-title = Phosphorize
settings-warpify-description-prefix = Configure whether Phosphor attempts to "Phosphorize" (add support for blocks, input modes, etc) certain shells.
...
settings-warpify-section-ssh-subtitle = Phosphorize your interactive SSH sessions.
...
settings-warpify-enable-ssh = Phosphorize SSH Sessions
settings-warpify-install-ssh-extension = Install SSH extension
settings-warpify-install-ssh-extension-description = Controls the installation behavior for Phosphor's SSH extension when a remote host doesn't have it installed.
settings-warpify-reuse-control-master = Reuse existing SSH ControlMaster
settings-warpify-reuse-control-master-description = Attach to a live SSH ControlMaster you already have configured for the destination host instead of creating a Phosphor-owned one. Takes effect in new tabs.
settings-warpify-use-tmux = Use Tmux Phosphorization
settings-warpify-tmux-description = The tmux ssh wrapper works in many situations where the default one does not, but may require you to hit a button to phosphorize. Takes effect in new tabs.
settings-warpify-ssh-tmux-toggle-binding-label = SSH session detection for Phosphorization
...
settings-ai-command-denylist-description = Regular expressions to match commands that the Phosphor Agent should always ask permission to execute.
settings-ai-command-allowlist = Command allowlist
settings-ai-command-allowlist-description = Regular expressions to match commands that can be automatically executed by the Phosphor Agent.
...
settings-ai-mcp-allowlist-description = Allow the Phosphor Agent to call these MCP servers.
settings-ai-mcp-denylist = MCP denylist
settings-ai-mcp-denylist-description = The Phosphor Agent will always ask for permission before calling any MCP servers on this list.
...
settings-ai-base-model-description = This model serves as the primary engine behind the Phosphor Agent. It powers most interactions and invokes other models for tasks like planning or code generation when necessary. Phosphor may automatically switch to alternate models based on model availability or for auxiliary tasks such as conversation summarization.
settings-ai-show-model-picker-in-prompt = Show model picker in prompt
settings-ai-codebase-context = Codebase Context
settings-ai-codebase-context-description = Allow the Phosphor Agent to generate an outline of your codebase that can be used for context. No code is ever stored on our servers.
...
settings-ai-auto-approve-bypasses-command-denylist-description = When enabled, fast forward and auto-approve run denylisted commands without asking for confirmation.
...
settings-ai-rules-description = Rules help the Phosphor Agent follow your conventions, whether for codebases or specific workflows.
...
settings-ai-prompt-template-dir = System prompt template directory
settings-ai-prompt-template-dir-description = Load system prompt templates and tool descriptions from this directory instead of the built-in ones, picking up edits without a rebuild. Leave empty to use the built-in versions. Missing or invalid files fall back individually. The ZAP_PROMPT_DIR environment variable overrides this.
settings-ai-prompt-template-dir-placeholder = /path/to/prompts
settings-ai-prompt-template-seed = Export built-in templates
settings-ai-prompt-template-status-inactive = ○ Inactive — using built-in templates. Edits on disk will not apply until this directory is set.
settings-ai-prompt-template-status-active = ● Active — { $on_disk } of { $total } files loaded from { $dir }. Saved edits apply on the next request.
settings-ai-prompt-template-status-active-env = ● Active via ZAP_PROMPT_DIR — { $on_disk } of { $total } files loaded from { $dir }. Saved edits apply on the next request.
...
settings-ai-auto-show-rich-input-tooltip = Requires the notification plugin for your coding agent
...
settings-ai-cloud-agent-computer-use-description = Enable computer use in agent conversations started from the Phosphor app.

# AWS Bedrock
settings-ai-aws-bedrock-toggle = Use AWS Bedrock credentials
settings-ai-aws-bedrock-description = Phosphor loads and sends local AWS CLI credentials for Bedrock-supported models.
settings-ai-aws-bedrock-description-managed = Phosphor loads and sends local AWS CLI credentials for Bedrock-supported models. This setting is managed by your organization.
...
settings-features-ssh-wrapper = Phosphor SSH Wrapper
settings-features-receive-desktop-notifications = Receive desktop notifications from Phosphor
...
settings-features-osc52-clipboard-access-label = OSC 52 clipboard access
settings-features-osc52-clipboard-access-description = Controls whether programs running in the terminal (including remote ones, e.g. over SSH) can access your clipboard via OSC 52 escape sequences. Write access lets them set your clipboard; read access also lets them read whatever you last copied, which is riskier.
...
settings-features-make-default-terminal = Make Phosphor the default terminal
...
settings-features-default-terminal-current = Phosphor is the default terminal
...
settings-features-restart-warp-to-apply = Restart Phosphor for changes to take effect.

# --- ANCHOR-SUB-SETTINGS-PAGE-NAV (agent-settings-page-nav) ---
# Strings for settings_view/{settings_page,nav,delete_environment_confirmation_dialog,directory_color_add_picker,pane_manager}.rs go under this anchor
...
settings-footer-alert-fix-with-oz = Fix with Phosphor Agent
...
settings-code-codebase-context-desc = Let the agent search an embedding index of the repositories you open, so it can find relevant code instead of grepping for it. Code fragments go only to the embedding provider you configure. For a repository on an SSH host, the index is built on that host, and turning this on is what sends that host your embedding provider's address and API key so it can call the provider itself.
...
settings-code-remote-indexed-folders = Remote indexed folders
settings-code-remote-indexed-folders-desc = Repositories on SSH hosts that the remote agent daemon has been asked to index, and what each host reports back. Embedding runs on the host and reaches no Phosphor server. While "Codebase indexing" above is on, every host you connect to is sent your embedding provider's address and API key, so it can embed your code by calling the provider directly. Switching that setting off withdraws the key from connected hosts and stops sending it to new ones. Only connect hosts you would trust with that key.
settings-code-remote-index-not-created = No index created
settings-code-remote-index-limit-reached = Index limit reached
settings-code-remote-index-unavailable = Unavailable
settings-code-remote-index-disabled = Disabled
settings-code-remote-index-queued = Queued
settings-code-remote-index-indexing = Indexing...
settings-code-remote-index-indexing-progress = Indexing - { $completed } / { $total }
settings-code-remote-index-indexing-completed = Indexing - { $completed }
...
settings-exec-profile-editor-section-prompts = SYSTEM PROMPTS
settings-exec-profile-editor-prompts-desc = Choose the system prompt for each model slot: Auto picks a built-in by model family; or pick a built-in, or a custom file from the "custom" folder in your prompt template directory.
settings-exec-profile-editor-prompt-auto = Auto
settings-exec-profile-editor-prompt-base = Base prompt
settings-exec-profile-editor-prompt-coding = Coding prompt
settings-exec-profile-editor-prompt-cli-agent = Full terminal use prompt
settings-exec-profile-editor-prompt-computer-use = Computer use prompt
settings-exec-profile-editor-prompt-title = Title generation prompt
settings-exec-profile-editor-prompt-suggestions = Prompt suggestions prompt
settings-exec-profile-editor-prompt-nld-predict = Input completion prompt
settings-exec-profile-editor-prompt-relevant-files = Relevant files prompt
settings-exec-profile-editor-prompt-workflow-metadata = Workflow metadata prompt
settings-exec-profile-editor-prompt-next-command = Next command prompt
settings-exec-profile-editor-prompt-base-desc = The main agent system prompt, used by the base model for the primary conversation.
settings-exec-profile-editor-prompt-coding-desc = System prompt for the coding subagent.
settings-exec-profile-editor-prompt-cli-agent-desc = System prompt for the full-terminal-use (CLI) agent.
settings-exec-profile-editor-prompt-computer-use-desc = System prompt for the computer-use model.
settings-exec-profile-editor-prompt-title-desc = Generates the conversation/tab title from the first message.
settings-exec-profile-editor-prompt-suggestions-desc = Suggests follow-up prompts to ask the agent after a command runs.
settings-exec-profile-editor-prompt-nld-predict-desc = Real-time autocompletion as you type in the agent input box.
settings-exec-profile-editor-prompt-relevant-files-desc = Selects which files are relevant to the current query.
settings-exec-profile-editor-prompt-workflow-metadata-desc = Autofills parameterized metadata from a command in the Workflow editor.
settings-exec-profile-editor-prompt-next-command-desc = Suggests the next command (grey inline / zero-state suggestions).
settings-exec-profile-editor-prompts-custom-location = Custom files:
settings-exec-profile-editor-prompts-builtin-location = built-ins:
settings-exec-profile-editor-prompts-no-dir = No prompt template directory set — configure one in Settings → AI to add custom prompt files.
settings-exec-profile-editor-section-permissions = PERMISSIONS
settings-exec-profile-editor-base-model = Base model
settings-exec-profile-editor-base-model-desc = This model serves as the primary engine behind the agent. It powers most interactions and invokes other models for tasks like planning or code generation when necessary. Phosphor may automatically switch to alternate models based on model availability or for auxiliary tasks such as conversation summarization.
...
settings-exec-profile-editor-command-allowlist-desc = Regular expressions to match commands that can be automatically executed by Phosphor Agent.
settings-exec-profile-editor-command-denylist = Command denylist
settings-exec-profile-editor-command-denylist-desc = Regular expressions to match commands that Phosphor Agent should always ask permission to execute.
settings-exec-profile-editor-mcp-allowlist = MCP allowlist
settings-exec-profile-editor-mcp-allowlist-desc = MCP servers that are allowed to be called by Phosphor Agent.
settings-exec-profile-editor-mcp-denylist = MCP denylist
settings-exec-profile-editor-mcp-denylist-desc = MCP servers that are not allowed to be called by Phosphor Agent.
...
settings-show-blocks-confirm-dialog-text = Are you sure you want to unshare this block?
    ...
    It will no longer be accessible by link and will be permanently deleted from Phosphor servers.
...
# Remaining strings for settings_view/appearance_page.rs go under this anchor (excluding the already-completed Language widget)
...
settings-appearance-custom-icon-restart-warning = You may need to restart Phosphor for MacOS to apply the preferred icon style.
...
settings-appearance-input-type-warp = Phosphor
settings-appearance-input-type-shell = Shell (PS1)
settings-appearance-input-position-label = Input position
settings-appearance-input-mode-pinned-bottom = Pin to the bottom (Phosphor mode)
...
settings-appearance-tab-enable-tab-groups-label = Enable tab groups
...
settings-appearance-host-footer-bar-colors-label = Color by session host
settings-appearance-host-footer-bar-colors-description = Color the window footer bar when a terminal session's host matches a rule below, so a host you don't want to mistake for another -- a production box, say -- is unmistakable. Rules are checked in order; the first match wins.
settings-appearance-host-footer-bar-empty = No rules configured yet.
...
settings-environments-page-description = Environments define where your ambient agents run. Set one up in minutes via GitHub (recommended), Phosphor-assisted setup, or manual configuration.
...
settings-environments-section-team-default = Provided by Phosphor and this device
settings-environments-section-team-named = Shared by Phosphor and { $team }
...
# Strings for settings_view/agent_providers_widget.rs go under this anchor
...
settings-agent-providers-field-vertex-project = GCP Project ID
settings-agent-providers-field-vertex-location = Location
settings-agent-providers-field-vertex-service-account = Service Account email (optional)
settings-agent-providers-vertex-login = Log in with gcloud
settings-agent-providers-vertex-login-launched = Opening gcloud login in your browser — once complete, retry your request.
...
settings-agent-providers-headers-label = Extra Headers
settings-agent-providers-header-key-placeholder = x-portkey-provider
settings-agent-providers-header-value-placeholder = openai
...
settings-agent-providers-hidden-catalog-collapse = ▾ Hidden providers ({$count})
...
settings-agent-providers-fetch-from-api-tooltip = Fetch the model list from this provider's API — IDs only, except Ollama which also fills in the context window
...
quit-warning-title-app = Quit Phosphor?
...
keybinding-desc-workspace-add-conversation-pane = New Conversation Pane
keybinding-desc-workspace-new-conversation-tab = New Conversation Tab
...
keybinding-desc-workspace-toggle-wire-inspector = Toggle BYOP wire inspector
...
keybinding-desc-workspace-install-cli = Install Phosphor CLI command
keybinding-desc-workspace-uninstall-cli = Uninstall Phosphor CLI command
keybinding-desc-workspace-install-warpctrl = Install the warpctrl CLI globally for use outside of Phosphor
keybinding-desc-workspace-uninstall-warpctrl = Undo global warpctrl CLI installation (warpctrl will still work within Phosphor)
...
# Phosphor: keybinding-desc-workspace-show-settings-account removed alongside the
...
keybinding-desc-workspace-show-settings-about-menu = About Phosphor
keybinding-desc-workspace-show-settings-warpify = Open Settings: Phosphorize
keybinding-desc-workspace-show-settings-warpify-menu = Configure Phosphorize...
...
keybinding-desc-workspace-send-feedback-oz = Send feedback with Phosphor Agent
keybinding-desc-workspace-view-logs = View Phosphor logs
...
keybinding-desc-workspace-init-project = Initiate project for Phosphor
...
keybinding-desc-workspace-open-oz-launch-modal = [Debug] Open Phosphor Agent Launch Modal
keybinding-desc-workspace-reset-oz-launch-modal-state = [Debug] Reset Phosphor Agent Launch Modal State
keybinding-desc-workspace-open-zap-launch-modal = [Debug] Open Phosphor Launch Modal
keybinding-desc-workspace-reset-zap-launch-modal-state = [Debug] Reset Phosphor Launch Modal State
...
keybinding-desc-input-show-network-log = Show Phosphor network log
...
keybinding-desc-terminal-warpify-subshell = Phosphorize subshell
keybinding-desc-terminal-warpify-ssh-session = Phosphorize ssh session
...
keybinding-desc-editor-vim-scroll-half-page-down = Scroll down half a page (vim)
keybinding-desc-editor-vim-scroll-half-page-up = Scroll up half a page (vim)
...
keybinding-desc-ai-assistant-close = Close Phosphor AI
keybinding-desc-ai-assistant-focus-terminal-input = Focus Terminal Input From Phosphor AI
keybinding-desc-ai-assistant-restart = Restart Phosphor AI
...
keybinding-desc-terminal-ask-ai-selection = Ask Phosphor AI about Selection
keybinding-desc-terminal-ask-ai-last-block = Ask Phosphor AI about last block
keybinding-desc-terminal-ask-ai = Ask Phosphor AI
...
agent-zero-state-title = New Phosphor Agent conversation
...
toggle-enable-auto-approve-bypasses-command-denylist = Allow auto-approve to bypass command denylist
toggle-disable-auto-approve-bypasses-command-denylist = Require approval for denylisted commands in auto-approve
...
toggle-suffix-ssh-wrapper = Phosphor SSH wrapper
toggle-suffix-ssh-reuse-control-master = reuse existing SSH ControlMaster in the Phosphor SSH wrapper
...
settings-external-editor-choose-layout = Choose a layout to open files in Phosphor
...
settings-external-editor-prefer-markdown = Open Markdown files in Phosphor's Markdown Viewer by default
...
# --- block context menu (terminal/view.rs) ---
...
menu-block-open-in-warp = Open in Phosphor
...
menu-block-ask-warp-ai = Ask Phosphor AI
...
# --- input context menu (terminal/view.rs) ---
menu-input-cut = Cut
...
menu-input-ask-warp-ai = Ask Phosphor AI
...
# --- tab context menu (tab.rs) ---
menu-tab-stop-sharing = Stop sharing
menu-tab-stop-sharing-all = Stop sharing all
menu-tab-copy-link = Copy link
# Metadata-copy entries. Only the tab title is offered on the horizontal tab
# bar; the vertical-tabs menus add branch / working directory / PR link, and
# swap the title entry for the pane title in "panes" display granularity.
menu-tab-copy-tab-title = Copy tab title
...
# --- code editor context menu (code/local_code_editor.rs) ---
...
slash-cmd-create-environment-desc = Create a Phosphor Agent environment (Docker image + repos) via guided setup
...
slash-cmd-create-new-project-desc = Have Phosphor Agent walk you through creating a new coding project
slash-cmd-create-new-project-hint = <describe what you want to build>
slash-cmd-open-skill-desc = Open a skill's markdown file in Phosphor's built-in editor
...
slash-cmd-open-file-desc = Open a file in Phosphor's code editor
...
slash-cmd-orchestrate-hint = <task>[; <task>...] one child agent per task
...
slash-cmd-copy-debugging-id-desc = Copy this conversation's debugging id to attach to a Phosphor issue
slash-cmd-copy-debugging-id-copied = Debugging id copied to clipboard — attach it to your Phosphor issue
slash-cmd-copy-debugging-id-none = No debugging id for this conversation yet.
...
prompt-editor-warp-prompt-section = Phosphor terminal prompt
...
welcome-tips-command-palette-description = Easily discover everything you can do in Phosphor without your hands leaving the keyboard.
...
welcome-tips-theme-picker-description = Make Phosphor your own by choosing a built-in theme. Or create your own.
...
workflow-ai-assist-tooltip = Generate a title, descriptions, or parameters with Phosphor AI
...
workspace-left-panel-wire-inspector = BYOP Wire Inspector
...
vertical-tabs-terminal-kind-oz = Phosphor Agent
vertical-tabs-pane-kind-terminal = Terminal
vertical-tabs-pane-kind-conversation = Conversation
...
wasm-nux-open-desktop-title = Open in Phosphor Desktop?
wasm-nux-open-desktop-detail = Future links will automatically open on desktop.
wasm-nux-open-desktop-confirm = Open in Phosphor
wasm-nux-download-title = Download Phosphor Desktop?
...
auth-override-warning-accessibility-help = Phosphor has detected a new login from a web browser. Press escape to cancel and continue using Phosphor without login.
...
auth-needs-sso-link-detail = Click the button below to link your Phosphor account to your SSO provider.
...
auth-offline-first-use-description = You are currently offline. An internet connection is required to use Phosphor for the first time.
auth-offline-first-use-learn-more = Learn more
auth-offline-overlay-title = Using Phosphor Offline
auth-offline-overlay-paragraph-1 = Phosphor can be used offline for local terminal and agent workflows.
...
auth-privacy-settings-help-improve = Help improve Phosphor
auth-privacy-settings-help-improve-description = High-level feature usage data helps Phosphor's product team prioritize the roadmap.
auth-privacy-settings-learn-more = Learn more
auth-privacy-settings-send-crash-reports = Send crash reports
auth-privacy-settings-crash-reports-description = Crash reporting helps Phosphor's engineering team understand stability and improve performance.
...
cli-agent-plugin-codex-install-title = Enable Phosphor Notifications for Codex
cli-agent-plugin-codex-install-subtitle = Update Codex to the latest version, then enable in-focus notifications so Phosphor can display them while you work.
...
cli-agent-plugin-codex-plugin-install-title = Install Notification Plugin for Codex
cli-agent-plugin-codex-plugin-run-commands-subtitle = Run the following commands, then restart Codex.
cli-agent-plugin-codex-add-marketplace-step = Add the plugin marketplace repository
cli-agent-plugin-codex-plugin-restart-note = Restart Codex to activate the plugin.
cli-agent-plugin-codex-plugin-update-title = Update Notification Plugin for Codex
cli-agent-plugin-codex-upgrade-marketplace-step = Upgrade the marketplace
cli-agent-plugin-codex-reinstall-plugin-step = Reinstall the plugin
cli-agent-plugin-codex-plugin-restart-update-note = Restart Codex to activate the update.
cli-agent-plugin-codex-marketplace-not-git-note = If this fails because codex-warp is not configured as a Git marketplace, remove and re-add the marketplace.
cli-agent-plugin-deepseek-install-title = Enable Phosphor Notifications for DeepSeek
...
cli-agent-plugin-claude-install-title = Install Notification Plugin for Claude Code
cli-agent-plugin-claude-install-subtitle = Ensure that jq is installed on your machine. Then, run these commands.
cli-agent-plugin-claude-add-marketplace-step = Add the plugin marketplace repository
cli-agent-plugin-install-warp-plugin-step = Install the plugin
...
cli-agent-plugin-claude-update-title = Update Notification Plugin for Claude Code
...
cli-agent-plugin-gemini-install-title = Install Notification Plugin for Gemini CLI
cli-agent-plugin-gemini-run-command-restart = Run the following command, then restart Gemini CLI.
cli-agent-plugin-install-warp-extension-step = Install the extension
cli-agent-plugin-gemini-restart-note = Restart Gemini CLI to activate the plugin.
cli-agent-plugin-gemini-update-title = Update Notification Plugin for Gemini CLI
cli-agent-plugin-update-warp-extension-step = Update the extension
cli-agent-plugin-gemini-restart-update-note = Restart Gemini CLI to activate the update.
cli-agent-plugin-opencode-install-title = Install Notification Plugin for OpenCode
cli-agent-plugin-opencode-install-subtitle = Add the plugin to your OpenCode configuration, then restart OpenCode.
...
cli-agent-plugin-opencode-update-title = Update Notification Plugin for OpenCode
...
terminal-bootstrapping-installing-warp-ssh-extension-progress = Installing Phosphor SSH Extension... ({ $p }%)
terminal-bootstrapping-installing-warp-ssh-extension = Installing Phosphor SSH Extension...
terminal-bootstrapping-updating-warp-ssh-extension = Updating Phosphor SSH Extension...
...
agent-tip-oz-headless = Use the `oz` command to run a Phosphor agent in headless mode, useful for remote machines.
...
agent-tip-warpify-ssh = Phosphorize a remote SSH session to enable Phosphor Agent inside that environment.
...
hoa-feature-native-code-review-description = Send inline comments from Phosphor's code review directly to Claude Code, Codex, or OpenCode
resource-center-whats-new-section = What's New?
resource-center-getting-started-section = Getting Started
resource-center-maximize-warp-section = Maximize Phosphor
...
resource-center-command-palette-description = Access all of Phosphor via the keyboard.
resource-center-set-theme-title = Set your theme
resource-center-set-theme-description = Make Phosphor your own by choosing a theme.
resource-center-custom-prompt-title = Use your custom prompt
resource-center-custom-prompt-description = Set up Phosphor to honor your PS1 setting
resource-center-view-documentation = View documentation
resource-center-integrate-ide-title = Integrate Phosphor with your IDE
resource-center-integrate-ide-description = Configure Phosphor to launch from your most used development tools
resource-center-how-warp-uses-warp-title = How Phosphor uses Phosphor
resource-center-how-warp-uses-warp-description = Learn how Phosphor's engineering team uses their favorite features
...
onboarding-intention-title = Welcome to Phosphor
...
onboarding-ai-feature-warp-agents = Phosphor agents
onboarding-ai-feature-oz-cloud-agents-platform = Phosphor local agents platform
...
onboarding-agent-title = Customize your Phosphor Agent
...
onboarding-agent-disable-warp-agent = Disable Phosphor Agent
onboarding-project-title = Open a project
onboarding-project-subtitle = Set up a project to optimize it for coding in Phosphor.
...
onboarding-customize-title = Customize your Phosphor
...
auth-opt-out-line-1 = Phosphor stores onboarding choices locally.
...
auth-local-privacy-note = Phosphor stores onboarding choices locally on this device.
...
auth-login-slide-subtitle-warp-drive = Save and organize notebooks, workflows, and more on this device.
...
get-started-welcome-title = Welcome to Phosphor
...
terminal-banner-p10k-supported = Powerlevel10k now supports Phosphor!{"  "}
...
terminal-banner-pure-unsupported = Pure is not yet supported in Phosphor. You might consider one of the supported prompts as an alternative.{"  "}
...
ai-footer-plugin-installed-restart-session = Notification plugin installed. Please restart the session to activate.
ai-footer-installing-warp-plugin = Installing notification plugin...
ai-footer-failed-install-warp-plugin = Failed to install notification plugin
ai-footer-plugin-updated-restart-session = Notification plugin updated. Please restart the session to activate.
ai-footer-updating-warp-plugin = Updating notification plugin...
ai-footer-failed-update-warp-plugin = Failed to update notification plugin
...
settings-import-reset-to-warp-defaults = Reset to Phosphor defaults
...
workspace-version-deprecation-without-permissions-banner = Some Phosphor features may not work as expected without updating immediately, but Phosphor is unable to perform the update.
workspace-new-version-unable-to-update-banner = A new version is available but Phosphor is unable to perform the update.
workspace-unable-to-launch-new-installed-version = Phosphor was unable to launch the new installed version.
...
terminal-warpify-without-tmux = Phosphorize without TMUX
terminal-continue-without-warpification = Continue without Phosphorization
terminal-always-install = Always install
terminal-never-install = Never install
terminal-ssh-report-issue-prefix = We are actively working on improving the stability of SSH in Phosphor. Please consider{" "}
...
terminal-agent-mode-setup-title = Optimize Phosphor for this codebase?
...
terminal-shell-premature-subtext = Something went wrong while starting { $shell_detail } and Phosphorizing it, causing the process to terminate. Phosphorize script output is displayed here, which may point at a cause.
...
notifications-banner-permissions-denied-title = Phosphor was denied permissions to send you notifications.
...
tooltip-secrets-not-sent-to-warp-server = *Secrets are not sent to Phosphor's server.
