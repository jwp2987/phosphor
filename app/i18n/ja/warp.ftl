...
common-skip = スキップ
common-get-warping = Phosphor を始める
...
agent-management-source-local-agent = Phosphor (ローカルエージェント)
agent-management-source-cloud-agent = Phosphor (ローカルエージェント)
...
workspace-menu-update-warp-manually = Phosphor を手動で更新
...
workspace-menu-view-warp-logs = Phosphor ログを表示
...
workspace-toast-all-conversations-deleted = { $count } 件の会話を削除しました
workspace-toast-no-conversations-to-delete = 削除できる会話がありません
...
workspace-update-and-relaunch-warp = Phosphor を更新して再起動
workspace-updating-to-version = ({ $version }) に更新中
workspace-update-warp-manually = Phosphor を手動で更新
...
terminal-banner-completions-not-working-suffix =  で tmux phosphorization を有効にすると解決する場合があります。
terminal-banner-shell-config-incompatible = シェルの設定が Phosphor と互換性がありません...{"  "}
...
terminal-agent-header-for-parent-named = { $name } 用
ai-document-show-version-history = 履歴を表示
ai-document-update-agent = エージェントを更新
ai-document-save-and-sync-tooltip = この計画をライブラリに保存
ai-document-show-in-warp-drive = ライブラリで表示
ai-document-copy-as-markdown = Markdown としてコピー
ai-document-copied-as-markdown = Markdown としてクリップボードにコピーしました
...
ai-conversation-view-in-oz = Phosphor Agent で表示
ai-conversation-view-in-oz-tooltip = この実行を Web アプリで表示
...
ai-footer-enable-notifications-tooltip = 通知プラグインをインストールして Phosphor 内のリッチなエージェント通知を有効化
ai-footer-notifications-setup-instructions = 通知セットアップ手順
ai-footer-install-plugin-instructions-tooltip = 通知プラグインのインストール手順を表示
ai-footer-update-warp-plugin = 通知プラグインを更新
ai-footer-plugin-update-available-tooltip = 通知プラグインの新バージョンがあります
ai-footer-plugin-update-instructions = プラグイン更新手順
ai-footer-plugin-update-instructions-tooltip = 通知プラグインの更新手順を表示
...
ai-cloud-setup-visit-oz = Phosphor Agent を開く
...
zap-launch-title = Phosphor がオープンソースになりました
zap-launch-description = コミュニティの皆さんがエージェントファーストのワークフローで Phosphor の構築に参加できます。
zap-launch-contribute-title = コントリビュート
zap-launch-contribute-description = Phosphor のクライアントコードがオープンソースになりました。/feedback スキルで Issue を作成し、こちらのコントリビューションガイドラインに従ってください。
zap-launch-contribute-link-text = こちら
zap-launch-oad-title = Open Automated Development
zap-launch-oad-description = Phosphor リポジトリは、Phosphor Agent のローカルエージェント体験を活用したエージェントファーストワークフローで管理されています。
...
hoa-finish = 完了
session-config-get-warping = Phosphor を始める
...
terminal-warpify-subshell = サブシェルを Phosphorize
terminal-warpify-subshell-tooltip = このセッションで Phosphor シェル統合を有効化
terminal-use-agent = エージェントを使う
terminal-use-agent-tooltip = Phosphor エージェントに支援を依頼
terminal-give-control-back-to-agent = 制御をエージェントに戻す
terminal-resume-agent-tooltip = Phosphor エージェントに再開を依頼
...
terminal-open-in-warp = Phosphor で開く
terminal-open-conversation-in-warp-tooltip = この会話を Phosphor デスクトップアプリで開く
...
terminal-input-agent-hint-deploy-react-vercel = 何でも Phosphor で。例: React アプリを Vercel にデプロイし環境変数を設定
terminal-input-agent-hint-debug-python-ci = 何でも Phosphor で。例: CI で Python テストが失敗する原因をデバッグ
terminal-input-agent-hint-setup-microservice = 何でも Phosphor で。例: Docker で新しいマイクロサービスをセットアップしデプロイパイプラインを作成
terminal-input-agent-hint-fix-node-memory-leak = 何でも Phosphor で。例: Node.js アプリのメモリリークを発見して修正
terminal-input-agent-hint-backup-postgres = 何でも Phosphor で。例: PostgreSQL DB のバックアップスクリプトを作成しスケジュール
terminal-input-agent-hint-migrate-mysql-postgres = 何でも Phosphor で。例: MySQL から PostgreSQL へのデータ移行
terminal-input-agent-hint-monitor-aws = 何でも Phosphor で。例: AWS インフラの監視とアラートを設定
terminal-input-agent-hint-build-fastapi = 何でも Phosphor で。例: FastAPI でモバイルアプリ向け REST API を構築
terminal-input-agent-hint-optimize-sql = 何でも Phosphor で。例: 遅い SQL クエリを最適化
terminal-input-agent-hint-github-actions = 何でも Phosphor で。例: マージ時に自動デプロイする GitHub Actions ワークフローを作成
terminal-input-agent-hint-redis-cache = 何でも Phosphor で。例: Web アプリに Redis キャッシュを設定
terminal-input-agent-hint-kubernetes-pods = 何でも Phosphor で。例: Kubernetes Pod がクラッシュし続ける原因をトラブルシュート
terminal-input-agent-hint-bigquery-pipeline = 何でも Phosphor で。例: CSV を処理して BigQuery に投入するデータパイプラインを構築
terminal-input-agent-hint-ssl-https = 何でも Phosphor で。例: SSL 証明書を設定してドメインを HTTPS 化
terminal-input-agent-hint-refactor-legacy-code = 何でも Phosphor で。例: レガシーコードをモダンな設計パターンにリファクタ
terminal-input-agent-hint-unit-tests = 何でも Phosphor で。例: 認証サービスのユニットテストを作成
terminal-input-agent-hint-elk-logs = 何でも Phosphor で。例: 分散システム向けに ELK スタックでログ集約を構築
terminal-input-agent-hint-oauth-express = 何でも Phosphor で。例: Express.js アプリに OAuth2 認証を実装
terminal-input-agent-hint-optimize-docker = 何でも Phosphor で。例: ビルド時間とサイズを削減するため Docker イメージを最適化
terminal-input-agent-hint-ab-testing = 何でも Phosphor で。例: Web アプリ向けに A/B テスト基盤を構築
...
terminal-model-specs-description = Phosphor のハーネスでのモデル性能、クレジット消費レート、タスク速度のベンチマーク。
...
onboarding-callout-meet-input-title = Phosphor 入力欄のご紹介
...
onboarding-callout-nl-support-text-prefix = 自然言語入力は既定で無効です。有効にすると、平易な英語でリクエストを入力でき、Phosphor がエージェント向けクエリを自動判定します。次の方法でいつでも上書きできます:
onboarding-callout-enable-nl-detection = 自然言語判定を有効にする
onboarding-callout-new-agent-title = Phosphor の新しいエージェント体験のご紹介
...
language-widget-secondary = 変更を完全に反映するには Phosphor を再起動してください。
language-restart-required-title = 言語が変更されました
language-restart-required-body = Phosphor の UI 言語が更新されました。一部のテキストは即座に切り替わりますが、すべての箇所に反映するには再起動が必要です。
# =============================================================================
...
settings-section-warp-drive = ライブラリ
settings-section-warpify = Phosphorize
settings-section-network = ネットワーク
settings-section-ai = AI
settings-section-warp-agent = Phosphor エージェント
...
settings-about-automatic-updates-description = 有効にすると、Phosphor はバックグラウンドで新バージョンを確認しダウンロードします。無効でも手動で更新を確認できます。
...
# ---- list_page.rs ----
settings-mcp-list-description = MCP サーバーを追加して Phosphor エージェントの機能を拡張します。MCP サーバーは標準化されたインターフェースを通じてデータソースやツールをエージェントに公開し、プラグインのように動作します。カスタムサーバーを追加するか、プリセットを使用して人気のサーバーから始められます。
...
settings-mcp-list-section-shared-by-warp-and-team = Phosphor と { $name } から利用可能
settings-mcp-list-section-shared-by-warp-and-other-devices = Phosphor および別のデバイスから共有
settings-mcp-list-section-shared-from-warp = Phosphor から共有
...
settings-platform-description = API キーを作成・管理して、ローカルエージェントから Phosphor アカウントへのアクセスを許可します。
    詳細は次を参照
...
settings-platform-zero-state-description = キーを作成して Phosphor への外部アクセスを管理します
settings-platform-create-api-key-description-personal = この API キーはユーザーに紐付き、Phosphor アカウントに対するリクエストを実行できます。
settings-platform-create-api-key-description-team = この API キーはチームに紐付き、チームを代理してリクエストを実行できます。
settings-platform-create-api-key-name-placeholder = Phosphor API キー
...
settings-referrals-page-title = 友人を Phosphor に招待
settings-referrals-anonymous-header = ローカル Phosphor ビルドでは紹介プログラムを利用できません
...
settings-referrals-reward-intro = 紹介で Phosphor 限定グッズを獲得*
...
settings-warpify-description-prefix = Phosphor が特定のシェルを「Phosphorize」(ブロック・入力モード等のサポートを追加) するかを設定します。
...
settings-warpify-section-ssh-subtitle = 対話的な SSH セッションを Phosphorize します。
...
settings-warpify-enable-ssh = SSH セッションを Phosphorize
settings-warpify-install-ssh-extension = SSH 拡張をインストール
settings-warpify-install-ssh-extension-description = リモートホストに Phosphor の SSH 拡張がインストールされていない場合のインストール挙動を制御します。
settings-warpify-use-tmux = Tmux Phosphorization を使用
settings-warpify-tmux-description = tmux ssh ラッパーは既定のラッパーが動作しない多くの状況で機能しますが、phosphorize するためにボタン押下が必要な場合があります。新しいタブから有効になります。
settings-warpify-ssh-tmux-toggle-binding-label = Phosphorization 用 SSH セッション検出
...
settings-ai-command-denylist-description = Phosphor エージェントが実行する前に必ず権限を確認するコマンドにマッチする正規表現。
settings-ai-command-allowlist = コマンド許可リスト
settings-ai-command-allowlist-description = Phosphor エージェントが自動的に実行できるコマンドにマッチする正規表現。
...
settings-ai-mcp-allowlist-description = Phosphor エージェントによるこれらの MCP サーバーの呼び出しを許可します。
settings-ai-mcp-denylist = MCP 拒否リスト
settings-ai-mcp-denylist-description = Phosphor エージェントはこのリストにある MCP サーバーを呼び出す前に必ず権限を確認します。
settings-ai-info-banner-managed-by-workspace = 一部の権限はワークスペースによって管理されています。

# Models / Profiles
settings-ai-base-model = ベースモデル
settings-ai-base-model-description = このモデルは Phosphor エージェントの中心エンジンとして機能します。ほとんどのやり取りを担い、必要に応じて計画やコード生成などのタスクで他のモデルを呼び出します。Phosphor はモデルの可用性や、会話要約などの補助タスクに応じて自動的に別のモデルへ切り替えることがあります。
settings-ai-show-model-picker-in-prompt = プロンプトにモデル選択を表示
settings-ai-codebase-context = コードベースのコンテキスト
settings-ai-codebase-context-description = Phosphor エージェントによるコードベースの概要生成を許可し、コンテキストとして利用します。コードがサーバーに保存されることはありません。
...
settings-ai-auto-approve-bypasses-command-denylist = 自動承認でコマンド拒否リストをバイパスすることを許可
settings-ai-auto-approve-bypasses-command-denylist-description = 有効にすると、早送りと自動承認は拒否リストのコマンドを確認なしで実行します。
...
settings-ai-rules-description = ルールは、Phosphor エージェントがコードベースや特定のワークフローのお作法に沿うよう導きます。
...
settings-ai-prompt-template-dir = システムプロンプトのテンプレートディレクトリ
settings-ai-prompt-template-dir-description = 内蔵版の代わりに、このディレクトリからシステムプロンプトのテンプレートとツールの説明を読み込みます。再ビルドなしで編集が反映されます。空欄の場合は内蔵版を使用します。存在しないファイルや構文エラーのあるファイルは個別にフォールバックします。環境変数 ZAP_PROMPT_DIR が優先されます。
settings-ai-prompt-template-dir-placeholder = /path/to/prompts
settings-ai-prompt-template-seed = 内蔵テンプレートをエクスポート
settings-ai-prompt-template-status-inactive = ○ 無効 — 内蔵テンプレートを使用中。このディレクトリを設定するまで、ディスク上の変更は反映されません。
settings-ai-prompt-template-status-active = ● 有効 — { $dir } から { $on_disk }/{ $total } 個のファイルを読み込み済み。保存した変更は次のリクエストで反映されます。
settings-ai-prompt-template-status-active-env = ● ZAP_PROMPT_DIR により有効 — { $dir } から { $on_disk }/{ $total } 個のファイルを読み込み済み。保存した変更は次のリクエストで反映されます。
...
settings-ai-auto-show-rich-input-tooltip = コーディングエージェント用の通知プラグインが必要です
...
settings-ai-cloud-agent-computer-use-description = Phosphor アプリから開始したエージェント会話でコンピュータ操作を有効化します。

# AWS Bedrock
settings-ai-aws-bedrock-toggle = AWS Bedrock の認証情報を使用
settings-ai-aws-bedrock-description = Phosphor はローカルの AWS CLI 認証情報を読み込み、Bedrock 対応モデルへ送信します。
settings-ai-aws-bedrock-description-managed = Phosphor はローカルの AWS CLI 認証情報を読み込み、Bedrock 対応モデルへ送信します。この設定は組織によって管理されています。
...
settings-features-ssh-wrapper = Phosphor SSH ラッパー
settings-features-receive-desktop-notifications = Phosphor からのデスクトップ通知を受信
...
settings-features-make-default-terminal = Phosphor を既定のターミナルにする
...
settings-features-default-terminal-current = Phosphor が既定のターミナルです
...
settings-features-restart-warp-to-apply = 変更を反映するには Phosphor を再起動してください。
...
settings-footer-alert-fix-with-oz = Phosphor Agent で修正
...
settings-code-codebase-context-desc = 開いているリポジトリの埋め込みインデックスをエージェントが検索できるようにし、grep に頼らず関連コードを見つけられるようにします。コード片は設定した埋め込みプロバイダーにのみ送信されます。リポジトリが SSH ホスト上にある場合、インデックスはそのホスト上で作成され、この項目を有効にすると、ホストが自分でプロバイダーを呼び出せるように、埋め込みプロバイダーのアドレスと API キーがそのホストへ送信されます。
...
settings-code-remote-indexed-folders-desc = リモートエージェントのデーモンにインデックス作成を依頼した SSH ホスト上のリポジトリと、各ホストから返ってきた状態です。埋め込み処理はホスト側で実行され、Phosphor のサーバーを経由することはありません。ただし上の「コードベースのインデックス」が有効な間は、接続するすべてのホストに埋め込みプロバイダーのアドレスと API キーが送信され、ホストが直接プロバイダーを呼び出してコードを埋め込みます。この設定を無効にすると、接続済みのホストからキーを撤回し、新しいホストへの送信も停止します。キーを預けても構わないホストにのみ接続してください。
...
settings-exec-profile-editor-section-prompts = システムプロンプト
settings-exec-profile-editor-prompts-desc = 各モデルスロットのシステムプロンプトを選択します。Auto はモデルファミリーに応じて組み込みテンプレートを自動選択します。組み込みテンプレート、またはプロンプトテンプレートディレクトリの「custom」フォルダ内のカスタムファイルを指定することもできます。
settings-exec-profile-editor-prompt-auto = 自動
settings-exec-profile-editor-prompt-base = ベースプロンプト
settings-exec-profile-editor-prompt-coding = コーディングプロンプト
settings-exec-profile-editor-prompt-cli-agent = フルターミナル利用プロンプト
settings-exec-profile-editor-prompt-computer-use = コンピュータ操作プロンプト
settings-exec-profile-editor-prompt-title = タイトル生成プロンプト
settings-exec-profile-editor-prompt-suggestions = プロンプト候補プロンプト
settings-exec-profile-editor-prompt-nld-predict = 入力補完プロンプト
settings-exec-profile-editor-prompt-relevant-files = 関連ファイルプロンプト
settings-exec-profile-editor-prompt-workflow-metadata = ワークフローメタデータプロンプト
settings-exec-profile-editor-prompt-next-command = 次コマンドプロンプト
settings-exec-profile-editor-prompt-base-desc = メインエージェントのシステムプロンプト。ベースモデルが主な会話で使用します。
settings-exec-profile-editor-prompt-coding-desc = コーディングサブエージェントのシステムプロンプト。
settings-exec-profile-editor-prompt-cli-agent-desc = フルターミナル利用(CLI)エージェントのシステムプロンプト。
settings-exec-profile-editor-prompt-computer-use-desc = コンピュータ操作モデルのシステムプロンプト。
settings-exec-profile-editor-prompt-title-desc = 最初のメッセージから会話/タブのタイトルを生成します。
settings-exec-profile-editor-prompt-suggestions-desc = コマンド実行後、エージェントに尋ねる追加質問の候補を提案します。
settings-exec-profile-editor-prompt-nld-predict-desc = エージェント入力欄で入力中にリアルタイムで補完します。
settings-exec-profile-editor-prompt-relevant-files-desc = 現在のクエリに関連するファイルを選択します。
settings-exec-profile-editor-prompt-workflow-metadata-desc = ワークフローエディタでコマンドからパラメータ化メタデータを自動入力します。
settings-exec-profile-editor-prompt-next-command-desc = 次のコマンドを提案します(グレーのインライン/ゼロステート候補)。
settings-exec-profile-editor-prompts-custom-location = カスタムファイル:
settings-exec-profile-editor-prompts-builtin-location = 組み込み:
settings-exec-profile-editor-prompts-no-dir = プロンプトテンプレートディレクトリが未設定です。設定 → AI で設定するとカスタムプロンプトファイルを追加できます。
settings-exec-profile-editor-section-permissions = 権限
settings-exec-profile-editor-base-model = ベースモデル
settings-exec-profile-editor-base-model-desc = このモデルはエージェントの主要エンジンとして機能します。ほとんどのやり取りを駆動し、必要に応じて計画やコード生成などのために他モデルを呼び出します。Phosphor はモデルの可用性や、会話の要約などの補助タスクのため、自動的に代替モデルに切り替える場合があります。
...
settings-exec-profile-editor-command-allowlist-desc = Phosphor Agent が自動実行できるコマンドにマッチさせる正規表現。
settings-exec-profile-editor-command-denylist = コマンド拒否リスト
settings-exec-profile-editor-command-denylist-desc = Phosphor Agent が常に実行許可を求めるべきコマンドにマッチさせる正規表現。
settings-exec-profile-editor-mcp-allowlist = MCP 許可リスト
settings-exec-profile-editor-mcp-allowlist-desc = Phosphor Agent による呼び出しが許可される MCP サーバー。
settings-exec-profile-editor-mcp-denylist = MCP 拒否リスト
settings-exec-profile-editor-mcp-denylist-desc = Phosphor Agent による呼び出しが許可されない MCP サーバー。
...
settings-show-blocks-confirm-dialog-text = このブロックを共有解除してもよろしいですか?
    ...
    リンク経由でアクセスできなくなり、Phosphor サーバーから完全に削除されます。
...
settings-appearance-custom-icon-restart-warning = MacOS で希望のアイコンスタイルを適用するには Phosphor の再起動が必要な場合があります。
...
settings-appearance-input-mode-pinned-bottom = 下部に固定 (Phosphor モード)
...
settings-environments-page-description = 環境はアンビエントエージェントが実行される場所を定義します。GitHub (推奨)、Phosphor 支援セットアップ、または手動構成で数分で設定できます。
...
settings-environments-section-team-default = Phosphor とこのデバイスから提供
settings-environments-section-team-named = Phosphor と { $team } で共有
...
quit-warning-title-app = Phosphor を終了しますか?
...
keybinding-desc-workspace-toggle-wire-inspector = BYOP ワイヤーインスペクターを切り替え
...
keybinding-desc-workspace-left-panel-warp-drive = 左パネル: ライブラリ
keybinding-desc-workspace-open-global-search = グローバル検索を開く
keybinding-desc-workspace-open-global-search-menu = グローバル検索
keybinding-desc-workspace-toggle-warp-drive = ライブラリを切り替え
keybinding-desc-workspace-toggle-warp-drive-menu = ライブラリ
...
keybinding-desc-workspace-search-drive = ライブラリを検索
...
keybinding-desc-workspace-terminate-app = Phosphor を終了
...
keybinding-desc-workspace-export-all-warp-drive-objects = すべてのライブラリオブジェクトをエクスポート
keybinding-desc-workspace-install-cli = Phosphor Agent CLI コマンドをインストール
keybinding-desc-workspace-uninstall-cli = Phosphor Agent CLI コマンドをアンインストール

# AI assistant / agents
keybinding-desc-workspace-toggle-ai-assistant = Phosphor AI を切り替え
...
keybinding-desc-workspace-import-to-personal-drive = 個人ライブラリにインポート
...
keybinding-desc-workspace-show-settings-about-menu = Phosphor について
keybinding-desc-workspace-show-settings-warpify = 設定を開く: Phosphorize
keybinding-desc-workspace-show-settings-warpify-menu = Phosphorize を設定...
...
keybinding-desc-workspace-send-feedback-oz = Phosphor Agent でフィードバックを送信
keybinding-desc-workspace-view-logs = Phosphor ログを表示
...
keybinding-desc-workspace-open-oz-launch-modal = [Debug] Phosphor Agent Launch モーダルを開く
keybinding-desc-workspace-reset-oz-launch-modal-state = [Debug] Phosphor Agent Launch モーダル状態をリセット
keybinding-desc-workspace-open-zap-launch-modal = [Debug] Phosphor Launch モーダルを開く
keybinding-desc-workspace-reset-zap-launch-modal-state = [Debug] Phosphor Launch モーダル状態をリセット
...
keybinding-desc-input-show-network-log = Phosphor ネットワークログを表示
...
keybinding-desc-terminal-warpify-subshell = サブシェルを Phosphorize
keybinding-desc-terminal-warpify-ssh-session = SSH セッションを Phosphorize
...
keybinding-desc-ai-assistant-close = Phosphor AI を閉じる
keybinding-desc-ai-assistant-focus-terminal-input = Phosphor AI からターミナル入力にフォーカス
keybinding-desc-ai-assistant-restart = Phosphor AI を再起動
...
keybinding-desc-terminal-ask-ai-selection = 選択について Phosphor AI に質問
keybinding-desc-terminal-ask-ai-last-block = 直前のブロックについて Phosphor AI に質問
keybinding-desc-terminal-ask-ai = Phosphor AI に質問
...
toggle-enable-auto-approve-bypasses-command-denylist = 自動承認でコマンド拒否リストをバイパスすることを許可
toggle-disable-auto-approve-bypasses-command-denylist = 自動承認でも拒否リストのコマンドには承認を求める
...
toggle-suffix-ssh-wrapper = Phosphor SSH ラッパー
...
settings-external-editor-choose-layout = Phosphor でファイルを開くレイアウトを選択
...
settings-external-editor-prefer-markdown = Markdown ファイルを既定で Phosphor の Markdown ビューアで開く
...
menu-block-open-in-warp = Phosphor で開く
...
menu-block-ask-warp-ai = Phosphor AI に質問
...
menu-input-ask-warp-ai = Phosphor AI に質問
...
prompt-editor-warp-prompt-section = Phosphor ターミナルプロンプト
...
welcome-tips-command-palette-description = キーボードから手を離さずに Phosphor の機能をすべて簡単に発見できます。
...
welcome-tips-theme-picker-description = 組み込みテーマから選んで Phosphor を自分好みに。または自作も可能。
...
search-filter-placeholder-tabs = タブを検索
...
drive-local-storage-limit-description = ローカルストレージ上限はこのデバイス上で適用されます。不要な項目を削除して、新しいライブラリオブジェクト用の容量を空けてください。
...
workflow-ai-assist-tooltip = Phosphor AI でタイトル・説明・パラメータを生成
...
workspace-left-panel-warp-drive = ライブラリ
workspace-left-panel-agent-conversations = エージェント会話
workspace-left-panel-wire-inspector = BYOP ワイヤーインスペクター
# TODO: review
# TODO: review
# TODO: review
# TODO: review
...
wasm-nux-open-desktop-title = Phosphor デスクトップで開きますか?
wasm-nux-open-desktop-detail = 今後のリンクは自動的にデスクトップで開きます。
wasm-nux-open-desktop-confirm = Phosphor で開く
wasm-nux-download-title = Phosphor デスクトップをダウンロードしますか?
wasm-nux-download-description = Phosphor は AI と開発チームのナレッジを内蔵したインテリジェントなターミナルです。
wasm-nux-learn-more = 詳しく見る
wasm-nux-download-confirm = ダウンロード
wasm-nux-object-kind-drive-objects = ライブラリオブジェクト
wasm-nux-object-kind-warp-links = Phosphor リンク
...
auth-override-warning-confirm-title = 個人のライブラリオブジェクトと環境設定を削除しますか?
auth-override-warning-description = Web ブラウザから Phosphor アカウントにログインしたようです。続行すると、この匿名セッションの個人ライブラリオブジェクトおよび環境設定はすべて完全に削除されます。
...
auth-override-warning-accessibility-help = Phosphor が Web ブラウザからの新しいログインを検出しました。ログインせずに Phosphor の使用を続けるには Esc を押してキャンセルしてください。
...
auth-needs-sso-link-detail = 下のボタンをクリックして Phosphor アカウントを SSO プロバイダにリンクしてください。
...
auth-offline-first-use-description = 現在オフラインです。Phosphor を初めて使用するにはインターネット接続が必要です。
auth-offline-first-use-learn-more = 詳しく見る
auth-offline-overlay-title = Phosphor をオフラインで使用
auth-offline-overlay-paragraph-1 = Phosphor のローカル機能はオフラインで動作します。
auth-offline-overlay-paragraph-2 = BYOP AI 機能を使う場合のみ、選択したプロバイダーへの接続が必要です。
auth-offline-overlay-paragraph-3 = Phosphor は匿名のローカルユーザー ID で動作し、外部オブジェクトや利用量計測のためにインターネット接続を要求しません。
auth-offline-overlay-dismiss = 閉じる
auth-privacy-settings-title = プライバシー設定
auth-privacy-settings-done = 完了
auth-privacy-settings-help-improve = Phosphor の改善に協力する
auth-privacy-settings-help-improve-description = ハイレベルな機能利用データは Phosphor プロダクトチームのロードマップ優先順位付けに役立ちます。
auth-privacy-settings-learn-more = 詳しく見る
auth-privacy-settings-send-crash-reports = クラッシュレポートを送信
auth-privacy-settings-crash-reports-description = クラッシュレポートは Phosphor エンジニアリングチームが安定性を理解しパフォーマンスを改善するのに役立ちます。
...
auth-logout-unsynced-drive-objects-warning = 未同期のライブラリオブジェクトが { $count } { $count ->
        [one] 件
       *[other] 件
    }あります。ログアウトすると、{ $count ->
        [one] そのオブジェクト
...
cli-agent-plugin-codex-install-title = Codex の Phosphor 通知を有効化
cli-agent-plugin-codex-install-subtitle = Codex を最新版に更新し、フォーカス時通知を有効にすることで Phosphor が作業中に通知を表示できるようにします。
...
cli-agent-plugin-deepseek-install-title = DeepSeek の Phosphor 通知を有効化
...
cli-agent-plugin-claude-install-title = Claude Code 用通知プラグインをインストール
cli-agent-plugin-claude-install-subtitle = まずマシンに jq がインストールされていることを確認し、続けて以下のコマンドを実行してください。
cli-agent-plugin-claude-add-marketplace-step = プラグインのマーケットプレイスリポジトリを追加
cli-agent-plugin-install-warp-plugin-step = プラグインをインストール
...
cli-agent-plugin-claude-update-title = Claude Code 用通知プラグインを更新
...
cli-agent-plugin-gemini-install-title = Gemini CLI 用通知プラグインをインストール
cli-agent-plugin-gemini-run-command-restart = 以下のコマンドを実行し、Gemini CLI を再起動してください。
cli-agent-plugin-install-warp-extension-step = 拡張機能をインストール
cli-agent-plugin-gemini-restart-note = プラグインを有効化するため Gemini CLI を再起動します。
cli-agent-plugin-gemini-update-title = Gemini CLI 用通知プラグインを更新
cli-agent-plugin-update-warp-extension-step = 拡張機能を更新
cli-agent-plugin-gemini-restart-update-note = 更新を有効化するため Gemini CLI を再起動します。
cli-agent-plugin-opencode-install-title = OpenCode 用通知プラグインをインストール
cli-agent-plugin-opencode-install-subtitle = OpenCode の設定にプラグインを追加し、OpenCode を再起動してください。
...
cli-agent-plugin-opencode-update-title = OpenCode 用通知プラグインを更新
...
terminal-bootstrapping-installing-warp-ssh-extension-progress = Phosphor SSH 拡張をインストール中... ({ $p }%)
terminal-bootstrapping-installing-warp-ssh-extension = Phosphor SSH 拡張をインストール中...
terminal-bootstrapping-updating-warp-ssh-extension = Phosphor SSH 拡張を更新中...
...
agent-tip-command-palette = <keybinding> でコマンドパレットを開き、Phosphor のアクションやショートカットにアクセスできます。
...
agent-tip-add-context = `@` でファイル、ブロック、ライブラリオブジェクトからコンテキストをプロンプトに追加できます。
...
agent-tip-warpify-ssh = リモート SSH セッションを Phosphorize するとその環境内で Phosphor Agent を有効化できます。
...
agent-tip-action-warp-drive = ライブラリ。
agent-tip-action-show-diff-view = 差分ビューを表示
agent-tip-voice-input = <keybinding> を押し続けるとプロンプトをエージェントに直接話しかけられます。
hoa-welcome-banner-title = ユニバーサルエージェントサポートのご紹介: Phosphor であらゆるコーディングエージェントをレベルアップ
...
hoa-feature-native-code-review-description = Phosphor のコードレビューからインラインコメントを Claude Code、Codex、OpenCode へ直接送信
resource-center-whats-new-section = 新着情報
resource-center-getting-started-section = はじめに
resource-center-maximize-warp-section = Phosphor を最大限に活用
...
resource-center-command-palette-description = キーボードから Phosphor のすべての機能にアクセス。
resource-center-set-theme-title = テーマを設定
resource-center-set-theme-description = テーマを選んで Phosphor を自分好みに。
resource-center-custom-prompt-title = カスタムプロンプトを使用
resource-center-custom-prompt-description = PS1 設定を尊重するよう Phosphor をセットアップ
resource-center-view-documentation = ドキュメントを表示
resource-center-integrate-ide-title = Phosphor を IDE と連携
resource-center-integrate-ide-description = よく使う開発ツールから Phosphor を起動できるよう設定
resource-center-how-warp-uses-warp-title = Phosphor チームの Phosphor 活用法
resource-center-how-warp-uses-warp-description = Phosphor のエンジニアリングチームがお気に入り機能をどう使っているか学びます
...
onboarding-intention-title = Phosphor へようこそ
...
onboarding-ai-feature-warp-agents = Phosphor エージェント
onboarding-ai-feature-oz-cloud-agents-platform = Phosphor Agent ローカルエージェントプラットフォーム
...
onboarding-agent-title = Phosphor エージェントをカスタマイズ
...
onboarding-agent-disable-warp-agent = Phosphor エージェントを無効化
...
onboarding-project-subtitle = Phosphor でのコーディング向けにプロジェクトを設定します。
...
onboarding-customize-title = Phosphor をカスタマイズ
...
onboarding-customize-warp-drive = ライブラリ
onboarding-customize-tools-panel = ツールパネル
onboarding-customize-code-review = コードレビュー
onboarding-free-user-title = はじめましょう。
onboarding-free-user-agent-title = Phosphor 内蔵エージェントによるエージェント駆動開発
onboarding-free-user-agent-description = Phosphor 内蔵エージェント Phosphor Agent でローカルに反復・計画・構築。
...
onboarding-free-user-subscribe-title = サブスクライブして Phosphor のエージェント駆動開発を利用しましょう。
...
onboarding-free-user-subscribe-item-drive = 無制限の Phosphor Drive オブジェクトとコラボレーション
onboarding-free-user-subscribe-item-support = プライベートメールサポート
onboarding-free-user-subscribe-item-cloud-storage = ローカル会話保存

auth-opt-out-line-1 = Phosphor はオンボーディングの選択をこのデバイスに保存します。
...
auth-local-privacy-note = Phosphor はオンボーディングの選択をこのデバイスに保存します。
...
auth-login-slide-title-warp-drive = ライブラリをはじめる
auth-login-slide-title-ai = AI をはじめる
auth-login-slide-subtitle-warp-drive = ノートブックやワークフローなどをこのデバイスに保存・整理します。
auth-login-slide-subtitle-ai = アカウントを接続して、AI による計画・コーディング・自動化を有効にします。
auth-disable-warp-drive = ライブラリを無効化
auth-disable-ai-features = AI 機能を無効化
auth-enable-warp-drive = ライブラリを有効化
...
auth-disable-warp-drive-confirm-title = 本当にライブラリを無効化しますか？
auth-disable-ai-features-confirm-title = 本当に AI 機能を無効化しますか？
auth-disable-warp-drive-confirm-body = ライブラリはワークフローやナレッジをこのデバイスに保存できます。続行すると、以下の機能が利用できなくなります:
auth-disable-ai-features-confirm-body = Phosphor は AI でより便利になります。続行すると、以下の機能はいずれも利用できなくなります:
...
auth-require-login-ai-collaboration = ローカル AI 機能に Phosphor アカウントは必要ありません。
auth-require-login-drive-limit = Phosphor ではライブラリオブジェクトをローカルに保存します。
auth-require-login-share = ローカル Phosphor ビルドでは共有機能を利用できません。
auth-welcome-title = Phosphor へようこそ！
auth-sign-up-for-warp = Phosphor を続行
...
get-started-welcome-title = Phosphor へようこそ
...
terminal-banner-p10k-supported = Powerlevel10k が Phosphor に対応しました！{"  "}
...
terminal-banner-pure-unsupported = Pure はまだ Phosphor で対応していません。代替として対応プロンプトの利用をご検討ください。{"  "}
...
ai-footer-plugin-installed-restart-session = 通知プラグインをインストールしました。アクティブ化するにはセッションを再起動してください。
ai-footer-installing-warp-plugin = 通知プラグインをインストール中…
ai-footer-failed-install-warp-plugin = 通知プラグインのインストールに失敗しました
ai-footer-plugin-updated-restart-session = 通知プラグインを更新しました。アクティブ化するにはセッションを再起動してください。
ai-footer-updating-warp-plugin = 通知プラグインを更新中…
ai-footer-failed-update-warp-plugin = 通知プラグインの更新に失敗しました
...
settings-import-reset-to-warp-defaults = Phosphor の既定にリセット
...
workspace-notification-permission-denied-toast = Phosphor にはデスクトップ通知を送る権限がありません。
workspace-troubleshoot-notifications-link = 通知のトラブルシューティング
workspace-plan-synced-to-warp-drive-toast = プランをライブラリに保存しました
workspace-remote-control-link-copied-toast = リモートコントロールリンクをコピーしました。
workspace-update-now = 今すぐ更新
workspace-update-warp = Phosphor を更新
...
workspace-version-deprecation-without-permissions-banner = 直ちに更新しないと一部の Phosphor 機能が想定どおり動作しない可能性がありますが、Phosphor は更新を実行できません。
workspace-new-version-unable-to-update-banner = 新しいバージョンが利用可能ですが、Phosphor は更新を実行できません。
workspace-unable-to-launch-new-installed-version = Phosphor はインストール済みの新バージョンを起動できませんでした。
...
terminal-warpify-without-tmux = TMUX なしで Phosphorize
terminal-continue-without-warpification = Phosphorization なしで続行
terminal-always-install = 常にインストール
terminal-never-install = インストールしない
terminal-ssh-report-issue-prefix = Phosphor の SSH 安定性向上に取り組んでいます。次の対応をご検討ください:{" "}
...
terminal-agent-mode-setup-title = このコードベース向けに Phosphor を最適化しますか？
...
terminal-shell-premature-subtext = { $shell_detail } の起動と Phosphorize 中に何らかの問題が発生し、プロセスが終了しました。Phosphorize スクリプトの出力をここに表示します。原因の特定に役立つ可能性があります。
...
notifications-banner-permissions-denied-title = Phosphor は通知送信の権限を拒否されました。
...
tooltip-secrets-not-sent-to-warp-server = *シークレットは Phosphor のサーバーに送信されません。
