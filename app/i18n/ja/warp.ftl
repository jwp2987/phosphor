...
settings-warpify-description-prefix = Zap が特定のシェルを「Phosphorize」(ブロック・入力モード等のサポートを追加) するかを設定します。
...
auth-override-warning-description = Web ブラウザから Zap アカウントにログインしたようです。続行すると、この匿名セッションの個人ライブラリオブジェクトおよび環境設定はすべて完全に削除されます。
...
agent-tip-warpify-ssh = リモート SSH セッションを Phosphorize するとその環境内で Oz を有効化できます。
...
auth-require-login-drive-limit = Zap ではライブラリオブジェクトをローカルに保存します。
