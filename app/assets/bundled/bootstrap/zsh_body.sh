...
if [[ -z $WARP_BOOTSTRAPPED ]]; then
  ...
  warp_precmd () {
      ...
      # Reset the bracketed-paste ZLE widget to its built-in default
      # (.bracketed-paste). Plugins like bracketed-paste-magic (e.g. the
      # plugin of the same name in Oh My Zsh) override this widget, escaping
      # characters like ')' right after a URL. Because Warp uses a bracketed
      # paste sequence when submitting commands, plugins like that end up
      # incorrectly mangling the command text on submit — e.g. turning
      # `(echo "https://example.com")` into `(echo "https://example.com"\)`,
      # causing ZSH to enter a continuation prompt instead of executing the
      # command directly.
      # Since Warp manages all editing operations through its own input layer
      # (rather than ZSH's ZLE), resetting this widget has no visible impact on the user.
  ...
  if [[ $WARP_IS_LOCAL_SHELL_SESSION == "1" ]]; then
      ...
      function warp_ssh_helper() {
          ...
          # registered) side, from /dev/urandom -- 64 bits, unguessable. It
          # is handed to the remote shell as $WARP_SESSION_ID and reported to
          # the client in the SSH hook below, so the client can register it
          # before the remote shell can emit anything. Do NOT derive it from
          # the clock or $RANDOM: it is the credential every subsequent DCS
          # hook from that session is validated against.
