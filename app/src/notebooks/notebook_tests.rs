...
fn test_close_unmodified() {
    ...
    App::test((), |mut app| async move {
        ...
        // Zap (Wave 4): SyncQueue was removed entirely, so the original
        // stop_dequeueing + assert-on-queue-length no longer applies.
        ...
        app.read(|ctx| {
            ...
            // Zap (Wave 4): SyncQueue was removed entirely, so the original
            // `sync_queue.is_empty()` assertion no longer applies.
        })
    });
}
