...
// Upstream Warp's docs site/Slack/privacy policy no longer apply to the Zap
// fork; these constants are kept as placeholder empty strings, to be filled
// in once Zap has its own channels in place.
// `ctx.open_url("")` is a harmless no-op at UI call sites.
