//! Ported from the pin (`02b53fcd8:app/src/remote_server/codebase_index_model_tests.rs`).
//!
//! All 39 tests exercise `RemoteCodebaseIndexModel` as a plain value
//! (`::default()`), never through `App::test`, so no singleton registration is
//! needed. The only fork difference is where `RemoteCodebaseIndexStatusWithPath`
//! comes from: this fork defines it in `codebase_index_model.rs` rather than in
//! `remote_server::manager` (see the type's doc comment). `use super::*` picks
//! it up either way, so the bodies below are the pin's, unchanged.
