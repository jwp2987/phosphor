//! Client-side model of the remote codebase index (Delta D2, client leg).
//!
//! Ported from the pin (`02b53fcd8:app/src/remote_server/codebase_index_model.rs`).
//! It owns the client's view of every daemon-reported index status, resolves
//! which remote repo a session's search should target, and drives auto-indexing.
//!
//! Two deliberate deviations from the pin, both forced by fork-side type
//! differences rather than by any behavioral choice; each is explained at the
//! point it appears:
//!
//! 1. `RemoteCodebaseIndexStatusWithPath` is defined here instead of in
//!    `remote_server::manager`. The pin's manager built a
//!    `warp_util::remote_path::RemotePath` inside the events it emitted; this
//!    fork's manager emits `host_id` plus the raw `repo_path` string, because
//!    the two `HostId` newtype families only meet in `app` (see
//!    `crate::code::buffer_location`). The conversion — and the "drop statuses
//!    whose path will not parse" rule the pin's manager applied — therefore
//!    lives here.
//! 2. The codebase-context enablement trigger is `CodeSettings`, not
//!    `UserWorkspacesEvent::CodebaseContextEnablementChanged`. See
//!    `handle_codebase_context_enablement_changed`.
...
use super::manager::{
    RemoteCodebaseIndexUpdateOperation, RemoteServerManager, RemoteServerManagerEvent,
};
...
///
/// The pin declared this in `remote_server::manager` and shipped it inside the
/// manager's events. This fork's manager cannot build a [`RemotePath`]: its
/// `HostId` is `warp_core::HostId` while [`RemotePath`] carries
/// `warp_util::host_id::HostId`, and the bridge between the two families lives
/// in `app` (`crate::code::buffer_location`). The manager therefore emits
/// `host_id` plus the status (whose `repo_path` is a raw string) and this model
/// reassembles the pair, which is also where the pin's manager dropped statuses
/// whose `repo_path` was not a valid absolute path.
...
fn remote_path_from_repo_path(host_id: &HostId, repo_path: &str) -> Option<RemotePath> {
    StandardizedPath::try_new(repo_path)
        .ok()
        // Fork: `RemotePath` speaks `warp_util::host_id::HostId`, the manager
        // speaks `warp_core::HostId`. Bridged, never re-derived.
        .map(|path| RemotePath::new(core_host_id_to_util(host_id), path))
}

/// The `(host_id, repo_path)` pair every mutating `RemoteServerManager` method
/// takes in this fork, where the pin passed a single `RemotePath`.
fn manager_target(remote_path: &RemotePath) -> (HostId, String) {
    (
        util_host_id_to_core(&remote_path.host_id),
        remote_path.path.as_str().to_string(),
    )
}

/// Reassembles one manager-reported status into the pin's status-plus-path
/// pair, dropping (with a warning) any status whose `repo_path` will not parse
/// — exactly what the pin's manager did before emitting the event.
fn status_with_remote_path(
    host_id: &HostId,
    status: &RemoteCodebaseIndexStatus,
) -> Option<RemoteCodebaseIndexStatusWithPath> {
    let Some(remote_path) = remote_path_from_repo_path(host_id, &status.repo_path) else {
        log::warn!(
            "[Remote codebase indexing] Client dropping codebase index status with unusable repo path: host_id={host_id} repo_path={}",
            status.repo_path
        );
        return None;
    };
...
fn statuses_with_remote_paths(
    host_id: &HostId,
    statuses: &[RemoteCodebaseIndexStatus],
) -> Vec<RemoteCodebaseIndexStatusWithPath> {
    statuses
        .iter()
        .filter_map(|status| status_with_remote_path(host_id, status))
        .collect()
}
...
impl RemoteCodebaseIndexModel {
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
        ...
        // Fork: the pin subscribed to
        // `UserWorkspacesEvent::CodebaseContextEnablementChanged`, which
        // announced an organization-level policy pushed from Warp's server.
        // With no server there is no such policy and no such event, so the
        // local trigger is the settings group that now owns the flag —
        // `UserWorkspaces::is_codebase_context_enabled` reads
        // `CodeSettings::codebase_context_enabled`. This mirrors the precedent
        // set by `crate::ai::persisted_workspace` for the local index.
        // `handle_codebase_context_enablement_changed` re-reads the setting
        // rather than trusting an event payload, so a change from any source is
        // handled identically.
        let code_settings = CodeSettings::handle(ctx);
        ctx.subscribe_to_model(&code_settings, |me, _event, ctx| {
    ...
    fn status_key_for_remote_path(&self, remote_path: &RemotePath) -> PathAtHost {
        PathAtHost {
            host: self.host_label_for_host(&util_host_id_to_core(&remote_path.host_id)),
    ...
    fn remote_path_for_status_key(&self, key: &PathAtHost) -> RemotePath {
        RemotePath::new(
            core_host_id_to_util(&self.host_id_for_label(&key.host)),
            key.path.clone(),
        )
    }
    ...
    pub fn request_active_repo_index(
        ...
        let (host_id, repo_path) = manager_target(&remote_path);
        RemoteServerManager::handle(ctx).update(ctx, |manager, ctx| {
            let _ = manager.ensure_codebase_indexed(
                host_id,
                repo_path,
    ...
    pub fn codebases_for_agent_context(&self, host_id: &HostId) -> Vec<RemoteCodebaseContextEntry> {
        ...
        let util_host_id = core_host_id_to_util(host_id);
            ...
            .filter(|&(key, status)| {
                key.host == host_label
                    && search_availability_for_status(
                        status,
                        RemotePath::new(util_host_id.clone(), key.path.clone()),
                    )
                    .is_ready()
            })
            .map(|(key, _)| {
    ...
    pub fn resync_index(&self, remote_path: RemotePath, ctx: &mut ModelContext<Self>) {
        ...
        let (host_id, repo_path) = manager_target(&remote_path);
        RemoteServerManager::handle(ctx).update(ctx, |manager, ctx| {
            let _ = manager.resync_codebase(host_id, repo_path, ctx);
        });
    }
    ...
    pub fn drop_index(&self, remote_path: RemotePath, ctx: &mut ModelContext<Self>) {
        let (host_id, repo_path) = manager_target(&remote_path);
        RemoteServerManager::handle(ctx).update(ctx, |manager, ctx| {
            let _ = manager.drop_codebase_index(host_id, repo_path, ctx);
        });
    }
    ...
    fn handle_remote_server_manager_event(
        ...
        match event {
            RemoteServerManagerEvent::CodebaseIndexStatusesSnapshot {
                ...
                // Fork: the manager ships raw `repo_path` strings, so the
                // status/path pairing (and the drop of unparseable paths the
                // pin's manager did) happens here.
                let statuses = statuses_with_remote_paths(host_id, statuses);
            ...
            RemoteServerManagerEvent::CodebaseIndexStatusUpdated {
                ...
                let Some(status_with_path) = status_with_remote_path(host_id, status) else {
                    return;
                };
                if let Some(update) = self.apply_status_update_with_telemetry(
                    status_with_path.remote_path,
                    status_with_path.status,
                ) {
            ...
            RemoteServerManagerEvent::NavigatedToDirectory {
                ...
                // Fork: the pin's event already carried a `RemotePath`; here the
                // navigated path arrives as a string and is built into one.
                let Some(remote_path) = remote_path_from_repo_path(host_id, indexed_path) else {
                    log::warn!(
                        "[Remote codebase indexing] Client ignoring navigation to unusable remote path: host_id={host_id} indexed_path={indexed_path}"
                    );
                    return;
                };
    ...
    /// Reacts to the codebase-context setting being turned on or off.
    ///
    /// Fork: the pin ran this from
    /// `UserWorkspacesEvent::CodebaseContextEnablementChanged`, an
    /// organization-level policy push from Warp's server. That event does not
    /// exist here; the subscription in `new` uses `CodeSettings` instead, and
    /// this method re-reads the effective value rather than trusting any event
    /// payload, so the behavior is identical for every source of the change.
    ...
    fn apply_statuses_snapshot_with_telemetry(
        ...
        for (key, status) in incoming_statuses {
            ...
            let remote_path = RemotePath::new(util_host_id.clone(), key.path.clone());
    ...
    fn record_navigated_directory(
        ...
        let host_id = util_host_id_to_core(&remote_path.host_id);
        self.active_repos_by_host
            .insert(host_id.clone(), remote_path.clone());
        if is_git {
            self.active_git_repos_by_session
                .insert(session_id, remote_path.clone());
            self.last_git_repos_by_host
                .insert(host_id, remote_path.clone());
        } else {
    ...
    fn mark_host_unavailable(&mut self, host_id: &HostId) -> bool {
        ...
        self.active_git_repos_by_session
            .retain(|_, remote_path| remote_path.host_id != util_host_id);
    ...
    fn best_status_for_path(
        ...
        self.statuses
            ...
            .map(|(key, status)| (RemotePath::new(util_host_id, key.path.clone()), status))
    }
    ...
    fn single_descendant_status_for_path(
        ...
        descendants
            .next()
            .is_none()
            .then(|| (RemotePath::new(util_host_id, key.path.clone()), status))
    }
