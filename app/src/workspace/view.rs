...
// Zap Wave 3-1: the `AuthClient` trait was physically removed together with server_api/auth.rs.
use crate::settings::{
...
// Zap Wave 7-3: the `environments_page::EnvironmentsPage` import was
...
// Zap: removed SharedObjectsCreationDeniedModal (cloud Drive quota-denial modal)
...
/// Shared position ID for the "Move to group" sidecar overlay. Used for both
/// the `SavePosition` wrapper and the parent menu's safe-zone rect lookup.
...
pub struct Workspace {
    ...
    /// Tab groups defined in this window, keyed by id. Member tabs reference
    /// their group via `TabData::group_id`. Gated by `FeatureFlag::GroupedTabs`.
    ...
    /// Set while the multi-tab selection right-click menu is open. Reuses the
    /// `tab_right_click_menu` view, so only one of the two flags is ever set.
    ...
    /// The notification center mailbox (the dropdown overlay from the Inbox
    /// button in the top-right of the title bar).
    ...
    /// The notification toast stack (floating in the bottom-right). Gated by
    /// the same `HOANotifications` flag as above.
    ...
    /// Sidecar menu listing the destination groups for the "Move to group"
...
impl Workspace {
    ...
    fn build_menus(ctx: &mut ViewContext<Self>) -> WorkspaceMenuHandles {
        ...
        // The move-to-group sidecar dispatches `MoveTabToGroup` /
        // `MoveSelectedTabsToGroup` directly when a group item is clicked.
    ...
    pub fn new(
        ...
        // Refresh the UI (new-tab menu, title bar buttons, etc.) once the CLI agent install scan completes
        ctx.subscribe_to_model(
        ...
        // Subscribe to notification model events so the unread dot on the title bar's Inbox button refreshes promptly.
        ...
        // Zap: removed the SharedObjectsCreationDeniedModal registration (cloud Drive quota-denial modal)
    ...
    /// Compatibility shim for the old server-token entry point: only opens a conversation that has already been restored locally.
    ...
    pub fn open_file_with_target(
        ...
        match target {
            ...
            FileTarget::ImageViewer(_layout) => {
                // Images always open in a new tab, see `insert_image_pane`.
                // The layout carried by `target` is intentionally ignored here.
    ...
    /// `open_file_layout` (new tab / split) and the multi-file grouping
    /// toggle (multiple remote files merged into internal tabs of the same
    /// code editor pane) just like local files.
    ///
    /// The buffer content is still opened via `GlobalBufferModel`'s
    /// `BufferLocation::Remote` path (sending `OpenBuffer` to the daemon,
    /// then syncing via `BufferEdit` / `BufferUpdatedPush`); that part is
    /// handled by `LocalCodeEditorView::new_with_remote_buffer` inside `CodeView`.
    #[cfg(feature = "local_tty")]
    ...
    fn view_logs(&mut self, ctx: &mut ViewContext<Self>) {
        // Collect diagnostic info synchronously on the calling thread
        // (version, platform, channel, execution mode, MCP/update log paths,
        // etc.); the actual zip bundling happens on a blocking thread, to
        // avoid crossing threads while reading `AppContext` global state.
    ...
    /// Unlike `view_logs`: lets the user pick a save location via the
    /// system's native save-file dialog, then writes the log bundle directly
    /// to that location. The bundle contents are identical to `view_logs`.
    ///
    /// Both failure and success are reported via `toast_stack`, so views like
    /// the settings page, which have no toast area of their own, can still
    /// see the result.
    #[cfg(not(target_family = "wasm"))]
    fn export_logs_to_path(&mut self, ctx: &mut ViewContext<Self>) {
        use warpui::platform::SaveFilePickerConfiguration;

        // Collect extras synchronously on the calling thread (reading
        // AppContext global state); the save dialog and the actual disk write
        // both happen in the subsequent async flow.
        let extras = Self::collect_log_bundle_extras(ctx);
        ...
        ctx.open_save_file_picker(
            move |path_opt, _me, ctx| {
                let Some(path_string) = path_opt else {
                    // User canceled; don't bother them further.
                    return;
                };
                ...
                ctx.spawn(
                    ...
                    |me, result, ctx| match result {
                        Ok(Ok(path)) => {
                            // i18n_embed_fl::fl! requires positional arguments
                            // to live until the macro expansion finishes, so
                            // bind to a String in this scope first, then borrow it as &str.
    ...
    /// Collects the diagnostic material to attach to the zip for this "export logs" run:
    ///
    /// - `manifest.txt`: version / channel / platform / arch / execution mode / generation timestamp;
    /// - Other subsystem logs (MCP server stderr, the Windows auto-updater,
    ///   the minidump service process), only included in the zip when the file actually exists.
    ///
    /// Content deliberately **not** bundled (tradeoffs):
    /// - `.dmp` minidump binaries (can be huge, need to be uploaded separately on demand);
    /// - `zap.prompt_chips.log` (contains command stdout/stderr, only generated on the debug channel, a privacy risk by default);
    /// - profiling artifacts (`dhat-heap.json` / `profile.pb`, only enabled by special cargo features).
    #[cfg(not(target_family = "wasm"))]
    fn collect_log_bundle_extras(ctx: &AppContext) -> warp_logging::LogBundleExtras {
        ...
        // 1) manifest.txt: a human-readable diagnostic summary, the first thing seen when investigating an issue.
        // The log directory is sanitized with `home_relative_path` (replacing
        // `$HOME` with `~` on Unix), to avoid leaking the username / real
        // home directory path when sharing the zip with someone investigating the issue.
        let manifest = {
        ...
        // 2) Other artifacts in the same directory: the minidump service
        // process log for the same channel, the Windows updater log.
        ...
        // 3) The MCP server's current-session stderr (`purge_on_startup: true`, only exists while running).
        // The path is obtained indirectly via
        // `simple_logger::manager::resolve_log_path`: take a placeholder file
        // name and then call `parent()` to get the namespace directory,
        // avoiding exposing the private `log_directory_path` API.
    ...
    fn unified_new_session_menu_items(
        ...
        // 9. New tab group (gated by the Grouped Tabs feature).
        if FeatureFlag::GroupedTabs.is_enabled() {
            menu_items.push(MenuItem::Separator);
            menu_items.push(
    ...
    /// True when `tab_index` belongs to a group and is that group's only member.
    ...
    fn check_and_trigger_onboarding(&mut self, ctx: &mut ViewContext<Self>) -> bool {
        // Zap: removed the first-open agentic-suggestions welcome-block
        // tutorial. Still mark the user as onboarded, so downstream consumers
        // (e.g. the telemetry banner) don't treat an existing user as new.
        if !self.auth_state.is_onboarded().unwrap_or_default() {
            AuthManager::handle(ctx).update(ctx, |auth_manager, ctx| {
                auth_manager.set_user_onboarded(ctx);
            });
    ...
    pub fn open_notebook(
        ...
        if let Some((window_id, locator)) = notebook_manager.as_ref(ctx).find_pane(source) {
            ...
            // notebook invitation path has no UI entry point anymore, but
            // `ZapDriveObjectSettings.invitee_email` is still passed in via
            // the URL handler / drag-drop path. When the invitee concept is
            // retired in Phase 5, remove the field from the settings struct too.
            let _ = settings;
            let _ = source;
    ...
    fn open_remote_image(
        ...
        // First create an empty pane with a loading spinner, then fetch the bytes asynchronously and fill it in (spinner-first UX).
        ...
        ctx.spawn(
            ...
            move |_me, result, ctx| match result {
                ...
                Err(error) => {
                    // Don't open a blank pane — the pane already shows
                    // loading, so just log the failure here, avoiding
                    // flattening every error into DoesNotExist the way `open_remote_buffer` does.
                    log::warn!(
    ...
    // Zap Wave 7-3: `open_environment_management_pane` was physically removed
    ...
    fn user_menu_items(&self, app: &AppContext) -> Vec<MenuItem<WorkspaceAction>> {
        // Decentralized branch: the user menu no longer shows the account
        // name / account CTA / Upgrade / Billing / Invite / Log out and other
        // cloud-account-related items; only local entry points remain
        // (Update, Settings, Docs, Feedback, Logs).
    ...
    ///
    /// TODO(#108 follow-up): in `warp/master` the sole caller is the
    /// group-aware branch of `on_tab_drag`, which reassigns membership when a
    /// dragged tab crosses into another group. That branch resolves the hovered
    /// group from the group container's rendered rect
    /// (`vtab_group_position_id` / `htab_group_position_id`), so it cannot be
    /// ported before the group rendering itself; until then this has no caller.
                    ...
                    .update(terminal_view_ctx, |controller, ctx| {
                        controller.send_slash_command_request(
                            SlashCommandRequest::Summarize {
                                ...
                                overflow: false, // ForkAndCompact is user-triggered, not an automatic overflow
                            },
                            ctx,
                        );
                    });
    ...
    fn summarize_active_ai_conversation(
        ...
        terminal_view.update(ctx, |terminal, ctx| {
            ...
            if let Some(prompt) = initial_prompt {
                ...
                // conversation, so `selected_conversation_id` is set by the time we get here.
                if let Some(conversation_id) =
                    terminal.ai_context_model().as_ref(ctx).selected_conversation_id(ctx)
                {
    ...
    fn handle_changelog_event(&mut self, _event: &ChangelogEvent, _ctx: &mut ViewContext<Self>) {
        // Zap is a localized fork that doesn't depend on a private changelog
        // service, so it doesn't pop up a toast/resource-center after updates.
    }
    ...
    fn handle_settings_pane_event(
        ...
        match event {
            // Zap decentralized branch: the `CheckForUpdate` / `ZapDrive`
            // event arms were physically removed together with the
            // same-named variants in `SettingsViewEvent`. Manually checking
            // for updates can still be triggered via
            // `WorkspaceAction::CheckForUpdate` (the
            // `workspace:check_for_updates` binding); Zap Drive can still be
            // triggered via `WorkspaceAction::ZapDrive`.
    ...
    pub(crate) fn handle_file_tree_event(
        ...
        match event {
                ...
                if let Ok(notification_data_str) = serde_json::to_string(&notification_data) {
                    ...
                    ctx.send_desktop_notification(
                        ...
                        move |workspace, notification_error, ctx| {
                            // Log unknown errors locally, to help debug notification system issues.
            ...
            // Zap Wave 7-3: the `pane_group::Event::OpenEnvironmentManagementPane`
    ...
    fn handle_warp_drive_event(&mut self, event: &DrivePanelEvent, ctx: &mut ViewContext<Self>) {
        match event {
            ...
            DrivePanelEvent::OpenSharedObjectsCreationDeniedModal(_, _) => {
                // Zap: the cloud Drive quota-denial modal has been removed, so the event is ignored outright
            }
    ...
    fn handle_update_manager_event(
            ...
            && matches!(result.operation, ObjectOperation::Create { .. })
        {
            ...
            {
                // TODO(zap-cloud-removal Phase 5): the drive sharing
                // onboarding block has been retired; `created_object` is
                // still the result of a cloud_object creation, and the
                // StoredObject model itself will be retired together in a later phase.
    ...
    fn keep_theme(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // self.theme_override, undoing the preview override that
        // select_theme wrote under ThisWindow scope while open (otherwise the
        // override would be lost after reopening).
        let window_id = ctx.window_id();
    ...
    // Zap: removed open_shared_objects_creation_denied_modal (cloud Drive quota-denial modal)
        ...
        let owner = match space {
            Space::Team { team_uid } => {
                if !UserWorkspaces::has_capacity_for_shared_workflows(team_uid, ctx, 1) {
                    // Zap: the cloud quota-denial modal has been removed, just return
                    return;
                }
    ...
    fn render_tab_bar_contents(
        ...
        if vertical_tabs_active {
            ...
            // away (floating in the preview, or handed off to another window).
            // While the dragged tab has been handed back into this window's own
            // tab bar it is reordered in place like an in-window drag and must
            // stay full width, otherwise the drop zone vanishes and the slot
            // oscillates ("fuzzy shake"). See
    ...
    fn render_cli_agent_titlebar_buttons(
        ...
        for agent in enum_iterator::all::<CLIAgent>() {
            ...
            // Icon shrunk to 14x14: padding increased from 4 to 5.0, button outer frame stays 24x24
    ...
    fn add_configurable_right_side_tab_bar_controls(
        ...
        if FeatureFlag::AvatarInTabBar.is_enabled() {
            ...
            // Decentralized branch: no longer render the Zap Essentials (lightbulb) button; keep only the settings gear.
            target.add_child(
...
impl TypedActionView for Workspace {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            // Decentralized branch: `Reauth` has been removed.
            // Decentralized branch: the cloud account registration / login actions have been removed.
...
impl View for Workspace {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // Notification center overlay: the mailbox dropdown panel + (when the mailbox isn't open) the toast stack.
        if FeatureFlag::HOANotifications.is_enabled()
...
/// Yields the indices of every tab in `tabs` that belongs to `group_id`, in
/// list order.
