...
///
/// No entries are currently registered in `FEATURE_INTROS` for shipping
/// builds (see its doc comment for why this fork ships the mechanism without
/// content). The only variant defined today is a `#[cfg(test)]` fixture used
/// to exercise the mechanism's tests.
...
/// view, model, or workspace wiring is required.
...
///
/// Empty in shipping builds. The pinned Warp oracle's only entry
/// (`CustomModelRouter`) promotes a Warp-hosted custom-model-router feature
/// that does not exist in this fork, and its copy ("Build a custom model
/// router for the Warp Agent...") is Warp-branded marketing content, not
/// something to carry into a BYOP fork — see `DECLINED.md`'s "Oz updates"
/// precedent (#321). The popover mechanism itself is generic and reusable;
/// populate this array again once there is a real, non-cloud Phosphor
/// feature worth announcing this way. Refs #404.
#[cfg(not(test))]
pub const FEATURE_INTROS: &[FeatureIntro] = &[];

#[cfg(test)]
pub const FEATURE_INTROS: &[FeatureIntro] = &[FeatureIntro {
    id: FeatureIntroId::Test,
    hero_image_path: "test/fixture.png",
    badge: None,
    title: "Test feature intro",
    description: "Fixture content used to exercise the feature-intro mechanism's tests.",
...
pub fn init(_app: &mut AppContext) {
    // Not wired into the workspace yet: this fork ships the popover mechanism
    // with an empty `FEATURE_INTROS` (see its doc comment), so nothing ever
    // triggers it and there is no workspace-level mount/keybinding to
    // register. Add the Escape binding here (gated on the workspace having an
    // active feature intro) when a real entry lands. Refs #404.
}
