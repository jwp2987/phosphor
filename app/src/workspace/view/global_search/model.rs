...
use remote_server::proto::{ripgrep_search_response, RipgrepSearchRequest, RipgrepSearchSuccess};
...
impl GlobalSearch {
    ...
    fn spawn_remote_search(
        ...
        host_id: HostId,
        pattern: String,
        roots: Vec<StandardizedPath>,
        ignore_case: bool,
        multiline: bool,
        ctx: &mut ModelContext<Self>,
    ) {
        // Bridge the warp_util HostId (carried by the remote root) to the
        // warp_core HostId the remote-server manager speaks, then resolve the
        // per-host client. The fork routes on each host's own connection, so
        // there is no host-scoped-request multiplexing here — the client call
        // is issued directly (see [`crate::code::buffer_location`]).
        ...
        self.spawn_source(
            ...
            async move {
                let Some(client) = client else {
                    log::warn!(
                        "GlobalSearch: no remote server client for host {}; skipping remote source",
                        host_id.as_str()
                    );
                    return None;
                };
                let request = RipgrepSearchRequest {
                    pattern,
                    roots: roots.iter().map(|p| p.as_str().to_string()).collect(),
                    ignore_case,
                    multiline,
                    max_matches: REMOTE_MAX_MATCH_COUNT,
                };
                match client.ripgrep_search(request).await {
                    Ok(response) => match response.result {
                        Some(ripgrep_search_response::Result::Success(success)) => {
                        ...
                        Some(ripgrep_search_response::Result::Error(err)) => {
                            log::warn!(
                                "GlobalSearch: remote search failed for host {}: {}",
                                host_id.as_str(),
                                err.message
                            );
                            None
                        ...
                        None => {
                            log::warn!(
                                "GlobalSearch: empty remote search response for host {}",
                                host_id.as_str()
                            );
                            None
                        }
                    ...
                    Err(err) => {
                        log::warn!(
                            "GlobalSearch: remote search RPC failed for host {}: {err}",
                            host_id.as_str()
                        );
                        None
                    }
