//! Shell prompt detection. Used by the SSH injectors (`secret_injector` /
//! `startup_command_injector` / `su_password_injector`) to trigger their
//! action only after login has completed and the shell is ready.
//!
//! Only looks at the last 256 bytes of the buffer, matching several common
//! prompt endings:
//! - ASCII: `$ ` / `# ` / `> `
...
/// Checks whether the end of the buffer matches a shell prompt pattern.
pub fn bytes_look_like_shell_prompt(bytes: &[u8]) -> bool {
    ...
    // Multi-byte prompt symbol + space
