...
// --- Custom theme portable path storage tests ---
// Ported from Warp pin 02b53fcd8's `app/src/themes/theme_tests.rs` (issue #295): the fork
// previously stored a custom theme's file path as a plain absolute `PathBuf`, which cannot
// round-trip through `settings.toml` on a different machine/OS/username. These tests exercise
// the portable relative-storage-string representation restored above.
