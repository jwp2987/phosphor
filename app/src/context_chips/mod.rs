...
impl ChipValue {
    ...
    /// Returns the `GitBranchTrackingStatus` payload, if this is the
    /// `GitBranchStatus` variant.
    ...
    chips.iter().find_map(|chip| {
        if matches!(chip.kind(), ContextChipKind::GitDiffStats) {
            chip.value().map(|value| match value {
                ...
                // Only GitDiffStats chips reach here; any other value has no line changes.
                _ => display_chip::GitLineChanges {
                    files_changed: 0,
                    lines_added: 0,
                    lines_removed: 0,
                },
