...
//!
//! Ported from Warp (`app/src/login_item/windows.rs`).
            ...
            async move {
                if add_app_as_login_item {
                    match register(&value_name, &exe) {
                        Ok(()) => true,
                        Err(err) => {
                            log::warn!("Failed to register as a login item: {err}");
                            false
                        }
                    }
                } else {
                    match unregister(&value_name) {
                        ...
                        Err(err) => {
                            ...
                            log::warn!("Failed to unregister as a login item: {err}");
                        }
                    }
                    false
                }
...
///
/// Using the channel's application name keeps different channel installs
/// isolated, so installing multiple channels side by side doesn't cause one
/// to overwrite another's startup entry.
...
/// `C:\Program Files\Phosphor\warp.exe`) are parsed as a single executable
/// path.
