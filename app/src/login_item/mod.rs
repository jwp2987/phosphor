//! Login-item registration — makes Phosphor start automatically when the
//! user signs in to their OS.
//!
//! Ported from Warp (`app/src/login_item/`). The user-facing toggle and
//! "already registered" bookkeeping live on
...
//!
//! Only macOS and Windows are supported (matching Warp); on Linux this
//! module compiles to an empty shell — there is no equivalent standard
//! "launch at login" mechanism to hook here, same as upstream Warp.
...
/// Reconciles whether the app is registered to launch at login with the
/// user's current preference.
///
/// Respects the existing `app_added_as_login_item` bookkeeping so a user who
/// removed the app from their OS's startup UI isn't silently re-added — the
...
/// auto-launch `target/debug/warp` at sign-in.
#[cfg(any(target_os = "macos", target_os = "windows"))]
