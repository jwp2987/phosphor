...
impl DriveIndex {
    ...
    fn online_only_operation_allowed(
        ...
        if let Some(object) = ObjectStoreModel::as_ref(app).get_by_uid(&object_type_and_id.uid()) {
            // Zap (decentralized branch): local objects (no server_id, since
            // the SyncQueue upload path is a no-op without auth) would never
            // get the trash/move menu under the original logic. This treats
            // "no server_id" as a local object, allowing it to perform
            // local-side operations (trash goes through sqlite, no server
            // coordination needed).
    ...
    fn render_item_and_children(
        ...
        let row = WarpDriveRow::new_from_cloud_object(
            ...
            is_selected,
            is_focused,
            false, /* Zap (Wave 4): SyncQueue was fully removed, is_dequeueing is always false */
            tools_panel_menu_direction(app),
            appearance,
        )?;
    ...
    fn retry_all_failed(&mut self, ctx: &mut ViewContext<Self>) {
        // Zap (Wave 4): after SyncQueue was fully removed, "retry"'s original
        // meaning (re-reporting to the server) no longer applies; after
        // localization, objects never enter the errored state, so this path is dead code.
        let _ = ctx;
    }
    ...
    fn index_menu_items(
        ...
        if let ObjectTypeAndId::Folder(folder_id) = object_type_and_id {
            // Zap: local folders (ClientId, since the SyncQueue upload path
            // is a no-op without auth) never get a server_id; the original
            // `SyncId::ServerId(_) && is_online` double gate would leave
            // local folders permanently without a "new child/Rename" context
            // menu. This treats "local folder" as always ready.
            ...
            if is_local_folder || server_folder_ready {
                ...
                if let Some(object) = object {
                    if let Some(object_link) = object.object_link() {
                        ...
                        // TODO(zap-cloud-removal Phase 5): `editability` at
                        // this point is only used to decide share-menu
                        // visibility. The share menu has been removed, but
                        // editability is still part of the StoredObject
                        // permission model, kept pending cloud_object's overall retirement.
                        let _ = editability;
                    }
                }
