// Ported from Warp's `app/src/local_control/mod_tests.rs` at the pinned oracle
// (`02b53fcd8`, Warp `2026.07.29.09.05` stable -- see `ORACLE.md`). This
// fork's `mod.rs` differs from the oracle only in doc wording and an event
// parameter that this fork's `subscribe_to_model` does not pass, so every
// test below ports unchanged except `capabilities_advertises_the_complete_catalog`:
// this fork demoted `SurfaceAgentManagementOpen` from `Implemented` to `Stub`
// in `crates/local_control/src/catalog.rs` (the Agent Management view it
// would open was removed with cloud-runner orchestration -- see DECLINED.md),
// so the implemented-catalog size is 83 here, not the oracle's 84.
...
fn capabilities_advertises_the_complete_catalog() {
    // 83, not the oracle's 84: this fork demoted `SurfaceAgentManagementOpen`
    // to `ActionImplementationStatus::Stub` (see module header comment).
