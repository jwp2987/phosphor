...
pub(crate) fn surface_unavailable_reason(
    ...
    match destination {
        ...
        // Unconditionally unavailable: the Agent Management view this surface would
        // open was removed along with cloud-runner orchestration (see DECLINED.md;
        // `app/src/notifications/model.rs`'s note on commit 002ce467) and there is no
        // `FeatureFlag::AgentManagementView` or `WorkspaceAction::OpenAgentManagementView`
        // to gate on in this fork. `ActionKind::SurfaceAgentManagementOpen` is marked
        // `ActionImplementationStatus::Stub` in `crates/local_control/src/catalog.rs`
        // for the same reason, so this arm is defense in depth, not the primary gate.
        SurfaceDestination::AgentManagement => Some("agent management is not available in this build"),
    }
}
