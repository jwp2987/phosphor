...
pub(crate) fn handle(
    ...
    match action {
        ...
        ActionKind::SurfaceWarpDriveOpen => surface_ensure_open(
            instance_id,
            action,
            SurfaceDestination::WarpDrive,
            WorkspaceAction::ToggleWarpDrive,
            |workspace| workspace.is_warp_drive_open(),
            target,
            ctx,
        ),
        // `SurfaceAgentManagementOpen` is `ActionImplementationStatus::Stub` in
        // `crates/local_control/src/catalog.rs` -- the Agent Management view this
        // would open was removed along with cloud-runner orchestration (see
        // DECLINED.md). `is_implemented()` checks in `bridge::validate_request_authority`
        // and `mod::issue_credential` reject it before dispatch ever reaches here;
        // this arm exists only because the match must stay exhaustive.
        ActionKind::SurfaceAgentManagementOpen => Err(ControlError::new(
            ErrorCode::UnsupportedAction,
            "surface.agent_management.open is not implemented",
        )),
        ...
        ActionKind::SurfaceProjectExplorerOpen => surface_workspace_action(
            ...
            // The fork has only `ToggleProjectExplorer` (the pin's `OpenProjectExplorer`
            // does not exist), and there is no `is_project_explorer_open` query to make
            // this idempotent the way `SurfaceWarpDriveOpen` is above. This dispatches
            // a toggle, so it may *close* an already-open project explorer instead of
            // being a no-op. Known limitation -- see the PR description.
        ...
        ActionKind::SurfaceConversationListOpen => surface_workspace_action(
            ...
            // Same limitation as `SurfaceProjectExplorerOpen` above: only
            // `ToggleConversationListView` exists in this fork.
...
/// Like [`surface_workspace_action`], but only dispatches `toggle_action` when
/// `is_open` reports the surface is not already open, so the net effect is an
/// idempotent "ensure open" rather than a toggle that could close it.
fn surface_ensure_open(
    instance_id: &Option<InstanceId>,
    action_kind: ActionKind,
    destination: SurfaceDestination,
    toggle_action: WorkspaceAction,
    is_open: impl Fn(&Workspace) -> bool,
    target: &TargetSelector,
    ctx: &mut ModelContext<LocalControlBridge>,
) -> Result<serde_json::Value, ControlError> {
    ensure_surface_available(action_kind, destination, ctx)?;
...
fn resolve_file_open_path(
    ...
    let (is_local, working_directory) = pane_group.read(ctx, |pane_group, ctx| {
    ...
    match working_directory {
        ...
        None if is_local == Some(false) => Err(ControlError::new(
            ErrorCode::TargetStateConflict,
