//! Settings configuring the TUI's zero-state (empty/idle) screen.
//!
//! Ported from Warp's `app/src/settings/tui_zero_state.rs` at the pinned
//! oracle (`02b53fcd8`, Warp `2026.07.29.09.05` stable — see `ORACLE.md`).
//! Warp's `surface: SettingSurfaces::TUI` marker is dropped, matching
//! `tui_autoupdate.rs`: this fork's settings macro predates the surface
//! concept, so these keys live in the shared settings file rather than a
//! TUI-only surface. Behavior is otherwise identical.
...
define_settings_group!(TuiZeroStateSettings, settings: [
    object: TuiZeroStateObjectSetting {
        ...
        max_table_depth: 0,
        description: "The object rotated in the TUI zero state. Use built_in or an ascii_file path relative to the TUI settings directory. Changing this setting reloads the object; editing the linked file requires a restart.",
    },
