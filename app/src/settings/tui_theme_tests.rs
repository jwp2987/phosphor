// Ported from Warp's `app/src/settings/tui_theme_tests.rs` at the pinned
// oracle (`02b53fcd8`, Warp `2026.07.29.09.05` stable — see `ORACLE.md`).
// The oracle's `theme_schema_entry_is_tui_only` test asserts on
...
// hierarchy/sync-to-cloud shape that does exist in this fork.
...
#[test]
fn theme_setting_is_local_and_defaults_to_automatic_detection() {
...
#[test]
fn theme_schema_entry_is_registered_and_public() {
    let entry = inventory::iter::<SettingSchemaEntry>
        .into_iter()
        .find(|entry| entry.hierarchy == Some("appearance") && entry.storage_key == "theme")
        .expect("expected TUI theme schema entry");

    assert!(!entry.is_private);
    assert_eq!(
        entry.description,
        "The TUI color theme. Auto matches the host terminal background."
    );
}
