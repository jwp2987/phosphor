...
impl UserWorkspaces {
    ...
    // Zap: team spaces are a cloud collaboration entry point; the local version doesn't expose any Team space.
    pub fn team_spaces(&self) -> Vec<Space> {
        vec![]
    }

    // Zap: Drive only keeps the local Personal space. Team / Shared are both
    // cloud collaboration surfaces; even if old cache still has workspace
    // metadata, it must not re-enter the Drive or Workflow UI.
    pub fn all_user_spaces(&self, ctx: &AppContext) -> Vec<Space> {
        let _ = ctx;
        vec![Space::Personal]
    }

    // Zap (localization branch): the personal space's owner is fixed to the
    // local placeholder user. This must stay stable, otherwise old objects'
    // owner fields won't match after a restart, and old data becomes
    // "invisible" in the Personal Space list.
    fn effective_personal_user_uid() -> UserUid {
        UserUid::new(TEST_USER_UID)
    }

    // Returns the [`Owner`] for the user's personal drive.
    // Zap: Create actions under Drive's Personal space — Workflow / EnvVar /
    // Folder / Notebook / Import, etc. — are all uniformly attributed to the
    // local placeholder user (persisted only to local sqlite).
    ...
    pub fn owner_to_space(&self, owner: Owner, ctx: &AppContext) -> Space {
        ...
        match owner {
            Owner::User { user_uid } => {
                ...
                // Zap: compare using effective_personal_user_uid, to ensure
                // that under no-auth, the local Owner (user_uid="zap") is also
                // attributed to Personal rather than Shared.
    ...
    pub fn remove_user_from_team(
        ...
        // Zap (localization): the remove-member path has no remote team write target locally → no-op.
    ...
    pub fn add_invite_link_domain_restrictions(
        ...
        // Zap (localization): the domain-restriction path has no remote
        // team/invite write target locally → emit a Success event so the UI doesn't get stuck.
    ...
    pub fn refresh_ai_overages(&mut self, _ctx: &mut ModelContext<Self>) {
        // Zap (localization, Phase 5): there's no cloud AI overages query locally, no-op.
        // The call site (`blocklist/controller.rs::maybe_refresh_ai_overages`) UI doesn't initiate any meaningful update.
    }
...
// localization these paths are unreachable, so the entire file was physically removed.
