...
use super::{
    ...
    TuiMcpServerStatus, TuiMcpTemplateVariable, TuiMcpVariableValue,
    is_represented_by_global_warp_server, server_priority, sort_servers, template_identity,
...
fn server(id: TuiMcpServerId, name: &str, status: TuiMcpServerStatus) -> TuiMcpServerSnapshot {
    TuiMcpServerSnapshot {
        ...
        name: name.to_owned(),
        description: None,
        source: TuiMcpServerSource::Template,
        transport: None,
        status,
        tool_count: 0,
        resource_count: 0,
...
fn installed_rows_sort_before_available_rows_then_by_name_and_id() {
    let mut servers = vec![
        server(
            TuiMcpServerId::Template(Uuid::from_u128(1)),
            "Alpha",
...
#[test]
fn file_source_label_names_every_provider_and_scope() {
    let source = TuiMcpServerSource::FileBased {
        sources: vec![
            TuiMcpFileSource {
                provider: "Claude".to_owned(),
            ...
            TuiMcpFileSource {
                ...
                root_path: "/src/my-repo".into(),
                scope: TuiMcpFileScope::Project,
            },
        ],
    };

    assert_eq!(source.label(), "Claude global, Codex · my-repo");
    assert_eq!(
...
#[test]
fn global_warp_server_replaces_only_the_matching_saved_template() {
    let local = template(
    ...
    assert!(is_represented_by_global_warp_server(&saved, &identities));
    assert!(!is_represented_by_global_warp_server(&other, &identities));
}
...
#[test]
fn variable_validation_rejects_an_empty_or_duplicated_value() {
    let variables = vec![TuiMcpTemplateVariable {
        key: "TOKEN".to_owned(),
        allowed_values: None,
    }];
