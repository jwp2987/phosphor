//! # Zap localization notes (Phase 2d-4b, revised 2026-05-11)
//!
//! In upstream Zap this module holds the "cloud object" abstraction, uniformly
//! describing object types like Notebook / Workflow / EnvVar / Fact / MCP /
//! ExecutionProfile / AIDocument that need to sync across multiple devices.
//!
//! In Zap the cloud sync pipeline (RTC / UpdateManager / SyncQueue /
//! ServerApiProvider) has been stripped out (see
//! `docs/zap-cloud-removal-plan.md`), and this module **becomes a purely
//! local-object abstraction**:
//!
//! - The `StoredObject` trait → its actual meaning is now "local domain object
//!   trait", carrying metadata / permissions / versions / display_name /
//!   upsert_event / as_any / clone_box; the `Cloud` prefix in its name is kept
//!   only to reduce the diff surface for cross-upstream cherry-picks and no
//!   longer carries any cloud meaning.
//! - `GenericStoredObject<K, M>` → the generic carrier struct for local domain objects.
//! - The `StoredObjectModel` trait → describes local object types.
//! - `ObjectStoreModel` (`model/persistence.rs`) → the in-process global store
//!   for local objects + SQLite-backed storage.
//! - `ObjectStoreEvent` → the local model change event bus, subscribed to by local UI views.
//! - `ObjectTypeAndId` → the local ID discriminant, used in 60+ places across the Drive UI / search / etc.
//!
//! The reason for "keeping the original names + doc comments" instead of
//! physically renaming (`StoredObject` → `LocalObject`) is to defer the
//! 200+-site cascading rename until the upstream sync strategy stabilizes;
//! this phase **only annotates that the semantics have been localized**,
//! without touching symbol names.
//!
//! The truly "server round-trip" types are being physically removed in
//! batches; the server object enum, field conversions, initial-load fan-in,
//! and the old server generic object carrier struct have been removed.
...
///
/// Zap (Wave 4): originally defined in `crate::server::sync_queue`; moved here
/// once SyncQueue was fully removed. Several models' `serialized()` still
/// return it (used when writing to sqlite locally).
...
///
/// Because the historical cloud-object naming is still kept, every current
/// local StoredObject needs to implement GenericStoredObject.
///
...
pub trait StoredObject: Debug {
    ...
    /// The StoredObject trait can't directly require Clone to be derived, or the trait stops being object safe.
    fn clone_box(&self) -> Box<dyn StoredObject>;
}

/// Defines a common trait for object store models to implement.
/// The "model" is the stored object's domain data — e.g. the actual content
/// of a notebook/workflow/folder; metadata, permissions, and sync-status logic don't live here.
///
...
/// GenericStoredObject is the generic local-object implementation carried over from the historical cloud-object system.
///
