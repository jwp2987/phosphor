...
fn take_conversation_hands_out_each_conversation_at_most_once() {
    ...
    let (mut store, failed) =
        RestoredAgentConversations::new(vec![persisted_conversation(conversation_id)]);
...
fn failed_take_does_not_consume_the_restore_opportunity() {
    let conversation_id = AIConversationId::new();
    // No seed: the first take fails to load.
    let (mut store, failed) = RestoredAgentConversations::new(vec![]);
    assert!(failed.is_empty());
