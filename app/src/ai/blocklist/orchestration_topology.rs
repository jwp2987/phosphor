...
//! orchestration pill bar so other surfaces (e.g. keyboard navigation) can
//! walk and order the same tree without duplicating the logic.
//!
//! Local only: every conversation walked here is a process running on this
//! machine (`app/src/pane_group/pane/local_harness_launch.rs`). There is no
//! remote-worker execution path in this fork — see
//! [`AIConversation::is_remote_child`]'s doc comment.
...
///
/// "Local" means the child runs in this client (`!is_remote_child()`) —
/// always true in this fork, since there is no remote-worker execution path;
/// the check is kept because the pin's `AIConversation` shape (and the
/// possibility of legacy/foreign data with the flag set) is preserved. See
/// [`AIConversation::is_remote_child`]. "Active" means the child is not in a
/// terminal state (`!ConversationStatus::is_done()`), so a session whose
/// children have all finished is not treated as still orchestrating.
///
/// Descendants whose `AIConversation` is not loaded are ignored — active
/// local children are always loaded in this client.
