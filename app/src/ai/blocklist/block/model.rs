...
pub mod testing {
    ...
    pub struct FakeAIBlockModel {
        ...
        /// `None` models a block that is still streaming output, so its status stays
    ...
    impl FakeAIBlockModel {
        ...
        /// Builds a fake model whose status stays [`AIBlockOutputStatus::Pending`], modeling a
        /// block that is still streaming output. Refs #365.
