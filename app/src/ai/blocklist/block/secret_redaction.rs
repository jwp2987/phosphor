...
/// Returns the ranges of detected secrets in the given text along with their SecretLevel, using an
/// already-cloned [`SecretsRegex`]. Callers that already hold one should prefer this so that the
/// text scan and any grid scan they perform agree on the same set of patterns.
pub(crate) fn find_secrets_in_text_with_levels_using_regex(
    text: &str,
    secrets_regex: &SecretsRegex,
) -> Vec<(SecretRange, SecretLevel)> {
    // Note that we purposely use the meta::Regex instead of the RegexDFAs since we are working on
    // Text (containing a normal String) rather than the Grid (where text is in Cells with 1
    // character each).
    let SecretsRegex {
        regex,
        level_metadata,
