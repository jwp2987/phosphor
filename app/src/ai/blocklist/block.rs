...
use crate::settings::{
    ...
    InputModeSettingsChangedEvent, OrchestrationMessageDisplayMode,
};
...
pub(super) struct AIBlockStateHandles {
    ...
    /// Settings link handle for the Ask-User-Question speedbump footer. Owned
    /// here (not by the inline view) so it survives the view being rebuilt.
...
/// Builds the default collapsible state for a newly-arrived orchestration
/// message body, per the `orchestration_message_display_mode` setting (#329).
...
impl AIBlock {
    ...
    pub fn new(
        ...
        // Zap: this used to subscribe to GetRelevantFilesController's Success event
        // to fire a `ctx.notify` and refresh the UI once RAG finished. That
        // controller was retired along with outline, so the subscription is removed
        // here too.
        ...
        // Zap (Phase 3c A1): removed the subscription to
        // `AIRequestUsageModelEvent::RequestBonusRefunded`. After localization this
        // event is never emitted (there's no `provide_negative_feedback_response` RPC
        // call), so the subscription itself was permanently-idle dead code. The
        // `request_refunded_count` field is kept but always stays at its initial
        // value of None.
    ...
    fn handle_updated_output(&mut self, output: &AIAgentOutput, ctx: &mut ViewContext<Self>) {
        ...
        for message in &output.messages {
            ...
            // collapsible transcript row. Default expansion follows the
            // orchestration_message_display_mode setting (#329) rather than
            // always starting expanded.
    ...
    fn handle_complete_output(&mut self, output: &AIAgentOutput, ctx: &mut ViewContext<Self>) {
        ...
        // The ask-user-question one-shot flag is consumed only after the footer is
        // actually attached, so restored blocks and views that haven't arrived yet
        // don't burn the flag.
        for action in output.actions() {
            let should_show_file_access_speedbump =
                action.is_get_specific_files() || action.is_grep() || action.is_file_glob();
            if should_show_file_access_speedbump {
                if is_agent_mode_autonomy_allowed(ctx)
    ...
    fn handle_ask_user_question_stream_update(
        ...
        // The speedbump variant may have been seeded before this view existed; attach
        // the footer now and only then consume the one-shot flag.
        if self.sync_ask_user_question_speedbump_footer(ctx)
            && *AISettings::as_ref(ctx).should_show_agent_mode_ask_user_question_speedbump
        {
            Self::mark_ask_user_question_speedbump_as_shown(ctx);
