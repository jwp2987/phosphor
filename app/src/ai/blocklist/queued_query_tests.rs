        ...
        + 'static,
{
    App::test((), |mut app| async move {
        ...
        // model-event sender to enqueue a sqlite delete. (Upstream Warp wraps this in
        // a `initialize_history_persistence_for_tests` helper; Zap inlines the same
        // steps, mirroring `history_model_test.rs::initialize_history_model_test_app`.)
...
/// Builds a locked queued row. `PendingLrcAutoQueue` is the only locked origin in Zap
/// (upstream Warp also locks `InitialCloudMode`, a cloud feature Zap does not have), so the
/// lock-semantics tests exercise it through this origin.
...
fn pop_front_no_ops_when_head_is_locked() {
    ...
    // `PendingLrcAutoQueue` head must not be popped even if a status-transition arrives before
    // the row is unlocked by `unlock_pending_lrc_rows`.
    with_model(|mut app, model, _events| {
        let conv = AIConversationId::new();
        let initial_id = model.update(&mut app, |model, ctx| {
...
fn clear_conversations_in_terminal_view_drops_every_listed_conversation() {
    // ClearedConversationsInTerminalView must drop every cleared conversation's queue. Zap's
    // event does not carry the ids, so the model reads them back from the history model.
    with_model(|mut app, model, _events| {
        let history = BlocklistAIHistoryModel::handle(&app);
        let terminal_view_id = warpui::EntityId::new();
        let conv_a = history.update(&mut app, |h, ctx| {
...
fn has_autofireable_prompt_is_false_when_only_a_locked_head_is_queued() {
    ...
    with_model(|mut app, model, _events| {
        let conv = AIConversationId::new();
        model.update(&mut app, |m, ctx| {
            m.append(conv, pending_lrc_query("initial"), ctx)
        });
        model.read(&app, |m, _| assert!(!m.has_autofireable_prompt(conv)));
    });
}
