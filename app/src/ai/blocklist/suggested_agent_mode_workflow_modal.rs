...
impl View for SuggestedAgentModeWorkflowModal {
    ...
    fn render(&self, _app: &AppContext) -> Box<dyn Element> {
        if let Some(modal) = &self.modal {
            ...
            // Zap: this view is always mounted in the workspace tree, and
            // modal=None by default is the normal state (cloud ShowSuggestions is
            // disabled, so the chip never appears and the modal never gets
            // populated). Doesn't log a warning, to avoid polluting logs every
            // frame.
            Empty::new().finish()
        }
    }
