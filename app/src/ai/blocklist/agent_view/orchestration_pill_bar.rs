...
use crate::ai::blocklist::agent_view::avatar_disc::{
    render_agent_avatar_disc, render_avatar_disc, render_orchestrator_avatar_disc, AvatarGlyph,
    pill_avatar_color, pill_initial,
};
...
use crate::ui_components::icon_with_status::{self, BadgeInnerShape, StatusBadgeStyle};
...
impl OrchestrationPillBar {
    ...
    pub fn new(
        ...
        ctx.subscribe_to_model(&history_model, |this, _, event, ctx| match event {
            ...
            // A child's parent linkage can land after StartedNewConversation;
            // the drill-down anchor, breadcrumbs and subtree badges are all
            // keyed on that linkage, so they must refresh when it does. The
            // pin listens on `ConversationServerTokenAssigned` here; this
            // fork's agent-id index is the local equivalent.
    ...
    fn open_menu_for(&mut self, conversation_id: AIConversationId, ctx: &mut ViewContext<Self>) {
        ...
        // No "View in Oz" entry: it opens a Warp-cloud web URL this fork has
        // no counterpart for. See telemetry.rs's PillBarActionKind doc
        // comment.
...
/// leaf anchors its parent's level so sibling navigation stays symmetric. With
/// a single orchestration level this matches the historical root-anchored
/// behavior exactly.
...
impl OrchestrationPillBar {
    ...
    fn navigate_to_conversation_pane(&self, id: AIConversationId, ctx: &mut ViewContext<Self>) {
        ...
        // site) and we just want to move the user's cursor there.
        //
        ...
        //     dispatch `TerminalAction::RevealChildAgent`.
...
impl View for OrchestrationPillBar {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // Not ported: `with_remap_cross_axis_wheel_to_main_axis`, which lets a
        // standard vertical mouse wheel pan the bar horizontally, doesn't
        // exist on this fork's `NewScrollable`. Trackpad horizontal swipes
        // still work through the default path;
        // `with_propagate_mousewheel_if_not_handled` below is unaffected.
        ...
        // clicked pill's overflow button. Otherwise, when no menu is open
        // but a pill is hovered, overlay the hover details card under that
        // pill instead. The two overlays are mutually exclusive by design:
        // opening the menu clears `hovered_pill` (see `open_menu_for`).
...
fn render_hover_card(
    ...
    // entirely when neither is populated.
    ...
    let harness = conversation.orchestration_harness().unwrap_or(warp_cli::agent::Harness::Oz);
...
/// status badge that sits in the top-right of the hover card.
...
fn navigation_action_for_pill(kind: PillKind, conversation_id: AIConversationId) -> TerminalAction {
    match kind {
        ...
        // created at spawn time. Revealing that pane keeps any live harness
        // session, CLI listener, and PTY output attached to the real owner
        // instead of trying to recreate the child transcript in the current
        // pane.
...
fn render_subtree_rollup_badge(
    ...
    // The pin passes a `StatusColorStyle` here; this fork's
    // `status_icon_and_color` has no style parameter.
...
fn render_pin_button(
    ...
    let button = Hoverable::new(mouse_state, move |hover_state| {
        ...
        if hover_state.is_hovered() || hover_state.is_clicked() {
            circle = circle.with_background_color(hover_background);
        }
        circle.finish()
    })
    .with_cursor(Cursor::PointingHand)
...
fn render_pill(
    ...
    let pill_body = Hoverable::new(mouse_state, move |hover_state| {
        ...
        // Group pills skip the label reserve: their trailing badge slot
        // (below) already absorbs the 3-dot overlay.
    ...
    // hover details card overlay can anchor relative to it.
    SavePosition::new(pill_body, &pill_body_position_id(conversation_id)).finish()
}

/// Renders the trailing 3-dot button on a child pill.
...
fn render_avatar_with_status_overlay(
    ...
    let lockup = icon_with_status::render_custom_avatar_with_status_badge(
        avatar,
        Some(&status),
        is_remote_child,
        ...
        PILL_BADGE_STYLE,
        theme,
        // Cutout ring color for the badge.
...
fn render_crumb(
    ...
    .on_click(move |ctx, app, _| {
        if let Some(conversation_view_id) =
