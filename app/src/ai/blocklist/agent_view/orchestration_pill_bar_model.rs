...
impl OrchestrationPillBarModel {
    ...
    ///
    /// In the pin this runs on logout, alongside `BlocklistAIHistoryModel::reset`,
    /// so the next account doesn't inherit the previous one's pill-bar UI state.
    /// Phosphor's BYOP auth has no equivalent multi-account logout flow that
    /// clears AI state — `BlocklistAIHistoryModel::reset` itself has no
    /// production caller here either (only its unit test, `history_model_test.rs`).
    /// Kept for the same reason: the persisted/topology shape otherwise matches
    /// the pin exactly, and dropping it would mean re-adding it the moment a
    /// real consumer (e.g. a "switch local profile" feature) needs it.
