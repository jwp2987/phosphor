...
impl AgentInputFooter {
    ...
    pub fn new(
        ...
        // Zap Wave 7-3: `EnvironmentSelector` init + subscription + ambient_agent
        ...
        let mut me = Self {
            ...
            // The subsystem was physically deleted.
    ...
    pub fn toggle_cli_voice_input(
        ...
        match &self.cli_voice_input_state {
            CLIVoiceInputState::Stopped => {
                ...
                // quota gate. After localization, voice input is no longer subject to
                // cloud quota limits and can always be sent.
...
async fn write_install_log(agent: CLIAgent, err: &PluginInstallError) -> Option<PathBuf> {
    ...
    let contents = format!(
        "Phosphor plugin installation — {agent:?}\n\
         {now}\n\
         \n\
         {log}",
        log = err.log,
