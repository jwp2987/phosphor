//! The "reasoning depth" picker in the bottom-right of the input box (BYOP mode).
//!
//! Uses the same template as `EnvironmentSelector`, simplified to:
//! - Data source: `LLMPreferences::get_reasoning_effort(...)` + the currently
//!   selected model's variants table
//! - State: `LLMPreferences::reasoning_effort_per_terminal` (session-only)
//! - Doesn't write to settings.toml, doesn't send telemetry, doesn't touch cloud
//! - When the current model doesn't support reasoning (variants is empty), the whole
//!   component renders empty and the picker naturally disappears
...
/// The "Reasoning Depth" selector in the input box toolbar.
pub struct ReasoningDepthSelector {
    ...
    /// The (api_type, model_id) the picker is currently associated with; refreshed as LLMPreferences changes.
...
impl ReasoningDepthSelector {
    pub fn new(
        ...
        let dropdown = ctx.add_typed_action_view(move |ctx| {
            // Uses the CodeReview style (no search input, no environment sidecar,
            // visually compact). Can't use Environments — its sidecar path feeds
            // action_data() into `ServerId::from_string_lossy`, and our effort names
            // (e.g. "Auto"/"Off") aren't 22 characters, which triggers a panic in
            // debug builds.
            DisplayChipMenu::new(
    ...
    /// Resolves the currently selected model → (api_type, model_id) if it's a BYOP model, otherwise None.
    ...
    fn refresh(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // Empty variants → the whole component renders empty from here on
    ...
    /// Whether this should currently render (the model supports reasoning).
...
impl View for ReasoningDepthSelector {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        if !self.is_visible() {
            // A model that doesn't support reasoning → render empty
