        ...
        #[cfg(not(target_family = "wasm"))]
        {
            ...
            ActionExecution::new_async(
                async move {
                    reconnecting_peer
                        .call_tool(
                            // `CallToolRequestParams` (was `CallToolRequestParam`)
                            // is `#[non_exhaustive]` in rmcp 1.x, so it must be
                            // built through its constructor rather than a literal.
...
fn coerce_value_against_schema(value: &mut serde_json::Value, schema: &serde_json::Value) {
    // NOTE: Warp writes the two guards below as let-chains. The fork's
    // `.rustfmt.toml` still pins `edition = "2018"` (issue #191), under which
    // `rustfmt` refuses to parse let-chains, so they are spelled as nested
    // `if let`s here. Semantics are identical.
