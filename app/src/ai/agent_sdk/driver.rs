...
pub struct AgentDriver {
    ...
    /// Model config for the selected third-party harness, if the run selected one.
...
pub enum AgentDriverError {
    ...
    // `EnvironmentSetupFailed(String)` was removed 2026-08-17: nothing in this
    // fork could construct it. At the pin its sole producer is
    // `impl From<PrepareEnvironmentError> for AgentDriverError`
    // (`42effe840:app/src/ai/agent_sdk/driver.rs:766-773`), and
    // `PrepareEnvironmentError` is defined in `driver/environment.rs` — a file
    // this fork never ported because it opens with
    // `use crate::ai::cloud_environments::{CodeForge, SourceRepo}` (Warp
    // Environments, DECLINED #211). Re-add it only alongside a local
    // environment-preparation path that can actually fail.
    ...
    HarnessRuntimeFailureDetected {
        harness: String,
        /// The originating needle from `ThirdPartyHarness::runtime_error_patterns` that hit.
        pattern: String,
...
impl AgentDriver {
    pub fn new(
        ...
        // Zap initializes the local user on startup; reaching this point means the local auth singleton wasn't initialized correctly.
    ...
    pub fn run(
        ...
        async move {
            ...
            // The pin defers here after its `EnvironmentSetupFailed` (a variant
            // this fork removed; see the note in `AgentDriverError`) /
            // `SetupCommandExitedShell`, sleeping up to a 120s
            // `SETUP_FAILED_IDLE_TIMEOUT` so a viewer can attach to the shared
            // session and read the error out of the scrollback. Deliberately
            // removed rather than ported: this fork has no viewer and no attach
            // path. `should_share` is hardcoded `false` (`agent_sdk/mod.rs`),
            // discarded in `driver/terminal.rs`, and documented as inert in
            // `warp_cli::share`; there is no `Attach` agent subcommand. The
            // sleep therefore waited for a connection that cannot arrive and
            // then returned the very error it returns immediately now.
            //
            // The pin's own rationale confirms the dependency: the doc comment
            // on `idle_window_for_terminal_status` in `42effe840:driver.rs`
            // says the failure window keeps "a failed run's shared session
            // attachable" and that "the agent process is the session sharer".
            // No sharer, no session to keep attachable. Do not re-add this from
            // the pin.
            match rx.await {
                Ok(result) => result,
                Err(Canceled) => {
    ...
    /// Resolve MCP specs into the harness-native JSON shape third-party CLIs consume.
    ///
    /// Third-party harnesses spawn their own MCP servers from their own config, so unlike
    /// the Oz path (which starts the servers in-process, see [`Self::start_mcp_servers`])
    /// this only materializes the configuration.
    ///
    /// Ported from the pin (`app/src/ai/agent_sdk/driver.rs:1133-1176`, `02b53fcd8`). The
    /// pin's version also takes a `ManagedMcpClient` so a UUID that isn't installed locally
    /// can be fetched from the managed-MCP GraphQL API; that is dropped cloud, so this fork
    /// resolves against locally-installed servers only — exactly what [`Self::resolve_mcp_specs`]
    /// already does for the Oz path.
    async fn resolve_mcp_specs_to_json(
        ...
        let (local_uuids, ephemeral_installations) = foreground
            .spawn(move |_, _| Self::resolve_mcp_specs(&specs))
            .await??;
        ...
        let mut installations = foreground
            ...
            .await??;
        installations.extend(ephemeral_installations);
    ...
    async fn prepare_harness(
        ...
        // Resolve MCP specs into harness-native JSON format. Third-party harnesses declare
        // their MCP servers in their own config, so nothing is started here.
        let resolved_mcp_servers =
            Self::resolve_mcp_specs_to_json(mcp_specs, Arc::clone(&secrets), foreground).await?;
    ...
    /// synthesize an [`AgentDriverError::HarnessRuntimeFailureDetected`],
    /// unless the CLI session already independently reported `Success` (see
    /// `harness_output_monitor::should_suppress_runtime_failure`).
    async fn run_harness(
        ...
        // value below after cleanup runs.
                ...
                detected = scanner_fut => {
                    if let Some(error) = detected {
                        ...
                        if harness_output_monitor::should_suppress_runtime_failure(
                            ...
                            log::info!(
                                "Ignoring runtime failure for {harness_name}: session already \
                                 reported Success (pattern={}, excerpt={})",
                                error.pattern,
                                error.excerpt,
                            );
                        } else {
    ...
    fn execute_run(
        ...
        ctx.subscribe_to_model(&history_model_handle, move |me, event, ctx| {
            ...
            match event {
                ...
                BlocklistAIHistoryEvent::UpdatedConversationStatus { terminal_view_id: conversation_terminal_id, conversation_id, .. } => {
                    ...
                    // wait_for_events keeps the run alive via the action_model's
                    // running_actions; the executor owns the watchdog. Don't resolve run_exit.
                    ...
                    if conversation.status().is_transient_error() {
                        ...
                        log::info!(
                            "Error occurred but automatic resume is pending; waiting up to {AUTO_RESUME_TIMEOUT:?} for retry"
                        );
        ...
        // openWarp doesn't sync plans to Zap Drive; the original "plan_artifact_created"
        // CLI output depended on the cloud notebook_link, so we no longer subscribe to
        // AIDocumentModel's SaveStatusUpdated event here.
