//! Claude Code transcript layout helpers.
//!
//! This module owns:
//! - [`ClaudeTranscriptEnvelope`] — the on-disk shape of a saved Claude session
...
//!   so Claude's `--resume <uuid>` lookup can find a jsonl written to a new location. Upstream
...
//! `ORACLE.md`), trimmed to the local, non-cloud subset: the oracle's [`ClaudeResumeInfo`],
//! `rehydrate_claude_transcript`, `rehydrate_claude_transcript_from_reader` and
//! `write_envelope_for_local_continuation` all exist to rehydrate an envelope *downloaded
//! from Warp's server* for a cloud-run resume -- this fork drops the cloud backend entirely
//! (`AGENTS.md` §5.10) and has no third-party-harness resume feature of any kind yet (no
//! `ResumePayload`, no `fetch_resume_payload` on `ThirdPartyHarness`), so those wrappers would
//! have no caller and no local envelope source to rehydrate from. What's ported here is the
//! reusable part: reading and writing Claude Code's own on-disk transcript format. See #289.
...
/// The on-disk shape of a complete Claude Code session.
///
...
/// `claude --resume <uuid>` lookup can find the jsonl written by [`write_envelope`].
///
