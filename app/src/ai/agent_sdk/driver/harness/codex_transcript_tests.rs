//! Ported verbatim from the pinned Warp oracle (`02b53fcd8`,
//! `app/src/ai/agent_sdk/driver/harness/codex_transcript_tests.rs`).
//!
//! All nine oracle tests port unchanged: they only exercise Codex's own on-disk
//! rollout format (`codex_sessions_root`, `find_session_file`, `parse_session_meta`,
//! `CodexTranscriptEnvelope`, `write_envelope`), none of which touches the dropped
//! cloud backend. See #323.
use std::fs;
use std::path::Path;
