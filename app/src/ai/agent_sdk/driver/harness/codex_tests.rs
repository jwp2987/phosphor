...
//! `app/src/ai/agent_sdk/driver/harness/codex_tests.rs`), for #323.
//!
//! 35 of the oracle's 38 tests port **verbatim** — every config-preparation,
//! auth-seeding, secret-resolution, MCP-writing, git-repo-discovery and
//! command-building test. None of them touches the dropped cloud backend.
//!
//! The three that are **not** ported are `fetch_resume_payload_maps_404_to_resume_state_missing`,
//! `fetch_resume_payload_maps_other_errors_to_load_failed` and
//! `fetch_resume_payload_returns_codex_variant_on_success`. They construct a
//! `MockHarnessSupportClient` and call `CodexHarness::fetch_resume_payload`, which
//! downloads a stored transcript envelope from Warp's server. This fork has no
//! `server_api::harness_support` module, no `HarnessSupportClient` to mock, no
//! `ResumePayload` enum, and no `fetch_resume_payload` method on `ThirdPartyHarness`
//! — the feature under test does not exist here at all. There is no local behaviour
//! to re-point them at, so per `AGENTS.md` §5.6/§5.11 they are left out rather than
//! stubbed into passing. `SCOPE-AI.md` estimates seven tests need adapting for the
//! cloud seam; the measured number is three, because the other four
//! (`resolve_openai_base_url_from_secret_*`) were blocked only by the absent
//! `ManagedSecretValue::OpenaiApiKey` variant — a local BYOP secret type, now
//! restored — and port unchanged.
...
fn resolve_openai_api_key_prefers_env_over_resolved_map() {
    // Process env var wins over the resolved secret map because
...
fn resolve_openai_base_url_from_secret_returns_none_when_worker_env_wins() {
    // When an OPENAI_API_KEY already exists in the process env,
