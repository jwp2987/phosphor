//! Tests for [`TerminalDriver`]'s shell-lifecycle handling.
//!
//! The pin's file (`app/src/ai/agent_sdk/driver/terminal_tests.rs`) also holds
//! `extend_shared_session_retention_emits_event_for_active_sharer`, which
//! exercises the cloud session-sharing retention path this fork does not have.
//! Only the shell-exit test is ported here.
...
fn shell_exit_fails_in_flight_and_subsequent_commands() {
    App::test((), |mut app| async move {
        ...
        // redacted, since the error flows into error reports.
