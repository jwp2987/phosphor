...
use crate::{
    ...
    terminal::{
        ...
        model::grid::{Dimensions, RespectDisplayedOutput},
        model::index::Point,
...
pub(crate) struct TerminalDriver {
    ...
    ///
    /// Stored redacted because it flows into error reports via
    /// [`AgentDriverError::SetupCommandExitedShell`].
...
impl TerminalDriver {
    ...
    fn new(
        ...
        // Immediately write task_id and the attachments download directory into the AI controller, for use by local task metadata and attachments.
    ...
    ) -> Result<impl Future<Output = Result<CommandHandle, AgentDriverError>> + use<>, AgentDriverError>
    {
        ...
        // flows into error reports if the shell dies, so never retain the raw
        // command here.
