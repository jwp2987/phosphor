...
// ── Ported from the pinned oracle (02b53fcd8), `app/src/ai/agent/task_store_tests.rs` ──
//
// `prune_unreachable_subtasks` is what keeps a rewind from silently re-running a
// sub-agent: rewinding past the `ToolCall::Subagent` message that spawned a
// subtask orphans the subtask, but nothing removes it, so every walk of the store
// -- request construction above all -- keeps finding it. That is a user-visible
// failure (the sub-agent runs again), and it had no coverage here.
//
// The pin returns `()` from `prune_unreachable_subtasks`; this fork returns the
// removed ids. The assertions are unchanged -- they read the store afterwards
...
fn test_prune_unreachable_subtasks_removes_orphan_and_its_exchanges() {
    // An optimistic root has no api messages, so the reachability walk finds
    // nothing beyond the root itself -- any subtask is unreachable.
    let root_task = create_test_task_with_exchanges(1);
    let root_task_id = root_task.id().clone();
    let mut store = TaskStore::with_root_task(root_task);
...
fn test_prune_unreachable_subtasks_keeps_reachable_subtask() {
    // A restored root whose api messages contain a subagent tool call -- the
    // subtask is reachable and must survive the prune.
    ...
    let root_task = Task::new_restored_root(root_api, std::iter::empty());
    let mut store = TaskStore::with_root_task(root_task);

    let subtask_api = create_api_task(subtask_id_str, vec![]);
    let subtask = Task::new_restored_root(subtask_api, std::iter::empty());
    ...
    // The subtask is reachable -- it must not be removed.
    assert_eq!(store.task_count(), 2);
...
fn test_prune_unreachable_subtasks_removes_nested_orphans() {
    // Optimistic root -> child (whose subagent-call exchange points at a
    // grandchild). The root has no api messages, so neither is reachable.
