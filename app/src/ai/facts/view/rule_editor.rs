...
impl RuleEditorView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        // Zap (localization, Phase 2d-1): the original NetworkStatus subscription
        // was used to redraw on online/offline state changes (in tandem with the
        // `is_delete_allowed` predicate); after localization that predicate is
        // always true, making the subscription dead code, so it's removed.
...
impl View for RuleEditorView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // Zap (localization, Phase 2d-1): the original `is_delete_allowed` depended
        // on network online + server_id; after localization, deletion is allowed
        // whenever an existing rule is being edited, so the predicate is dropped.
