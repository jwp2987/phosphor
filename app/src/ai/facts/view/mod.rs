// Zap (localization, Phase 2d-1): this file used to carry the role of "offline
// banner / sync-status predicates". Now that the cloud leg (SyncQueue /
// NetworkStatus online gating) has been fully retired, all that code is
// meaningless, so it's removed wholesale and imports are trimmed. The pane
// container view itself is kept, responsible for switching between the Rules /
// RuleEditor pages.
...
// Zap (localization, Phase 2d-1): the original `is_online` / `is_delete_allowed` /
// `is_edit_allowed` / `is_syncing` predicates depended on the cloud SyncQueue and
// network online state. After localization, rules are always editable, always
// deletable, and never in a "syncing" state, so the predicates are eliminated
// outright, along with their call sites.
