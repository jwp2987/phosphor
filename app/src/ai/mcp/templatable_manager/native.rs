...
async fn spawn_server(
    ...
    let service = match transport_type {
        TransportType::CLIServer(cli_server) => {
            ...
            // rmcp::transport::TokioChildProcess::builder mandates
            // tokio::process::Command, so it can't be swapped for the
            // command::r#async::Command wrapper; instead, CREATE_NO_WINDOW is set
            // manually below to keep Windows from flashing a window.
            ...
            .map_err(|err| {
                if err.kind() == std::io::ErrorKind::NotFound {
                    let cwd_display = cwd_for_log
                        .as_deref()
                        .unwrap_or("<inherited from Phosphor's process cwd>");
                    logger.log(format!(
