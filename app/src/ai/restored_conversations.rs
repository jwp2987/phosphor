...
impl RestoredAgentConversations {
    ...
    /// Takes and returns the `AIConversation`s for the given IDs, sorted by first exchange
    /// start time (oldest first). IDs with no stored conversation are skipped.
...
#[cfg(test)]
#[path = "restored_conversations_test.rs"]
mod tests;
