...
/// Recognizes the `"invalid_arguments: "` prefix that
/// `chat_stream::dispatch_byop_get_relevant_files_tool` puts on the message it hands to
/// [`error_to_json`] specifically when `serde_json::from_str::<GetRelevantFilesArgs>` failed
/// to parse the model's arguments — as opposed to a disabled-profile/no-mechanism gate
/// rejection. See `web_runtime::strip_invalid_arguments_prefix` for why message-prefix
/// sniffing is the only signal available: `error_to_json`'s signature is fixed by its caller
/// in `chat_stream.rs`, which this module does not own and cannot extend with an extra "this
/// was a parse failure" flag.
...
pub fn error_to_json(e: &anyhow::Error) -> Value {
    ...
    let tool = super::get_relevant_files::TOOL_NAME;
