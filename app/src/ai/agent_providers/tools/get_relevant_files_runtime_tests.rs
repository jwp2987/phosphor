...
/// `chat_stream::dispatch_byop_get_relevant_files_tool` wraps a `serde_json::from_str::
/// <GetRelevantFilesArgs>` failure as `anyhow::anyhow!("invalid_arguments: {e}")` before
/// handing it to `error_to_json` -- this is the exact shape a malformed model tool call
/// produces. Previously that always came out as a plain `{"status":"error",...}` payload,
/// which `convert_from::invalid_arguments_rejected_tool_call` doesn't recognize, so the UI
/// rendered nothing at all: the user saw the agent silently do nothing. This locks the fix:
/// the synthesized error must carry the same `error: "invalid_arguments"` marker
...
fn malformed_args_error_is_recognizable_as_invalid_arguments() {
    let serde_err = serde_json::from_str::<GetRelevantFilesArgs>("{}").unwrap_err();
