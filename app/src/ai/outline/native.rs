...
impl RepoOutlines {
    ...
    /// Check if outlines should be built based on the "outline codebase symbols
    /// for @ context menu" setting.
    ///
    /// Zap: upstream also builds outlines when "codebase context" is enabled to
    /// feed the (cloud) codebase semantic-search feature. That feature is not
    /// present in this fork, so outlines are driven solely by the local @ context
    /// menu symbol setting.
