...
// NOTE: the pin's json_tree_tests.rs also has 5 `mcp_result_to_renderable` tests
// that exercise `McpRenderable`/`mcp_result_to_renderable` in
// `app/src/ai/blocklist/inline_action/requested_command.rs`. Those helpers are
// call-site production code, not part of `json_tree.rs` itself, and porting them
// is out of this change's scope (`app/src/ui_components/` only — see the PR
// description). They are intentionally NOT ported here; porting them requires
// also porting the MCP request/response tree wiring in `requested_command.rs`.
