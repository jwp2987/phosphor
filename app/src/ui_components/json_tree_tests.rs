...
// -----------------------------------------------------------------------
//
// `McpRenderable`/`mcp_result_to_renderable` live in
// `app/src/ai/blocklist/inline_action/requested_command.rs` (call-site
// production code, not `json_tree.rs` itself), but the pin colocates their
// tests here alongside the rest of the JSON-tree data-layer tests, so this
