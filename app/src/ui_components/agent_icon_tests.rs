...
//!
//! The invariant under test: for every canonical logical run state, every surface produces the
//! same [`IconWithStatusVariant`]. Surfaces today are:
//! - Terminal view (vertical tabs + pane header) via
//!   [`super::agent_icon_variant_from_terminal_inputs`]
//! - Run cards / conversation list via [`super::agent_icon_variant_for_run`]
...
/// [`IconWithStatusVariant`] itself can't derive `PartialEq` because `NeutralElement` carries
/// a `Box<dyn Element>`, so we extract the agent-variant fields here.
...
enum CanonicalRunState {
    ...
    CloudOzInProgress,
    /// Cloud Claude harness selected, pre-dispatch (no session, no status yet). This is the
    /// state the pre-setup icon bug regressed on -- the tab must already render the Claude
    /// brand circle even though no CLI session exists yet.
    ...
    /// Local Claude CLI session detected via command matching only (no listener, no rich
    /// status).
...
impl CanonicalRunState {
    ...
    /// Run-card inputs for this state, if it can surface as a run card. Cards only exist for
...
/// A local Claude session and an ambient Claude run must render with the same CLI agent brand
/// but differ only by `is_ambient`. This answers the product-spec ambiguity about whether
/// those should look different -- they should.
#[test]
...
/// Deviation from the pinned oracle: the pin's version of this test also toggles
/// `provenance`/`backing.has_ambient_run`/`identity.ambient_agent_task_id` on a richer
/// `AgentConversationEntry` and asserts `is_cloud_agent_run()` flips to `true`. This fork's
/// `AgentConversationEntry` (`ai::conversation_entry`) is a deliberately minimized BYOP-local
/// projection with none of those fields -- `is_cloud_agent_run()` is hard-coded `false` there
/// (see that module's doc comment). So this test keeps only the harness-mapping assertion,
/// which is the part of the pin's test that still has a fork home.
#[test]
fn non_ambient_entry_uses_display_harness() {
    ...
    let entry = AgentConversationEntry {
        ...
        display: AgentConversationDisplayData {
            title: "Claude conversation".to_string(),
