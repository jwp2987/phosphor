//! The "Add regex pattern" modal used by the Privacy settings page to append a
//! user-defined secret-redaction pattern to `PrivacySettings::user_secret_regex_list`.
//!
//! Ported from Warp unchanged apart from routing the user-visible strings through
//! `crate::t!` (this fork localizes its UI; Warp hard-codes English).
...
/// Example pattern shown as the regex field's placeholder. Kept as a Rust
/// constant rather than a Fluent message: it is a literal regex (and Fluent
/// would need `{` escaped), and it carries no language-specific content.
const PATTERN_PLACEHOLDER: &str = r"\bAIza[0-9A-Za-z-_]{35}\b";
...
impl AddRegexModal {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let name_editor = ctx.add_typed_action_view(|ctx| {
            ...
            editor.set_placeholder_text(crate::t!("settings-privacy-regex-name-placeholder"), ctx);
            editor
        });
        ...
        let pattern_editor = ctx.add_typed_action_view(|ctx| {
            ...
            let mut editor = EditorView::single_line(options, ctx);
            editor.set_placeholder_text(PATTERN_PLACEHOLDER, ctx);
            editor
        });
...
impl View for AddRegexModal {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let name_label = Text::new(
            crate::t!("settings-privacy-regex-name-label"),
        ...
        let regex_label = Text::new(
            crate::t!("settings-privacy-regex-pattern-label"),
            appearance.ui_font_family(),
            LABEL_FONT_SIZE,
        )
        .with_color(theme.active_ui_text_color().into())
        ...
        let mut add_button = appearance
            ...
            .with_text_label(crate::t!("settings-privacy-add-regex"))
            ...
            .with_child(
                Expanded::new(
                    ...
                    Container::new(if !is_valid_regex && !pattern_text.trim().is_empty() {
                        Text::new(
                            crate::t!("settings-privacy-regex-invalid"),
                            appearance.ui_font_family(),
                            LABEL_FONT_SIZE,
                        )
                        .with_color(
