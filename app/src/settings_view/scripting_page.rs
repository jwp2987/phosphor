//! Settings UI for local scripting and Phosphor control permissions.
//!
//! Ported from Warp's `app/src/settings_view/scripting_page.rs` at the pinned
//! oracle (`02b53fcd8`, Warp `2026.07.29.09.05` stable -- see `ORACLE.md`).
//!
//! This is the only user-reachable path that turns local control on. The
//! authoritative enablement bit lives in [`crate::settings::LocalControlSettings`]
//! (`app/src/settings/local_control.rs`), whose channel default is
//! `Enabled` on dogfood builds and `Disabled` everywhere else precisely
//! because users are expected to opt in here. Without this page the whole
//! `crates/local_control` + `app/src/local_control` stack is unreachable on a
//! public channel.
//!
//! Fork adaptations against the oracle:
//! - `warp_errors::report_if_error` -> `crate::report_if_error` (this fork has
//!   no `warp_errors` crate; the macro lives in `warp_core::errors` and is
//!   re-exported at the crate root).
//! - Product-name prose is de-branded per `docs/DESIGN-PHOSPHOR-FORK.md` §6.
            ...
            move |view, result, ctx| {
                ...
                match result {
                    Ok(()) => {
                        ...
                        let message = format!(
                            "Successfully installed the Phosphor Control CLI! You can now run '{command_name}' from the command line."
                        );
                    ...
                    Err(error) => {
                        let message =
                            format!("Failed to install Phosphor Control command: {error}");
                        log::warn!("{message}");
                        ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
...
impl SettingsWidget for WarpControlCliInstallWidget {
    ...
    fn search_terms(&self) -> &str {
        "phosphor control cli command warpctrl install scripting"
    }
    ...
    fn render(
        ...
        render_body_item::<ScriptingSettingsPageAction>(
            "Phosphor Control CLI command".into(),
            None,
            ...
            button,
            Some(
                "Install the warpctrl command for scripting Phosphor from your terminal."
                    .to_owned(),
            ),
        )
    }
...
impl SettingsWidget for LocalControlModeWidget {
    ...
    fn search_terms(&self) -> &str {
        "scripting phosphor control automation warpctrl local cli scripts disabled enabled"
    }
    ...
    fn render(
        ...
        render_body_item::<ScriptingSettingsPageAction>(
            ...
            Some("warpctrl allows for scripting Phosphor's UI. Use with care.".to_owned()),
        )
    }
}
