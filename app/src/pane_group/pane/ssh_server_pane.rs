//! SSH server editor pane (a central pane, opened via the SSH manager tree).
//!
//! Modeled on the minimal structure of `get_started_pane.rs`, with no cloud
//! sync / sharing. The pane is not persisted (`LeafContents::SshServer { .. }`
//! returns false from `is_persisted()`); business data (host/user/port/...)
//! is driven by `warp_ssh_manager::SshRepository` operating on SQLite.
