...
/// Regression test for warpdotdev/warp#9011: the slow-bootstrap banner used to
/// persist indefinitely when shell integration never sent the bootstrap signal
...
/// Regression test for warpdotdev/warp#9011: when the banner is dismissed by
/// another path (manual user dismissal or a successful bootstrap event), any
/// pending auto-dismiss timer should be aborted so it can't fire after the fact.
#[test]
...
/// As [`open_cli_agent_rich_input_for_agent`], but also returns the window id.
///
/// Keystroke dispatch needs the window, so the Ctrl-G tests cannot use the
/// handle-only form. Mirrors the pin's identical split (#418).
...
// Ported from warp/master `app/src/terminal/view_tests.rs`
// (`focus_reporting_writes_focus_events_in_normal_screen`,
// `copy_forwards_etx_to_pty_on_linux_alt_screen_without_warp_selection`,
// `copy_does_not_forward_when_alt_screen_has_warp_selection`,
// `copy_does_not_forward_on_normal_screen`). Assertions are unchanged.
...
// Ported from the pin's `app/src/terminal/view_tests.rs`
// (`should_right_click_paste_true_only_without_shift_when_setting_enabled`,
// `block_list_right_click_forwards_to_pty_when_long_running_block_owns_mouse`,
// `block_list_shift_right_click_opens_context_menu_when_right_click_pastes`,
// `alt_screen_shift_right_click_opens_context_menu_when_right_click_pastes`,
// `input_shift_right_click_opens_context_menu_when_right_click_pastes`,
// `waterfall_background_right_click_honors_right_click_pastes_setting`),
// upstream `4111d08f9` / commit `c25ac4070`. Assertions are unchanged; the only
// adaptation is `EntityIdSet::default()` -> `HashSet::new()`, because this
// fork's `WindowInvalidation::updated` is still a `std` `HashSet` rather than
// the pin's `FxHashSet` alias.

/// Ported from the pin's `should_right_click_paste_true_only_without_shift_when_setting_enabled`
/// (upstream 4111d08f9).
#[test]
...
///
/// Ported from the pin's helper of the same name (upstream 4111d08f9, commit
/// `9921300b7`).
...
/// Ported from the pin's `ctrl_c_from_shared_viewer_forwards_and_arms_cancel_window`
/// (upstream 4111d08f9). Assertions are unchanged.
#[test]
...
/// Ported from the pin's
/// `ctrl_c_from_shared_viewer_rejected_by_agent_in_control_does_not_arm_cancel_window`
/// (upstream 4111d08f9). Assertions are unchanged.
#[test]
...
fn copy_forwards_etx_to_pty_on_linux_alt_screen_without_warp_selection() {
    App::test((), |mut app| async move {
        ...
        assert_eq!(
            *pty_writes.borrow(),
            vec![vec![escape_sequences::C0::ETX]],
...
// ── Context menu "Clear Blocks" entry, ported from the pinned oracle ────────────────
...
// ── Find bar close path, ported from the pinned oracle ─────────────────────────────

/// Regression test for the async-find branch of warpdotdev/warp#11212.
///
...
// ── Agent view lifecycle, ported from the pinned oracle ────────────────────────────
    ...
    App::test((), |mut app| async move {
        ...
        let (prompt_view_ids, user_shell_index) = terminal.update(&mut app, |view, ctx| {
            ...
            append_inputs_to_conversation_and_handle_event(
                view,
                conversation_id,
                vec![agent_jump_user_query("prompt 1")],
                ctx,
            );
            ...
            append_inputs_to_conversation_and_handle_event(
                view,
                conversation_id,
                vec![agent_jump_user_query("prompt 2")],
                ctx,
            );
            ...
            let navigable_ai: Vec<_> = navigable
                ...
                .filter_map(|item| match item {
                    AgentTranscriptNavigableItem::AiBlock { view_id } => Some(*view_id),
                    AgentTranscriptNavigableItem::ShellBlock(_) => None,
                })
                .collect();
            assert_eq!(
                navigable_ai.len(),
        ...
        terminal.update(&mut app, |view, ctx| {
            ...
            assert_eq!(
                ...
                Some(AgentTranscriptNavigableItem::AiBlock { view_id: prompt_2 })
            );
            assert_eq!(view.selected_blocks.tail(), None);
            ...
            assert_eq!(
                view.agent_transcript_selection_for_test(),
                Some(AgentTranscriptNavigableItem::ShellBlock(user_shell_index))
            );
            ...
            assert_eq!(
                view.agent_transcript_selection_for_test(),
                Some(AgentTranscriptNavigableItem::AiBlock { view_id: prompt_1 })
            );
            assert_eq!(view.selected_blocks.tail(), None);
...
fn agent_transcript_navigation_marks_target_user_query() {
    App::test((), |mut app| async move {
        ...
        terminal.update(&mut app, |view, ctx| {
            ...
            view.select_less_recent_block(false /* is_shift_down */, ctx);
            assert_eq!(view.agent_transcript_navigated_ai_block(ctx), Some(prompt_2));
            ...
            view.select_less_recent_block(false, ctx);
            assert_eq!(view.agent_transcript_navigated_ai_block(ctx), Some(prompt_1));
...
// ---------------------------------------------------------------------------
// Test helpers ported from the pinned oracle's view_tests.rs (#418 group D).
// Shared by the conversation-transfer and Cmd-K tests below.
// ---------------------------------------------------------------------------
...
) -> (AIConversationId, TaskId, AIAgentExchangeId, ResponseStreamId) {
...
fn append_inputs_to_conversation_and_handle_event(
    ...
    view.handle_ai_history_model_event(
        ...
        &BlocklistAIHistoryEvent::AppendedExchange {
            exchange_id,
            task_id,
            // NOTE(adapted): the oracle names this field `terminal_surface_id`.
            terminal_view_id: view.view_id,
            conversation_id,
            is_hidden: false,
...
// ---------------------------------------------------------------------------
// Ctrl-G keystroke dispatch through the responder chain (#418).
//
// The fork already covers `TerminalAction::OpenCLIAgentRichInput` invoked
// directly; these dispatch a real `ctrl-g` keystroke, which is what actually
// exercises the three-way keymap predicate in `terminal/view/init.rs`.
// ---------------------------------------------------------------------------
...
// --- Cmd-Enter agent view entry (#418) ---
...
fn cmd_k_in_agent_view_cancels_in_progress_conversation_and_starts_new_one() {
    App::test((), |mut app| async move {
        ...
        // Cancelling the in-progress conversation and starting a new one persists
        // conversation history, which needs GlobalResourceHandlesProvider registered.
        let global_resource_handles = crate::GlobalResourceHandles::mock(&mut app);
        app.add_singleton_model(|_| {
            crate::GlobalResourceHandlesProvider::new(global_resource_handles)
        });
...
fn cmd_k_does_not_clear_buffer_when_agent_is_driving_command() {
    App::test((), |mut app| async move {
        ...
        terminal.update(&mut app, |view, ctx| {
            ...
            let conversation_id =
                BlocklistAIHistoryModel::handle(ctx).update(ctx, |history, ctx| {
                    // The pin passes a 4th `is_cli_agent_transcript` bool here (tied to
                    // #423's TranscriptScope work), which this fork's `start_new_conversation`
                    // does not yet have. This test never inspects transcript scope, so the
                    // fork's existing 4-arg call (equivalent to `is_cli_agent_transcript:
                    // false`) is behaviorally identical for this assertion. See the #418
                    // report for the arity-drift finding in full.
                    history.start_new_conversation(view.view_id, false, false, ctx)
                });
...
// --- Conversation transfer between terminal surfaces (#418) ---
        ...
        original_view.update(&mut app, |view, ctx| {
            view.handle_ai_history_model_event(
                ...
                &BlocklistAIHistoryEvent::AppendedExchange {
                    exchange_id,
                    task_id: task_id.clone(),
                    terminal_view_id: original_view_id,
                    conversation_id,
                    is_hidden: false,
                    response_stream_id: Some(response_stream_id.clone()),
                },
    ...
    App::test((), |mut app| async move {
        ...
        BlocklistAIHistoryModel::handle(&app).read(&app, |history, _| {
            let original_view_live_conversation_ids = history
                .all_live_conversations_for_terminal_view(original_view_id)
...
fn updated_conversation_metadata_refreshes_selected_conversation_pane_title() {
    App::test((), |mut app| async move {
        ...
        // Restoring conversations and applying a title update persist conversation
        // history, which needs GlobalResourceHandlesProvider registered.
        let global_resource_handles = crate::GlobalResourceHandles::mock(&mut app);
        app.add_singleton_model(|_| {
            crate::GlobalResourceHandlesProvider::new(global_resource_handles)
        });
...
// --- #399: restored now that CLIAgentEventSource / received_rich_notification exist ---
...
// --- #418: restored now that #423's TranscriptScope + BlockList::transcript_scope exist ---

#[test]
fn cmd_enter_from_terminal_with_selected_block_enters_agent_view_with_context() {
    App::test((), |mut app| async move {
        initialize_app_for_terminal_view(&mut app);
        // Starting a new conversation persists conversation history, which needs
        // GlobalResourceHandlesProvider registered (same idiom as elsewhere in this file).
        let global_resource_handles = crate::GlobalResourceHandles::mock(&mut app);
        app.add_singleton_model(|_| crate::GlobalResourceHandlesProvider::new(global_resource_handles));
        app.add_singleton_model(ImportedConfigModel::new);
        app.update(|ctx| {
...
//
// Ported from the pinned oracle's `app/src/terminal/view_tests.rs`
// (`back_button_label_names_the_direct_parent_at_depth`,
// `back_button_label_resolves_token_only_parent_linkage`). The fork's test
// file is `view_test.rs`, and `start_new_conversation` takes one fewer flag
// here; the labels come from fluent (`terminal-agent-header-*`), which
// `initialize_history_persistence_for_tests` pins to English.
// ---------------------------------------------------------------------------
