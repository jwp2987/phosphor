...
// NOTE(adapted): fork renamed these `_terminal_surface` APIs to `_terminal_view`.
use super::{prompt_history_for_terminal_view, UpArrowHistoryConfig};
...
fn build_history_model(prompts: Vec<String>) -> BlocklistAIHistoryModel {
    ...
    let persisted_queries = prompts
        ...
        .collect();
    // NOTE(adapted): fork's `BlocklistAIHistoryModel::new` dropped the middle
...
fn combined_history(
    ...
    History::handle(app)
        .as_ref(app)
        // NOTE(adapted): renamed from Warp's `up_arrow_suggestions_for_terminal_surface`.
        ...
        .into_iter()
        .map(|suggestion| {
            // NOTE(adapted): fork's `HistoryInputSuggestion` has no `normalized_text()`
            // helper (Warp's is simply `self.text().trim()`); inlined here.
