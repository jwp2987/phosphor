...
/// environment (`$HOME`/`$USER`) or built-in shell defaults. Resolution failure
/// must never abort the process — an unresolvable uid is a degraded session,
/// not a crash.
...
/// static/musl binary cannot do in-process.
