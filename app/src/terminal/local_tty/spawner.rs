...
fn invoke_without_crash_reporting<T>(
    ...
    // Suspend the local crash reporting state before spawning the child process, to
    // avoid leaking signal/exception handling state into the shell.
    #[cfg(feature = "crash_reporting")]
    crate::crash_reporting::suspend_crash_reporting_for_child_spawn();

    let retval = func();

    // Restore the local crash reporting state after the child process has spawned.
