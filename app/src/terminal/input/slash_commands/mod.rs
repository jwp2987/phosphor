...
use crate::ai::blocklist::{
    BlocklistAIHistoryModel, QueuedQuery, QueuedQueryId, QueuedQueryModel, QueuedQueryOrigin,
    SlashCommandRequest,
};
...
impl Input {
    pub(super) fn select_slash_command(
        ...
        if command.argument.as_ref().is_none() {
            self.execute_slash_command(
                ...
                /*queued_conversation_id*/ None, /*queued_query_id*/ None, ctx,
            );
        } else if command
            .argument
            .as_ref()
    ...
    /// the agent was busy. `queued_conversation_id` and `queued_query_id` then identify the
    /// queued row being fired, so commands that need to act on that row (rather than on the
    /// currently-selected conversation) can find it. Both are `None` for a direct send.
    ///
    ...
    pub(super) fn execute_slash_command(
        ...
        match command.name {
            ...
            set_tab_color if command.name == commands::SET_TAB_COLOR.name => {
                // Ported from the pinned oracle's `SlashCommandKind::SetTabColor` handler.
                // Reuses the already-shipped, already-tested `WorkspaceAction::SetActiveTabColor`
                // (see `app/src/workspace/view_test.rs`) -- only this dispatch arm was missing.
            ...
            compact_and if command.name == commands::COMPACT_AND.name => {
                // A fired queued row carries its own conversation; a direct send uses the
                // current selection.
                ...
                if is_queued_prompt {
                    ...
                    // When invoked from the menu with no argument, `argument` is Some("");
                    // normalize it to None to avoid triggering an empty user query (and wasting
                    // a token) after summarizing.
            ...
            compact if command.name == commands::COMPACT.name => {
                // Zap: `/compact` and `/compact-and` share the local conversation-compaction
                // pipeline — dispatch `WorkspaceAction::SummarizeAIConversation`, with
                // initial_prompt: None meaning "don't send a follow-up prompt after
                // compacting," just silently summarizing into the conversation.
                // A custom instruction (`/compact <instruction>`) goes into the prompt field;
                // on the BYOP build_chat_request path it gets appended after
                // SUMMARY_TEMPLATE as plugin_context.
                //
                // On the non-BYOP path, `SummarizeConversation { prompt }` still goes through
                // the server protobuf (`api::request::input::SummarizeConversation`), with the
                // server handling the summary — the previous prefix-injection semantics are
                // now fully replaced by SummarizeConversation.
                if self
                    .ai_context_model
                    .as_ref(ctx)
                    .selected_conversation_id(ctx)
                ...
                // When invoked from the menu with no argument, `argument` is Some(""); normalize
                // it to None to avoid an extra empty-filter branch when chat_stream assembles
                // the summary prompt.
    ...
    /// conversation.
    ///
    /// A queued `/compact-and` is two turns, not one: the summary runs now, and the follow-up
    /// prompt has to wait for it. Re-queueing the follow-up as a
    /// [`QueuedQueryOrigin::CompactAndSlashCommand`] row on `conversation_id` is what makes the
    /// normal drain path fire it once the summary finishes. The row's attachments are carried
    /// over so the follow-up keeps the context the user attached when they queued it.
    pub(super) fn execute_queued_compact_and(
        ...
        // Read the attachments before sending: the caller removes the fired row afterwards.
