...
//!
//! Zap note: upstream Warp forwarded the attached diagnostic through a feature-specific telemetry
//! event. Telemetry sending has been physically removed from this fork, so the emission path is
//! dropped; the state machine, its diagnostics, and their rate limiting are otherwise intact. See
//! `telemetry.rs` for details.
