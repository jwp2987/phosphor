// Ported from Warp's `app/src/terminal/model/lifecycle/mod_tests.rs`.
//
// One upstream test, `lifecycle_telemetry_payload_is_allowlisted_and_non_ugc`, is intentionally
// omitted. It exercised `LifecycleTelemetryEvent::payload()` / `contains_ugc()` on the upstream
// `TelemetryEvent` implementation, which asserts the emitted telemetry payload's field allowlist.
// Zap has physically removed telemetry sending and dropped that emission type entirely (see
// `telemetry.rs`), so the test targets code that no longer exists. Every non-telemetry assertion,
// including the diagnostic rate limiter, is preserved below unchanged.
