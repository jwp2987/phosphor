...
//! contents, working directories, or other UGC.
//!
//! Zap has physically removed telemetry sending (see the compatibility shims in
//! `warp_core::telemetry`). Upstream Warp emitted these records through an internal terminal event
//! that `ModelEventDispatcher` forwarded to a feature-specific `TelemetryEvent`. That emission path
//! and its `TelemetryEvent`/`TelemetryEventDesc` implementation have been dropped here; no event is
//! ever sent. The record type and its per-transition rate limiter are retained unchanged because
//! they are part of the lifecycle state machine: [`super::BlockLifecycleCoordinator::plan`] attaches
//! a rate-limited [`LifecycleRecoveryRecord`] to conservative or corrective transitions, and callers
//! (and tests) observe whether a record is present. Records are logged, not transmitted.
...
///
/// The first occurrence is surfaced immediately. A later aggregate includes the number of
...
impl LifecycleTelemetryLimiter {
    /// Records an occurrence and returns it only when it should be surfaced immediately.
    pub(super) fn record(
        &mut self,
