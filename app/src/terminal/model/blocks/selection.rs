...
impl BlockList {
    ...
    /// paths find the selected text via `rich_content_blocks_in_selection`.
    ...
    fn rich_content_blocks_in_selection(&self) -> Vec<EntityId> {
        let Some(original_selection) = self.selection.as_ref() else {
            ...
            // their selected text can still be copied -- returning an empty vec
            // here is why copying from inside an AI block yielded "".
