...
fn test_secret_redacted_after_multibyte_prefix() {
    ...
    grid_handler.input_at_cursor(&format!("\u{1f4a3} {secret}"));
    grid_handler.on_finish_byte_processing(&ansi::ProcessorInput::new(&[]));

    // The bomb emoji is a single char; input_at_cursor advances the cursor by one column per
    ...
    assert_eq!(
        ...
        format!("\u{1f4a3} {}", "*".repeat(secret.len()))
    );
}
...
fn test_secret_with_word_boundaries_redacted_after_multibyte_prefix() {
    ...
    grid_handler.input_at_cursor("\u{1f4a3} TOKEN123");
    grid_handler.on_finish_byte_processing(&ansi::ProcessorInput::new(&[]));
