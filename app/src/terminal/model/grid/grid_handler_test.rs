...
fn test_find_url_with_delimiter() {
    ...
    // Fullwidth comma (U+FF0C), a CJK punctuation mark, must also terminate
    // the URL even with no ASCII whitespace before it.
    let blockgrid = mock_blockgrid("https://google.com，");
    assert_eq!(
        blockgrid
            .grid_handler
...
fn test_full_grid_clear_drops_active_hyperlink() {
    let _flag = crate::features::FeatureFlag::OscHyperlinks.override_enabled(true);
    let mut grid = GridHandler::new_for_test(3, 20);
...
// Ported from warp/master `app/src/terminal/model/grid/grid_handler_tests.rs`
// (`test_possible_file_paths_across_wrapped_lines`,
// `test_possible_file_paths_across_multiple_wrapped_lines`,
// `test_possible_file_paths_across_wide_wrapped_line`,
// `test_possible_file_paths_across_wide_multiple_wrapped_lines`).
