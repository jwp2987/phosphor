...
// `app/src/terminal/model/blocks_tests.rs`. Assertions unchanged except that
// `TranscriptScope::Terminal` is adapted to `&AgentViewState::Inactive`, this
// fork's equivalent for the terminal scope (#423 tracks adopting
// `TranscriptScope` itself; revert this adaptation when it lands). See #415: a
// completed iTerm2/Kitty image must render into the active block during
// `ScriptExecution` (shell-startup output), and must route into a background
// block -- created only when the image is actually visible -- when it arrives
// as early output between blocks.
