...
pub struct BlockList {
    ...
    /// Pressing Ctrl+L right after an SSH subshell-bootstrap completes, before the
    /// active block's prompt has arrived, triggers a bug where height=0 blocks pile
    /// up in the sum tree and the gap can't shrink. When in this state, a
    /// `clear_visible_screen` call is recorded as pending, and only actually
    /// executed on the next `precmd` (meaning the remote prompt has been redrawn).
...
impl BlockList {
    ...
    pub fn clear_visible_screen(&mut self) {
        // Background: pressing Ctrl+L at the instant an SSH subshell-bootstrap has
        // just completed, before the active block has drawn its prompt (`height=0`
        // + `state=BeforeExecution` + not started), causes the gap to be inserted
        // right before/after that zero-height active block. The remote then
        // dispatches in-band generator commands at high frequency, each one
        // triggering `command_finished` → `create_new_block`, pushing out a new
        // active block that is itself "just created, height=0, prompt not yet
        // arrived." The result is a run of height=0 hidden/empty blocks piling up
        // at the end of the sum tree, with the same full-screen-height gap still
        // sitting before them, and the user's actual input block pushed far behind
        // the gap. But the gap can only shrink based on the increase in "total
        // height after the gap," and those zero-height blocks contribute no
        // increase, so the gap can never shrink — leaving the user's input command
        // block permanently off-screen.
        //
        // Fix: if the active block isn't ready yet and there's still an in-band
        // command in flight at this moment (height=0 + BeforeExecution + not itself
        // in_band + in_flight_in_band_command_count > 0), record it as pending.
        // Only actually clear the screen once the next `precmd` arrives (meaning
        // the remote prompt has been redrawn and the active block has real
        // content). In the normal local case (no in-band command in flight), this
        // branch isn't taken, so local Ctrl+L isn't incorrectly intercepted.
    ...
    /// Whether the active block is in the "just created, prompt not yet arrived"
    /// state. In this state, `clear_visible_screen` should not be allowed to
    /// proceed, or it will leave a run of height=0 blocks in the sum tree.
    ///
    /// Note: the defer path is only taken while an in-band command is still in
    /// flight (typically a generator dispatched at high frequency during an SSH
    /// subshell-bootstrap). Otherwise `height=0 + BeforeExecution + !in_band` is
    /// actually the **default state** of a local shell's empty prompt waiting for
    /// user input, and would incorrectly intercept every local Ctrl+L, setting it
    /// to pending and never getting a precmd to fall back on — manifesting as
    /// local screen-clear becoming broken (the SSH remote side works fine, since
    /// its active block has already drawn the remote prompt, so height > 0 bypasses
    /// this check directly).
    fn should_defer_clear_for_unprepared_active_block(&self) -> bool {
        ...
        // height=0 means the prompt hasn't been drawn yet; BeforeExecution means
    ...
    fn restore_block(
        ...
        if block.did_execute {
            ...
            self.preexec(PreexecValue { command, session_id: None });
        }
