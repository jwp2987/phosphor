...
use crate::ai::blocklist::{
    ai_indicator_height, BlocklistAIActionModel, QueuedQuery, QueuedQueryId, QueuedQueryModel,
...
use super::{
    ...
    should_right_click_paste, History, HistoryEntry, SizeInfo, TerminalModel,
...
impl Input {
            ...
            ctx.add_typed_action_view(|ctx| {
                let options = EditorOptions {
                    ...
                    keymap_context_modifier: Some(Box::new(move |context, app| {
                        ...
                        // While the CLI agent rich input is open, Ctrl+Enter belongs to the
                        // rich input composer, so the "enter agent view" binding must not
                        // claim it as well.
    ...
    fn handle_input_settings_event(
        ...
        match event {
            ...
            InputSettingsChangedEvent::EnableAiCommandSearchHashTrigger { .. } => {
                // Refresh the zero-state hint so the "Type '#' ..." advertisement appears
                // and disappears with the setting.
                self.set_zero_state_hint_text(ctx);
                ctx.notify();
            }
    ...
    fn update_tab_completion_menu(
        ...
        //
        // The fork previously exempted classic completions unconditionally, with a
        // TODO acknowledging exactly that stale-result bug. Restored from the pin.
            ...
            Some(results) => {
                ...
                let decision = if completions_trigger == CompletionsTrigger::Keybinding {
                    ...
                    ExplicitTabCompletion::Open {
                        suggestions: results.prepare_for_query(query, self.path_separators(ctx).all),
    ...
    fn predict_am_query(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let _ = (processed_input, processed_output); // Used by the legacy ServerApi path; BYOP one-shot only takes the last_block summary
    ...
    fn maybe_queue_input_for_in_progress_conversation(
        ...
        // accepting rows so FIFO order is preserved.
