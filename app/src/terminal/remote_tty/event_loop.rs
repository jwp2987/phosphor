...
impl EventLoop {
    ...
    ///
    /// Registers a freshly generated session ID with `terminal_model` before writing the init
    /// script, so the shell's own session ID is already recognized by the time it can write
    /// anything back.
