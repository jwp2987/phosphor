...
impl View for SshInstallTmuxBlock {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let explanation = if self.outdated_version {
            "In order to Phosphorize your SSH session, a more recent version of tmux (>=3.0) must be installed. "
        } else {
            "In order to Phosphorize your SSH session, tmux must be installed. "
        };
