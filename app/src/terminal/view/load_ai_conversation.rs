...
/// Controls whether restoring a conversation also enters Agent View for it (#418).
...
impl TerminalView {
        ...
        F: FnOnce(&mut Self, &mut ViewContext<Self>) + 'static,
    {
        ...
        match restore_context_state {
            RestorationDirState::NeedsCd { path } => {
                ...
                // here as the same call. ; verified
                // by reading only.
    ...
    fn restore_conversations_from_block_params(
        ...
        // Restored command blocks are inserted into the block list without going through the
    ...
    pub fn restore_conversation_after_view_creation(
        ...
        // Insert command blocks into the existing blocklist first, mirroring the quit/restart
        // path where blocks are passed to the `TerminalModel` constructor. This ensures
        // `command_block_indices_for_exchanges` finds them and AI blocks get
        // `command_block_index = Some(..)`, so `process_restored_outputs` doesn't need to
        // create blocks itself.
        //
    ...
    pub fn create_and_insert_ai_block(
        ...
        // Process restored outputs: handle code diffs.
...
///
/// For each exchange, finds the first command block whose timestamp is at or after the
/// exchange's start time. The AI block should be inserted before that command block.
///
...
fn command_block_indices_for_exchanges<'a>(
    ...
    // Deliberately shares the TUI's implementation rather than keeping a GUI copy.
    // The two surfaces MUST break a tied timestamp on the same side or a conversation
    // restores in a different order depending on which one you are in (#174). A local
    // duplicate here used `>=` -- the pin's rule, which this fork declined -- and that
    // regression is exactly what `script/check_declined_collisions` caught.
    crate::terminal::conversation_restoration::find_block_indices_for_exchange_timestamps(
        &command_blocks,
        &exchange_timestamps,
    )
}
