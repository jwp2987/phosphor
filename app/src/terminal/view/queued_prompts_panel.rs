...
fn build_row_state(
    ...
    // Retained for signature parity with upstream; the origin only gated the removed
    // cloud-mode row affordances, so it is unused here.
    _origin: QueuedQueryOrigin,
    text: &str,
    ...
    // "wait for the agent" message while send-now is disabled; "Send now" is the default.
...
impl QueuedPromptsPanelView {
    ...
    pub(crate) fn commit_edit(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // Upstream reports a `QueuedPromptEdited` cloud-telemetry event here; Zap does not send
        // Warp cloud telemetry, so it is omitted (no user-facing behavior change).
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            QueuedPromptsPanelAction::ToggleCollapsed => {
                ...
                // Upstream reports a `QueuedPromptPanelCollapseToggled` cloud-telemetry event
                // here; Zap does not send Warp cloud telemetry, so it is omitted.
                ctx.notify();
            }
            ...
            QueuedPromptsPanelAction::DeleteRow(query_id) => {
                ...
                if removed.is_some() {
                    // Upstream reports a `QueuedPromptDeleted` cloud-telemetry event here; Zap
                    // does not send Warp cloud telemetry, so it is omitted.
            ...
            QueuedPromptsPanelAction::DropEnd => {
                if self.dragging_query_id.take().is_none() {
                    return;
                }
                let _ = self.drag_start_index.take();
                // Upstream reports a `QueuedPromptReordered` cloud-telemetry event here (with the
                // from/to indices and row origin); Zap does not send Warp cloud telemetry, so the
                // reorder itself already happened live in `DragMoved` and nothing else is needed.
                ctx.notify();
            }
        }
    }
