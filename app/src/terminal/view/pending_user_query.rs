...
impl TerminalView {
    ...
    /// Inserts a pending user query block for a local ambient agent run, until the
    /// harness CLI starts. This block only shows the user prompt and queued state;
    /// it doesn't provide a local queue-callback button.
