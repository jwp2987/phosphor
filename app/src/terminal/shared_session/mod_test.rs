...
// A shared-session viewer receives the terminal byte stream directly (not
// through the sharer's `precmd_with_completion_metadata` call), so recovery
// of a missed `CommandFinished` must also work when the `Precmd` DCS hook
// arrives as raw bytes, with no ordered-event hint channel wired up at all.
#[test]
...
/// Ported from warp/master `app/src/terminal/shared_session/mod_tests.rs`
/// (`test_loading_scrollback_with_completed_last_block_creates_active_block`).
/// Shape adaptation only: the fork's `Block::height` takes an `AgentViewState`
/// where Warp's takes a `TranscriptScope`. Assertions unchanged.
#[test]
