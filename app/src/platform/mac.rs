...
///
/// The bundled CLI wrapper `exec`s the GUI executable from inside the app
/// bundle, so Launch Services binds the process to a bundle whose `Info.plist`
/// makes it a dockable foreground app. This applies at runtime what the
/// standalone CLI binary gets from the `LSBackgroundOnly` key in its own
/// `Info.plist`.
///
