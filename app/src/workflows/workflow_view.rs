...
const RUN_ON_DESKTOP_BUTTON_TEXT: &str = "Run in Phosphor";
    ...
    pub fn load(
        ...
        if let ContainerConfiguration::Pane(pane_config) = &mut self.container_configuration {
            pane_config.update(ctx, |pane_config, ctx| {
                ...
                // TODO(zap-cloud-removal Phase 5): the sharing UI has been
                // retired, and the workflow ShareableObject injection
                // removed; workflow.id is still a cloud_object id — clean
                // this up together when cloud_object is retired in Phase 5.
                let _ = workflow.id;
            });
        }
        ...
        // TODO(zap-cloud-removal Phase 5): workflow invitee_email likewise
        // has no UI exit point, but settings is passed in from the layer
        // above; clean up the field once it's fully retired.
        let _ = settings;
    ...
    fn render_edit_toggle_button(
        ...
        if let Some((mode_text, mut edit_button)) = text_and_button {
            if matches!(editability, ContentEditability::RequiresLogin) {
                ...
                // openWarp UX: consistent with the notebook details_bar, add
                // a tooltip to the workflow edit toggle button, so the user
                // isn't left not knowing this pencil is clickable or what
                // happens when clicked. The tooltip describes "the action that will happen on click".
        ...
        if let Some((label, icon)) = label_and_icon {
                ...
                && !self.is_for_agent_mode
            {
                ...
                let button_with_tool_tip = appearance.ui_builder().tool_tip_on_element(
                    "Generate a title, descriptions, or parameters with Phosphor AI".to_string(),
