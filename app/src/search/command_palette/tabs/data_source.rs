...
impl SyncDataSource for DataSource {
    ...
    fn run_query(
        ...
        // `enumerate()` before filtering on purpose: the index is the tab's
        // rank in the full MRU order, and `SearchItem::score` turns it into a
        // descending score. Ranking after the filter would renumber from zero
        // and lose the ordering between the surviving tabs.
        let results = self
            .tabs
            .iter()
            .enumerate()
