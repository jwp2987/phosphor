...
impl SearchItemTrait for SearchItem {
    ...
    /// Recency, not relevance: rank 0 (most recent) scores highest and each
    /// step down the MRU list scores one less. The mixer sorts descending, so
    /// this reproduces the MRU order exactly.
