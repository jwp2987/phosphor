...
/// Zap Wave 7-3: the URI handler / Settings UI have been removed; this only
/// serves as a stopgap for `update_environment_form` before the agent UI's
/// major-surgery commit lands (the latter doesn't use it to build outgoing URLs).
    ...
    fn handle(&self, primary_window_id: Option<WindowId>, url: &Url, ctx: &mut AppContext) {
        ...
        match self {
            ...
            UriHost::Settings => {
                ...
                // zap://settings?widget=<slug> opens settings scrolled to a
                // specific widget. This takes precedence over sub-page routing.
                if let Some((page, widget_id)) = query_string
                    .get("widget")
                    .and_then(|slug| settings_widget_deeplink_target(slug))
                {
                    open_settings_scrolled_to_widget(primary_window_id, page, widget_id, ctx);
                    return;
                }
                ...
                if let Some(settings_sub_page) = settings_sub_page {
                    match settings_sub_page.as_str() {
                        "environments" => {
                            // Zap Wave 7-3: the warp://settings/environments URI
                            // handler was physically removed along with the
                            // ambient-agent UI subsystem. The GitHub auth
                            // completion notification is still kept — other
                            // independent components may need to listen for it.
                        ...
                        // Zap Wave 3-1: the "platform" URI route originally
                        // pointed to `SettingsSection::OzCloudAPIKeys` (the
                        // cloud API key management page), and was physically
                        // removed along with the UI. The arm is kept to
...
/// Handles `zap://tab_config/<name>` deeplinks.
///
/// Resolution rules:
/// - `<name>` is matched case-insensitively against each tab config's file
///   stem, so both `zap://tab_config/my_tab` and
///   `zap://tab_config/my_tab.toml` work.
/// - When `?new_window=true` (or no window is open) the tab config opens in a
///   brand-new window. Otherwise it opens as a new tab in the active window.
...
/// Opens a settings tab scrolled to a specific widget, resolved from a
/// `zap://settings?widget=<slug>` deeplink. Reuses the active window's
/// workspace when available, otherwise opens a new window.
fn open_settings_scrolled_to_widget(
    primary_window_id: Option<WindowId>,
    ...
    let Some(workspace) = WorkspaceRegistry::as_ref(ctx).get(window_id, ctx) else {
        log::warn!("no workspace available for settings widget deeplink");
        return;
    };
