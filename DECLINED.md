...
| **Agent-invoked agent spawning (`AIAgentActionType::RunAgents`)** | #325 | **DEFERRED 2026-08-09 (maintainer) — deferred, not declined.** Phosphor ships the USER-invoked route instead: `/orchestrate` (`ad06c852c`), a GUI-only slash command spawning one local child per `;`-separated task, wired into the #304 pill bar and transcript rendering. What is deferred is letting the MODEL decide to spawn: the pin's `RunAgentsRequest` is cloud-typed so there is nothing to port, and `AIAgentActionType` has no spawn variant here while **60 files** match that enum — a change of that size was judged too risky beside in-flight branches. Also absent and deferred with it: `StartAgentExecutionMode`, `RunAgentsExecutionMode`, `RunAgentsAgentRunConfig`. **`AIAgentActionType::SendMessageToAgent` is part of the same gap** and has already blocked three separate pieces (`render_send_message` in Step 1b, a `blocklist::telemetry` variant in Step 2, `default_collapsible_state_for_orchestration_action` in Step 4) — worth considering on its own, separately from the full 60-file change. Revisit once the tree is quiet; the `/orchestrate` implementation reported that nothing pushed it toward needing the enum, so the user-invoked route stands on its own. |
| **`SendMessageToAgent` executor (the cloud half)** | #325 | **PARTIALLY LANDED 2026-08-09 — the variant exists, the executor does not.** `AIAgentActionType::SendMessageToAgent` and its result type were added on their own (separately from the deferred `RunAgents` family) because their absence had blocked three pieces of shipped work: `render_send_message` in tier-3.5 Step 1b, a `blocklist::telemetry` variant in Step 2, and `default_collapsible_state_for_orchestration_action` in Step 4. All three are now reconnected. **What is NOT ported is the executor**: the pin's `app/src/ai/blocklist/action_model/execute/send_message.rs` depends on `crate::server::server_api::ServerApiProvider` and `SendAgentMessageRequest`/`SendAgentMessageResponse` — cloud. So the variant is declared, rendered, labeled, persisted and telemetry-typed, but **inert**: nothing constructs it from a real model tool call (`convert_from.rs`'s proto `Tool::SendMessageToAgent` still hits its catch-all) and `execute.rs` treats it as non-autoexecuting and resolves it to `Cancelled`. A local BYOP equivalent — agent-to-agent messaging over the existing local child-process channel rather than Warp's servers — would be new design, not a port. Recorded so nobody mistakes the present state for a wiring bug. |
