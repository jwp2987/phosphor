...
pub fn convert_wsl_to_windows_host_path(
    ...
    match prefix.as_slice() {
            ...
            if bytes.len() == 1 && bytes[0].is_ascii_alphabetic() =>
        {
            ...
            // Many directories are symlinks into the underlying file-system location in Windows.
            // Only the drive-letter arm resolves them: those paths live on the Windows
            // filesystem, so `read_link` is a local call. Doing the same on the `\\WSL$\...`
            // UNC arm below would force a 9p round-trip into the distribution on every
            // conversion, which freezes the UI while WSL tabs are open.
            match std::fs::read_link(&pathbuf) {
                Ok(linked_file) => Ok(linked_file),
                Err(_) => Ok(pathbuf),
            }
