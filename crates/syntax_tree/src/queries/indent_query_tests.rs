...
use languages::{language_by_local_filename, Language};
...
fn test_indent_query() {
    App::test((), |mut app| async move {
        let language = language_by_local_filename(std::path::Path::new("test.rs"))
...
fn test_indent_query_on_go() {
    App::test((), |mut app| async move {
        let language = language_by_local_filename(std::path::Path::new("test.go"))
...
fn test_indent_query_on_python() {
    let language = language_by_local_filename(std::path::Path::new("test.py"))
...
fn test_indent_query_on_javascript() {
    let language = language_by_local_filename(std::path::Path::new("test.js"))
...
fn test_indent_query_on_typescript() {
    let language = language_by_local_filename(std::path::Path::new("test.ts"))
