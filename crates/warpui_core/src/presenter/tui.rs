...
pub struct TuiPresenter {
    ...
    ///
    /// This is what lets a driver tell "focused but off-screen" from "focused
    /// and rendered": a view that is not in here painted nothing this frame, so
    /// keystrokes routed to it go nowhere the user can see.
...
impl TuiPresenter {
    ...
    pub fn present_element(
        ...
        // No view ancestry is recorded for a raw element tree, so nothing can
        // be claimed to have been presented.
        self.presented_views.clear();
        let mut layout_ctx = TuiLayoutContext {
            rendered_views: &mut self.rendered_views,
        };
