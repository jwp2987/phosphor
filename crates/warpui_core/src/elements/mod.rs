...
pub trait Element {
    ...
    /// Returns whether this element is of Overlay type.
    ///
    /// Stack checks this method when painting child elements: if it returns
    /// true, `start_overlay_layer` is used instead of `start_layer`, giving
    /// the child element an Overlay z-index, which ensures that during event
    /// dispatch `is_covered` doesn't mistake its own child element's hit rect
    /// for an occlusion.
