...
use super::{Point, SelectableElement};
use crate::{
...
///
/// Wrap any child element in `LiveElement` to ensure the view repaints on a timer, which is
/// useful for content that changes over time (e.g. an elapsed duration counter). The repaint
/// cycle is self-sustaining: each `paint` call schedules the next repaint.
