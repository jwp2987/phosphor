...
fn test_transfer_view_tree_reconciles_views_known_only_to_target_presenter() {
    ...
    // its subtree from the *source* window's graphs, so a render-time
    ...
    // it previously lived in, that window's `view_parents` map still
    // references the child, so `transfer_view_tree_to_window` must re-walk
    // from the target and reconcile the straggler instead of stranding it
    // (which previously desynced `view_to_window` and triggered "circular
    // view reference").
    //
    // `record_view_parent` is `#[cfg(feature = "tui")]`-gated (it is the TUI
    // render path's ancestry-tracking hook), so this test is gated the same
    // way.
    #[derive(Default)]
    struct TestView;
    ...
    App::test((), |mut app| async move {
        ...
        // 2's `view_parents` map: not a structural child, not exposed via
        ...
        assert!(
            ...
            "child reachable only via the target's view_parents map should be reconciled"
        );
