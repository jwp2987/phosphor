...
// `miss_cache` is private to the `legacy` module, so it's reached relatively rather than
// through the `crate::signatures` re-export.
use super::super::miss_cache::MAX_CACHED_MISSES;
...
#[test]
fn test_distinct_misses_do_not_grow_the_caches_without_bound() {
    // The pre-fix cache retained an `Option<Signature>` entry for every distinct token ever
    // looked up, so a long session typing distinct tokens grew it forever. Both caches must
    // now stay bounded no matter how many distinct normal-length misses are seen.
    let registry = create_test_command_registry([test_signature()]);
    let positive_len_before = registry.signatures.signatures.len();
    ...
    assert_eq!(
        registry.signatures.signatures.len(),
        positive_len_before,
        "misses must never reach the append-only positive cache"
    );
    assert!(
        registry.signatures.misses.len() <= MAX_CACHED_MISSES,
        "the negative cache must stay bounded across many distinct misses, found {}",
        registry.signatures.misses.len()
    );
}
...
fn test_registered_signature_longer_than_the_cap_is_unresolvable() {
    ...
    assert_eq!(
        registry.signatures.signatures.len(),
        0,
        "an over-cap name must not be stored either: `get` rejects it on length, so storing it \
         would both grow the append-only map and make `registered_commands()` list a name \
         `signature()` can never return"
    );
    assert!(registry.registered_commands().next().is_none());
}

#[test]
fn test_oversized_registrations_do_not_grow_the_positive_cache() {
    // `register_signature` is reachable at runtime over plugin IPC -- see
    // `app/src/plugin/app/service_impl/completions.rs`, which registers every signature in a
    // `RegisterCommandSignatureRequest`. The positive cache is append-only, so without a cap
    // on the insert path a plugin sending long names grows it without bound; the cap on the
    // lookup path alone does not close that. The companion test above,
    // `test_all_known_signature_names_are_within_the_length_cap`, is what establishes that this
    // cap never drops a signature we actually ship.
    let oversized = (0..500)
        .map(|i| signature_with_name(&format!("{}{i}", "a".repeat(MAX_CACHEABLE_COMMAND_LEN))))
        .collect::<Vec<_>>();
    let registry = create_test_command_registry(oversized);

    assert_eq!(registry.signatures.signatures.len(), 0);
    assert_eq!(registry.signatures.misses.len(), 0);

    // A name exactly at the cap is still registrable -- the guard is `>`, not `>=`.
    let at_cap = "b".repeat(MAX_CACHEABLE_COMMAND_LEN);
    let registry = create_test_command_registry([signature_with_name(&at_cap)]);
    assert_eq!(
        registry.signature(&at_cap).map(|s| s.name.as_str()),
        Some(at_cap.as_str())
    );
}
