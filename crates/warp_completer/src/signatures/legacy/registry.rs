...
impl SignatureCache {
    ...
    fn get(&self, command: &str) -> Option<&Signature> {
        ...
        if command.len() > MAX_CACHEABLE_COMMAND_LEN {
            ...
            // to the negative cache. Rejecting before the allocation is the point: a pasted
            // multi-megabyte blob must not be copied at all.
            //
            // Note this measures the token as typed, not the lowercased key that gets stored.
            // `str::to_lowercase` can grow a string (U+0130 lowercases to two code points), so
            // the stored key is bounded by 3x MAX_CACHEABLE_COMMAND_LEN bytes, not by
            // MAX_CACHEABLE_COMMAND_LEN. Still a constant; just not this constant.
            return None;
        }
    ...
    ///
    /// Names longer than `MAX_CACHEABLE_COMMAND_LEN` are dropped, applying the same cap `get`
    /// applies and measuring it the same way (on the name as given, not the lowercased key).
    /// This is not merely symmetry: `register_signature` is reachable at runtime from plugins
    /// over IPC (`app/src/plugin/app/service_impl/completions.rs` registers every signature in
    /// a `RegisterCommandSignatureRequest`), so without a cap here the append-only map is still
    /// growable without bound by a caller we do not control. It also removes an inconsistency
    /// -- an over-cap name used to be stored, so `registered_commands()` listed a name that
    /// `signature()` could never return, because `get` rejects it on length.
    ///
    /// If there is already a cached value for the given `Signature::name`, this is a no-op.
    fn insert(&self, signature: Signature) {
        if signature.name.len() > MAX_CACHEABLE_COMMAND_LEN {
            return;
        }
        self.signatures
