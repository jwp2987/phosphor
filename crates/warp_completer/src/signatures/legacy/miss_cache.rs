...
pub(super) const MAX_CACHED_MISSES: usize = 256;
...
impl MissCache {
    ...
    pub(super) fn insert(&self, command: String) {
        if self.capacity == 0 {
            // Nothing may be retained at all. Without this the eviction below is a no-op --
            // `order.len() >= 0` holds but `pop_front` returns `None`, short-circuiting the
            // let-chain -- and the push still runs, giving an effective capacity of 1. Not
            // reachable today (the only constructor in production is `Default`), but a
            // bounds-enforcing type must not have an off-by-one at its own boundary.
            return;
        }
