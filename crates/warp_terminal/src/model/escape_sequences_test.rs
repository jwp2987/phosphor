...
///
/// Ported from upstream `af2f7c61d` ("Encode Cmd/Option modifiers for editing keys under the
/// Kitty keyboard protocol", #14012). Upstream reaches `OperatingSystem` through
/// `warpui_core`; this fork's `warp_terminal` depends on `warpui`, which re-exports it, and the
/// test module already imports it at the top.
///
