...
// `test_create_team_workflow_pane_from_command_palette` was DELETED, not lost: it
// drove the "Create a New Team Workflow" palette action through `join_a_workspace()`,
// whose assertion is `UserWorkspaces::has_teams()` — hard-coded `false` here and
// staying that way (DECLINED.md, "Teams stay stubbed"). The personal-workflow twin
// above is the live coverage. See that row before restoring it.
