...
///
/// The trailing `true` step is a no-op warmup command: in this harness the
/// very first command run immediately after bootstrap can fail to reach a
/// `DoneWithExecution` block state within the step timeout (a first-command
/// shell-integration race that reproduces with any command — `echo`, `printf`
/// — independent of OSC 8 or the feature flag). Running one throwaway command
/// first lets every subsequent OSC 8 command block settle normally, so the
/// tests exercise the hyperlink behavior rather than that race. It adds a
/// leading command block but never becomes the last block, so the per-test
/// `last_non_hidden_block()` lookups still resolve the OSC 8 output block.
