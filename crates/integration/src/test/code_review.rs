...
use warp::{
    integration_testing::{
        code_review::{
            ...
            expand_first_hidden_section_and_assert_full_reveal,
            scroll_code_review_to_deleted_range, scroll_code_review_to_footer,
            scroll_code_review_to_header, scroll_code_review_to_line, ScrollRegion,
        },
    ...
    new_builder()
        ...
        .with_step(
            TestStep::new("Wait for the diff to collapse unchanged context into hidden sections")
                .set_timeout(Duration::from_secs(20))
                .add_assertion(assert_min_hidden_sections(PathBuf::from(TEST_FILE_NAME), 1)),
        )
