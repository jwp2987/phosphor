...
use warp::{
    integration_testing::warp_drive::assert_is_left_panel_open,
...
pub fn test_suggestions_menu_positioning() -> Builder {
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Open Library")
                        ...
                        workspace
                            ...
                            .read(app, |pane_group, ctx| {
                                pane_group
                                    ...
                                    .as_ref(ctx)
                                    .as_any()
                                    .downcast_ref::<warp::terminal::local_tty::TerminalManager<warp::terminal::view::TerminalView>>()
...
// `test_create_folder_from_command_palette` was DELETED, not lost: it drove the
// "Create a New Team Folder" palette action through `join_a_workspace()`, whose
// assertion is `UserWorkspaces::has_teams()` — hard-coded `false` here and staying
// that way (DECLINED.md, "Teams stay stubbed"). See that row before restoring it.
