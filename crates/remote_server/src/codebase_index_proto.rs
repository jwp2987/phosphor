...
//!
//! Ported from the pin (`02b53fcd8:crates/remote_server/src/codebase_index_proto.rs`)
//! unchanged except for [`EmbeddingProviderConfig`], which has no pin
//! counterpart — see the `EmbeddingProviderConfig` message in
//! `proto/remote_server.proto` for why this fork sends the user's own
//! embedding endpoint where the pin sent a Warp bearer token.
