...
//!
//! Prefers reading the `http_client::current_proxy_config()` global singleton.
//! If it's `ProxyMode::Custom` or `Off`, apply it directly; if it's
//! `ProxyMode::System`, fall back to the original environment-variable
//! parsing logic (`HTTPS_PROXY` / `HTTP_PROXY` / `ALL_PROXY` / `NO_PROXY`).
//! This way, the settings page's Custom URL / Off can also cover WebSocket. See Issue #72.
//!
...
/// A mirror of the proxy mode. Corresponds 1:1 with `http_client::ProxyMode`;
/// mirrored locally here purely to avoid a
/// `websocket -> http_client -> warp_core -> websocket` circular dependency.
/// The `app` calls both `http_client::set_global_proxy_config` and
/// `websocket::set_global_proxy_config` together at startup / when settings
...
/// A mirror of `http_client::ProxyConfig`; see [`ProxyMode`]'s description.
...
fn current_proxy_config() -> ProxyConfig {
    match slot().read() {
        ...
        Err(err) => {
            log::error!("Failed to read WebSocket proxy config: RwLock poisoned ({err})");
            ProxyConfig::default()
        }
    }
}
...
///
/// Priority order:
/// 1. Reads `http_client::current_proxy_config()`:
///    - `Custom`: uses the URL / auth / no_proxy filled in on the settings page.
///    - `Off`: returns `None`.
///    - `System`: falls through to environment-variable parsing.
...
pub fn resolve_proxy(uri: &http::Uri) -> anyhow::Result<Option<ProxyInfo>> {
    ...
    // Prefer the global setting (mirrored from http_client, avoiding a circular dependency).
    let global_cfg = current_proxy_config();
    match global_cfg.mode {
        ProxyMode::Off => return Ok(None),
        ProxyMode::Custom => {
            // Reuse the comma-separated no_proxy list from settings.
            ...
            if trimmed.is_empty() {
                ...
                log::warn!("WebSocket: HTTP proxy set to Custom but URL is empty, falling back to environment variables");
            } else {
        ...
        ProxyMode::System => {
            // Falls through to environment-variable parsing.
        }
    }
...
/// A variant of `parse_proxy_url` that layers in explicit username / password from settings (overriding those in the URL).
