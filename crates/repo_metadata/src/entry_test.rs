...
// ---------------------------------------------------------------------------
// Fork-retained tests below.
//
// `path_passes_filters` is a fork-only entry-point (upstream folded its
// behavior into `should_watch_repo_directory`); these guard it directly.
// The `is_within_symlink_*` tests exercise the `pub(crate)` pruning check
// consumed by `watcher.rs`, which upstream only covers indirectly through the
// `should_watch_*` tests.
// ---------------------------------------------------------------------------
