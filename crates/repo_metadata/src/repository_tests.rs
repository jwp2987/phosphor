...
use super::{
    merge_repository_updates, Repository, RepositorySubscriber, RepositorySubscription,
    RepositoryWatchMode, TrackedRemoteRef,
};
...
// The four tests below are ported from upstream `d78ced530` ("Avoid Git probes for
// filesystem-only directory watchers", #14830). They pin the `RepositoryWatchMode`
// contract: a `FilesystemOnly` subscription must never activate Git metadata tracking,
// Git tracking activates exactly once no matter how many Git subscribers there are, and
// the per-subscriber update fan-out strips the Git flags for filesystem-only subscribers.
