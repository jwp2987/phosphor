...
impl Repository {
    ...
    pub(super) fn enrich_external_git_directory(
        ...
        // A repository can learn it is a linked worktree *after* a git-mode subscriber
        // already started watching, so the newly-known git paths must be registered now.
        #[cfg(feature = "local_fs")]
    ...
    /// The set of Git-metadata directories a `GitRepository`-mode subscriber needs
    /// watched: the per-worktree gitdir (linked worktrees only), and — for linked
    /// worktrees — the shared `.git/refs` directory (commit refs + remote-tracking
    /// refs) and `.git/config` (tracking state) so shared-ref and tracking-state
    /// changes are visible even when the main worktree isn't itself registered.
    ///
    /// The working-tree root is deliberately *not* included: it is watched for every
    /// subscriber regardless of mode, and registering it here would reintroduce the
    /// Git tracking that `RepositoryWatchMode::FilesystemOnly` exists to avoid.
    #[cfg(feature = "local_fs")]
    ...
    pub(crate) fn scan_subscriber(
        ...
        match self.subscribers.remove(&subscriber_id) { Some(mut subscription) => {
    ...
    /// Returns the subscriber IDs of this repository's `GitRepository`-mode subscribers.
    #[cfg(feature = "local_fs")]
