//! Metadata for a "workspace" — a repository root the user has actually used.
//!
//! Restored verbatim from the pinned oracle `02b53fcd8`
//! (`crates/ai/src/workspace.rs`). It was deleted by `efcaa42b8` together with
//! `PersistedWorkspace`; both are back.
//!
//! `WorkspaceMetadataEvent` is the update channel the pin's
//! `CodebaseIndexManager` uses to tell `PersistedWorkspace` that a repo was
//! queried/modified/created. That manager does not exist in this fork yet (see
//! `app/src/ai/persisted_workspace.rs`, `handle_index_metadata_event`), but the
//! type is kept so the seam can be wired up without redefining it.
