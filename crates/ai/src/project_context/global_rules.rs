//! File-based global rules — a well-known home-directory location (currently
//! `~/.agents/AGENTS.md`) whose contents are layered on top of project rules
//! for every agent query, regardless of which project (if any) is active.
//!
//! Ported from the pinned oracle (`02b53fcd8`, release `2026.07.29.09.05`
//! stable — see `ORACLE.md`), `crates/ai/src/project_context/global_rules.rs`.
//! **Adapted, not a byte-for-byte port**: the pin's `ProjectRule::path` is a
//! `LocalOrRemotePath`; this fork's `ProjectRule::path` (`super::model::
//! ProjectRule`) is a plain local `PathBuf` (`path_to_rules` has no per-host
//! dimension at all — see #575). File-based global rules are inherently a
//! local-machine concept (a well-known `$HOME` location on whichever machine
//! this `ProjectContextModel` instance runs on, local app or remote daemon),
//! so the `LocalOrRemotePath` wrapping the pin used is dropped rather than
//! adapted; every path here is local to this process by construction.
