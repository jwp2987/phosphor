//! Ported from Warp's `crates/warp_tui/src/zero_state_animation_config.rs` at
//! the pinned oracle (`02b53fcd8`, Warp `2026.07.29.09.05` stable — see
//! `ORACLE.md`). See #384.
//!
//! Deviations from the pin, both required by earlier fork decisions:
//! - `warp::settings::TuiZeroStateSettingsChangedEvent` is this fork's
//!   already-merged settings group (PR #472, commit `0d889ff0e`); the type
//!   names match the pin one-for-one.
//! - Relative `AsciiFile` paths resolve against `warp_core::paths::config_local_dir()`
//!   instead of a TUI-only `tui_config_local_dir()`. The pin keeps a separate
//!   TUI config directory because it gives the GUI and TUI separate app ids;
//!   this fork deliberately shares one app id (and one config file) between
//!   GUI and TUI (see `DECLINED.md`, "TUI/GUI shared app id"), so there is no
//!   TUI-only config directory to resolve against.
...
use super::{
    MAX_ASCII_ART_BYTES, MAX_ASCII_ART_COLS, MAX_ASCII_ART_ROWS, built_in_mark_contains,
    BUILT_IN_MARK_CELL_ASPECT_RATIO,
};
...
impl ZeroStateShape {
    pub(super) fn contains(&self, x: f64, y: f64) -> bool {
        match self {
            Self::BuiltIn => built_in_mark_contains(x, y),
    ...
    pub(super) fn cell_aspect_ratio(&self) -> f64 {
        match self {
            Self::BuiltIn => BUILT_IN_MARK_CELL_ASPECT_RATIO,
...
impl Default for ZeroStateAnimationConfig {
    fn default() -> Self {
        Self {
            ...
            shape: Arc::new(ZeroStateShape::BuiltIn),
    ...
    pub(crate) fn load(
        ...
        let (active_object, shape, load_failure) = match load_object_shape(object, config_dir) {
            ...
            Err((path, error)) => {
                safe_warn!(
                    safe: (
                        "Could not load custom TUI zero-state ASCII art; using the built-in mark"
                    ),
                    full: (
                        "Could not load custom TUI zero-state ASCII art; using the built-in mark: path={} error={error}",
                        path.display()
                    )
                );
                (
                    TuiZeroStateObject::BuiltIn,
                    ZeroStateShape::BuiltIn,
    ...
    pub(super) fn reload_object(
        ...
        match load_object_shape(object, config_dir) {
            ...
            Err((path, error)) => {
                safe_warn!(
                    safe: (
                        "Could not reload custom TUI zero-state ASCII art; keeping the current object"
                    ),
                    full: (
                        "Could not reload custom TUI zero-state ASCII art; keeping the current object: path={} error={error}",
                        path.display()
                    )
                );
...
fn load_object_shape(
    ...
    match object {
        TuiZeroStateObject::BuiltIn => Ok(ZeroStateShape::BuiltIn),
