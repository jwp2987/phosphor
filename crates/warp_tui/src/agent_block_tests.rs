...
use warp::tui_export::{
    ...
    FailedOutputPresentation, LLMId, MessageId, OutputStatusUpdateCallback,
...
fn streaming_conversation_summary_renders_collapsed_by_default() {
    App::test((), |mut app| async move {
        ...
        app.read(|app_ctx| {
            ...
            assert_eq!(
                render_block_lines(block, 40, app_ctx),
                // Zap labels this section "Conversation summarized" rather than
                // upstream Warp's "Conversation summary" (pre-existing fork
                // wording, unrelated to this collapse-default fix).
