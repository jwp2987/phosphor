...
/// Resolves the currently-bound keystroke display for an editable binding, if
/// any binding by that name is both registered and active in `context`.
/// Returns `None` for an unbound, standard, custom, or empty-trigger binding —
/// including a name that has no registered binding at all — so callers can use
/// it to gate optional shortcut-hint UI without special-casing "not wired up
/// yet".
