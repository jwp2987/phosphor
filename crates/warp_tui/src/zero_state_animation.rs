...
/// Eases the rotation phase so the mark dwells on its readable front and back
/// poses. The easing is anti-symmetric about every cardinal angle, so the
/// configured revolution period and the exact quarter/half-turn poses are
/// preserved; only the speed between them varies.
