...
//!
//! Ported from Warp's `crates/warp_tui/src/zero_state_animation.rs` at the
//! pinned oracle (`02b53fcd8`, Warp `2026.07.29.09.05` stable — see
//! `ORACLE.md`). See #384: the fork previously shipped a pre-rewrite plain
//! starfield here; this replaces it with the pin's object system.
//!
//! The built-in mark or a user-provided ASCII silhouette (see
//! `zero_state_animation_config`, backed by the `TuiZeroStateObject` /
//! `TuiZeroStateRotationPeriodSeconds` / `TuiZeroStateExtrusionDepth`
//! settings already ported in PR #472) is sampled into a shallow, ghosted
//! wireframe, rotated around its vertical axis, and projected back onto
//! terminal cells. A tiny z-buffer keeps the front-most sample for each
//! cell, while directional ASCII edges and sparse stippling retain depth
//! without a solid fill.
//!
//! **Branding deviation from the pin (see #384 / DECLINED.md):** the pin's
//! built-in object is the actual Warp logo, sampled from that product's SVG
//! artwork (`warp-logo-light.svg`) via two hand-picked polygons. Copying
//! that artwork into this fork would embed Warp's trademark in a project
//! that has otherwise deliberately dropped Warp naming/branding (see
//! `docs/DESIGN-PHOSPHOR-FORK.md` §6). The built-in object here is instead a
//! simple rotating diamond mark ([`built_in_mark_contains`]) — thematically
//! a "spark", fitting a project named Phosphor, and trivial to reason about
//! (a single `|x| + |y| <= r` predicate) rather than derived from anyone's
//! copyrighted artwork. `TuiZeroStateObject::AsciiFile` (a user-supplied
//! silhouette) is unaffected and works identically to the pin.
...
/// Column:row aspect ratio for the built-in mark. Terminal cells are roughly
/// twice as tall as wide, so a shape that should read as visually symmetric
/// (like the diamond below) needs about twice as many columns as rows.
const BUILT_IN_MARK_CELL_ASPECT_RATIO: f64 = 2.0;
...
/// Half-width of the built-in diamond mark in normalized model space
/// (`[-1, 1]` on both axes, matching [`ZeroStateShape::contains`]'s
/// coordinate system). `0.85` leaves a small margin so the rotated silhouette
/// never touches the fitted bounding box exactly at any angle.
const BUILT_IN_MARK_RADIUS: f64 = 0.85;

#[derive(Clone, Copy)]
pub(crate) struct ZeroStateMarkStyles {
...
impl ZeroStateInteractionHandle {
    // The pin also has `non_interactive()`: a permanently hidden handle for
    // surfaces that render the object as pure decoration and must keep
    // forwarding mouse events to their own handlers. Its only caller is the
    // pin's `ui.rs` `auth_layout` — the cloud sign-in screen, which this fork
    // does not have — so it is deliberately not ported rather than landed as
    // an uncalled constructor. Restore it alongside any future decorative use
    // of `ZeroStateAnimationElement` outside the zero state.
...
/// The column the radial starfield emits from: the center of the responsive
/// object panel, so the stars appear to stream out from behind the mark rather
/// than from the middle of the terminal. Falls back to the screen center when
/// the panel is too narrow to show the object at all.
...
/// The starfield as its own full-bleed layer, painted behind both the copy and
/// the (narrower) object panel so the stars fill the whole transcript.
...
impl ZeroStateAnimationElement {
    ...
    /// Drops this element's own starfield, for the layout that paints the
    /// stars once across the full transcript in [`ZeroStateStarfieldElement`].
...
/// Eases a rotation phase so the mark dwells on its readable front and back
...
/// Ghost stippling keyed on the *projected* cell rather than the source
/// sample, so the interior speckle stays anchored to the screen while the
/// geometry rotates through it.
...
/// The built-in mark: a simple diamond in normalized model space. See the
/// module doc comment for why this isn't Warp's logo.
fn built_in_mark_contains(x: f64, y: f64) -> bool {
