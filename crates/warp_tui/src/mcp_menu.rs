//! Searchable `/mcp` menu state for the TUI.
//!
//! The model reads the app-side [`TuiMcpManager`] snapshot, filters it by the
//! input editor's query, and renders exactly one row per server. A row carries
//! two actions: a primary action driven by the server's runtime status (Enter)
//! and an optional log-out action (Ctrl+R). Selection is keyed by
//! `TuiMcpServerId` so it survives live refreshes.
...
struct TuiMcpMenuRow {
    /// `None` for non-server rows (the config-error row), which are rendered
...
impl TuiMcpMenuModel {
    ...
    pub(crate) fn open(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        // The menu owns the input line while open: it is the search field.
        self.input_editor
    ...
    fn refresh_rows(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        // Keep the same server selected across a live refresh, falling back to
        // the first selectable row when it disappeared or was filtered out.
...
fn menu_rows(snapshot: &TuiMcpSnapshot, query: &str) -> Vec<TuiMcpMenuRow> {
    ...
    // One row per unhealthy config file, so a broken Codex config does not hide
    // behind a healthy Claude one.
...
/// A row is selectable whenever it stands for a server, even when that server's
/// current status offers no primary action — the Ctrl+R log-out action and
/// stable selection both need transitional rows to stay reachable.
