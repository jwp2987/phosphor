//! Construction/closed-state tests for [`TuiExchangeMenuModel`]. Opening the
//! menu reads the `BlocklistAIHistoryModel` singleton and a live conversation,
//! so the open/list/accept behavior is exercised end-to-end via the PTY harness
//! instead (see `crates/warp_tui/scripts/tui_harness.py`).
...
fn new_menu_is_closed() {
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            ...
            let menu = ctx.add_model(|ctx| TuiExchangeMenuModel::new(editor, mode, ctx));
