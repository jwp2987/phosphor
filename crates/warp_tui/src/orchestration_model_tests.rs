...
// ---------------------------------------------------------------------------
// Multi-level drill-down snapshot (upstream 683d40782, CODE-1946).
//
// Upstream builds these trees with `seed_remote_child` + `restore_descendants`
// -- the cloud restore path, which does not exist here (`DECLINED.md` #290).
// They are rebuilt on `add_child_session`, the fork's local child fixture, so
// the tree SHAPES and every assertion are upstream's; only the way a child
// comes into existence differs. That is the right substitution: these tests
// are about topology, breadcrumbs, paging and teardown, none of which depend
// on whether a child is remote -- and `is_remote_child` is permanently false
// in this fork anyway.
//
// Not ported: upstream's edits to
// `restoring_parent_materializes_supported_descendant_sessions`, which is
// itself a cloud-restore test absent from this file.
// ---------------------------------------------------------------------------
