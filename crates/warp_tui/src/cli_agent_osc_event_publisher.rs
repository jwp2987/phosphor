//! Publishes this TUI session's agent lifecycle over the CLI-agent OSC 777
//! protocol, so a Phosphor GUI pane hosting the TUI can track it exactly the
//! way it tracks Claude Code, OpenCode and the other plugin-backed agents.
//!
//! The emitted sequence is
//! `ESC ] 777 ; notify ; warp://cli-agent ; <json> BEL`, where `<json>` is a
//! [`CLIAgentNotification`]. See `warp_core::cli_agent_protocol` for why those
//! wire identifiers keep their `warp` spelling.
//!
//! Every emission is gated on the *selected* (foreground) conversation, so a
//! background or orchestrated child conversation cannot flip the GUI's status
//! chip. The Blocked -> Unblocked transition is symmetric: whichever
//! conversation can set Blocked (via `permission_request` / `question_asked`)
//! is the only one that can clear it (via `tool_complete` /
//! `permission_replied`). Restorations are skipped so replaying conversations
//! at startup does not re-emit a session's whole history.
...
/// The `"agent"` value on the wire. This is a protocol string, not a display
/// name: the GUI resolves it through `CLIAgent::command_prefixes()`, whose
/// `PhosphorTui` list already contains `"warp-tui"` for exactly this
/// self-recognition purpose. The user-visible name stays "Phosphor TUI"
/// (`CLIAgent::display_name()`).
const WARP_TUI_AGENT_NAME: &str = "warp-tui";
const MAX_NOTIFICATION_DESCRIPTION_CHARS: usize = 320;

/// Returns whether the host terminal advertises structured CLI-agent
/// notifications.
///
/// Requiring both values matches the Claude plugin's structured-notification
/// capability check and prevents standalone TUI runs (no hosting GUI) from
/// writing OSC 777 into whatever terminal happens to be attached.
...
impl CliAgentOscEventPublisher {
    ...
    pub(crate) fn publish_session_start(&self, ctx: &AppContext) {
        ...
        notification.summary = Some("Phosphor Agent session started.".to_owned());
    ...
    pub(crate) fn publish_prompt_submit(&self, prompt: String, ctx: &AppContext) {
        ...
        notification.summary = Some("Phosphor Agent is working on your request.".to_owned());
        self.publish(notification);
    }
    ...
    fn handle_history_event(&self, event: &BlocklistAIHistoryEvent, ctx: &AppContext) {
        ...
        notification.summary = Some(
            match new_status {
                ConversationStatus::Success => "Phosphor Agent completed your request.",
                ConversationStatus::Error => "Phosphor Agent encountered an error.",
                ConversationStatus::Cancelled => "Phosphor Agent was cancelled.",
                ConversationStatus::InProgress => "Phosphor Agent is working on your request.",
                ConversationStatus::Blocked { .. } => "Phosphor Agent is waiting for your input.",
                ConversationStatus::TransientError => "Phosphor Agent is reconnecting.",
                ConversationStatus::WaitingForEvents => "Phosphor Agent is waiting for events.",
            }
            .to_owned(),
        );
