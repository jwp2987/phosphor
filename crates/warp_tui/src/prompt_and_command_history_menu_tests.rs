//! Tests for [`TuiPromptAndCommandHistoryMenuModel`]: combined ordering/dedupe
//! (kept distinct per kind), mode-based filtering, selection-driven input-type
//! preview/restore, acceptance kind resolution, and empty states.
//!
//! Ported from the pinned Warp oracle (`02b53fcd8`)
//! `crates/warp_tui/src/prompt_and_command_history_menu_tests.rs` for issue
//! #387. Session/history fixtures go through this fork's
//! `add_tui_history_test_models` / `append_tui_history_test_command`
//! (`app/src/tui_test_support.rs`), ported alongside the model itself since
//! the pin's up-arrow combiner reads real command history through a real
//! active session, not just `BlocklistAIHistoryModel`.
...
fn setup(
    ...
    input_mode.update(ctx, |input_mode, ctx| {
        // NOTE(adapted): the pin's `set_input_type` also takes an
        // `InputTypeAutoDetectionSource`; this fork's does not (no
        // detection-provenance tracking — see `input_mode_policy.rs`).
