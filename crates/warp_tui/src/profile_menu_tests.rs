//! Construction/closed-state tests for [`TuiProfileMenuModel`]. Opening the
//! menu reads the `AIExecutionProfilesModel` singleton (a heavy registration
//! chain), so the open/list/accept behavior is exercised end-to-end via the PTY
//! harness instead (see `crates/warp_tui/scripts/tui_harness.py`).
    ...
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            ...
            let menu = ctx.add_model(|ctx| {
                TuiProfileMenuModel::new(editor, mode, EntityId::new(), ctx)
            });
