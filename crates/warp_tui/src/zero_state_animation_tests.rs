// Ported from Warp's `crates/warp_tui/src/zero_state_animation_tests.rs` at
// the pinned oracle (`02b53fcd8`, Warp `2026.07.29.09.05` stable — see
// `ORACLE.md`). See #384.
//
// Deviations from the pin, all downstream of the branding decision recorded
// in the module doc comment on `zero_state_animation.rs`:
// - `logo_mask_preserves_the_offset_warp_faces` tested hand-picked points
//   against Warp's actual logo silhouette (two offset polygons sampled from
//   `warp-logo-light.svg`). The built-in mark here is a diamond instead, so
//   this becomes `built_in_mark_contains_the_diamond_region`, testing the
//   same kind of thing (containment at representative points) against the
//   new shape's `|x| + |y| <= r` predicate.
// - `full_face_frame_is_recognizable_and_centered` asserted a `"------"` run
//   (Warp's logo has long flat block edges). The diamond's edges are
...
//   proportions). The custom-aspect-ratio case (`4.0`) is unchanged, since it
//   doesn't depend on the built-in shape at all.
// - `WARP_W_ART` is renamed `CHEVRON_ART` (a fixture name, not a rendered
//   glyph identity) so no "Warp" identifier appears in this fork's source.
//
// Every other test in this file is unchanged from the pin: the geometric
// properties they assert (narrower + Side-exposed at a quarter turn, only
// Back visible at a half turn, an exact match after one full revolution, the
// star density/motion, the ASCII-art parser, and all the config-loading /
// settings-reload behavior) are true of the depth-extrusion algorithm and
// the settings plumbing generically, not specific to Warp's silhouette, and
// were verified against the diamond shape's actual output before landing
// (see PR #384 for the verification method).
...
use super::{
    ActivePress, BUILT_IN_MARK_CELL_ASPECT_RATIO, FLICK_RELEASE_VELOCITY_GAIN, LogoCell, LogoGlyph,
    LogoProjector, LogoSurface, MAX_INTERACTIVE_RADIANS_PER_SECOND, MIN_ANIMATION_COLS,
    MIN_ANIMATION_ROWS, MOMENTUM_SETTLE_DURATION, REPAINT_INTERVAL, ZeroStateAnimationElement,
    ZeroStateInteractionHandle, ZeroStateMarkStyles, built_in_mark_contains,
    configured_idle_velocity, face_linger_angle, fitted_logo_size, glyph_for_tangent, idle_angle,
    is_ghost_stipple_cell, logo_frame_at, object_frame_at, object_frame_at_angle,
    object_frame_at_angle_with_background, object_frame_at_with_background, rotation_angle,
    star_count_for_size, starfield_emitter_x,
};

const PANEL_SIZE: TuiSize = TuiSize::new(52, 20);
const DIAMOND_ART: &str = "   #\n  ###\n #####\n  ###\n   #\n";
const ROCKET_ART: &str = "    #\n   ###\n  ####\n #####\n   ###\n  #  #\n";
const CHEVRON_ART: &str = "#       #\n#       #\n#   #   #\n#  # #  #\n ##   ##\n";
...
#[test]
fn built_in_mark_contains_the_diamond_region() {
    assert!(built_in_mark_contains(0.0, 0.0));
    assert!(built_in_mark_contains(0.4, 0.3));
    assert!(built_in_mark_contains(0.0, -0.8));
    assert!(built_in_mark_contains(-0.8, 0.0));
    assert!(!built_in_mark_contains(0.6, 0.6));
    assert!(!built_in_mark_contains(-0.9, 0.9));
    assert!(!built_in_mark_contains(0.0, 0.9));
}
...
fn rotation_lingers_on_faces_and_preserves_cardinal_angles() {
    ...
    assert!(
        face_step < edge_step,
        "the mark should move more slowly near a face ({face_step}) than near an edge ({edge_step})"
    );
}
...
fn full_face_frame_is_recognizable_and_centered() {
    ...
    assert!(
        ...
        "expected a sparse mark outline, got {occupied} cells"
    );
    assert!(
        frame
            .iter_cells()
...
/// Frame-to-frame churn must stay bounded so the mark reads as a rotating
/// object rather than a shimmering field of noise — occupancy *and* glyph or
/// surface content, since screen-space stippling can change a cell's glyph
/// without changing whether it is occupied. The pin's `<= 80` / `<= 120`
/// bounds hold for this fork's diamond mark unchanged (its worst adjacent
/// frames at this panel size change 40 cells' occupancy and 84 cells'
/// content).
#[test]
...
fn logo_scales_down_while_preserving_cell_aspect() {
    assert_eq!(
        fitted_logo_size(TuiSize::new(100, 40), BUILT_IN_MARK_CELL_ASPECT_RATIO),
        Some((34, 17))
    );
    assert_eq!(
        fitted_logo_size(TuiSize::new(30, 12), BUILT_IN_MARK_CELL_ASPECT_RATIO),
        Some((20, 10))
    );
...
fn representative_ascii_fixtures_have_distinct_dimensions() {
    ...
    assert_eq!(AsciiArtMask::parse(CHEVRON_ART).unwrap().size(), (9, 5));
}
...
fn relative_ascii_paths_resolve_from_the_tui_config_directory() {
    let config_dir = Path::new("/tmp/phosphor-tui-config");
    assert_eq!(
...
fn startup_loader_falls_back_for_missing_or_invalid_art_only() {
    ...
    for path in ["missing.txt", "invalid.txt"] {
        ...
        assert!(matches!(config.shape.as_ref(), ZeroStateShape::BuiltIn));
...
fn custom_animation_element_paints_and_requests_another_frame() {
    App::test((), |mut app| async move {
        let config = Arc::new(custom_config(CHEVRON_ART, 4.0, 0.18));
        let (_, view) = app.update(|ctx| {
