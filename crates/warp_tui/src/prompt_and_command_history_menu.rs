...
//! the shared GUI/TUI history combiner ([`tui_up_arrow_history`]). Agent mode
//! shows prompts and commands; shell mode shows commands only. Selection
//! previews both text and input type, and dismissing restores the original
//! buffer and input type.
//!
//! Ported from the pinned Warp oracle (`02b53fcd8`)
//! `crates/warp_tui/src/prompt_and_command_history_menu.rs` for issue #387,
//! replacing the prompts-only `prompt_history_menu.rs` the same way the pin
//! does. Two adaptations to this fork's API surface, both noted inline below:
...
use warp::tui_export::{
    ...
    History, HistoryEvent, InputType, TuiUpArrowHistoryItemKind, UpArrowHistoryConfig,
...
impl TuiPromptAndCommandHistoryMenuModel {
    /// Creates a closed prompt-and-command history menu and subscribes it to
    /// input and history changes.
    pub(crate) fn new(
        input_editor: ModelHandle<CodeEditorModel>,
        input_mode: ModelHandle<BlocklistAIInputModel>,
        suggestions_mode: ModelHandle<TuiInputSuggestionsModeModel>,
        ...
        ctx.subscribe_to_model(&History::handle(ctx), |model, event, ctx| {
    ...
    /// Closes the menu and restores the buffer and input type the user had
    /// before opening it.
    pub(crate) fn dismiss(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        self.close(ctx);
        // NOTE(adapted): the pin passes an `InputTypeAutoDetectionSource` here;
        // this fork's `set_input_type` has no detection-provenance parameter.
        self.set_input_type(original_input_type, ctx);
    ...
    /// Accepts the current selection, closing the menu and returning the row
    /// to submit. The input's current visible type determines how the row is
    ...
    fn on_command_history_initialized(
        ...
        if !self.is_open(ctx) || self.session_id(ctx) != Some(session_id) {
            return;
        }
    ...
    fn set_input_type(&self, input_type: InputType, ctx: &mut ModelContext<Self>) {
        self.input_mode.update(ctx, |input_mode, ctx| {
            input_mode.set_input_type(input_type, ctx);
        });
    }

    /// The current active session's id, if any. `ActiveSession` in this fork
    ...
    fn preview_selection(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        self.set_input_type(input_type, ctx);
