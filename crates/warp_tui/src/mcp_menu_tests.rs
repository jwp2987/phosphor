...
use warp::tui_export::{
    ...
    TuiMcpServerSnapshot, TuiMcpServerSource, TuiMcpServerStatus, TuiMcpSnapshot, TuiMcpTransport,
};
...
fn file_based(id: u64, name: &str, status: TuiMcpServerStatus) -> TuiMcpServerSnapshot {
    server(
        TuiMcpServerId::FileBased(id),
        name,
...
fn query_filters_server_rows_case_insensitively() {
    ...
    let snapshot = snapshot(vec![
        ...
        file_based(2, "Linear", TuiMcpServerStatus::Offline),
    ]);
...
#[test]
fn query_also_matches_a_row_source_label() {
    let snapshot = snapshot(vec![
        server(
            TuiMcpServerId::Template(Uuid::from_u128(7)),
            "Figma",
    ...
    let rows = menu_rows(&snapshot, "saved");

    assert_eq!(rows.len(), 1);
    assert_eq!(rows[0].title, "Figma");
}

#[test]
fn available_row_is_distinct_from_offline_and_requires_enable() {
    let id = TuiMcpServerId::Template(Uuid::from_u128(7));
    ...
    assert_eq!(
        rows[0].description.as_deref(),
        Some("saved template · stdio · available")
    );
}
...
fn multiple_diagnostics_render_without_hiding_healthy_servers() {
    let mut snapshot = snapshot(vec![file_based(
        2,
        "Healthy",
        TuiMcpServerStatus::Offline,
    )]);
...
fn logout_capability_keeps_a_transitional_server_selectable() {
    ...
    assert!(row_is_selectable(&rows[0]));
    assert_eq!(rows[0].logout_action, Some(TuiMcpAction::LogOut(id)));
}

#[test]
fn actionless_server_transition_remains_selectable() {
    let snapshot = snapshot(vec![file_based(
        1,
        "GitHub",
