        ...
        match &self.source {
            TuiTerminalSessionStateSource::Session {
                ...
                let state = TuiBlockSessionState {
                    ...
                    // RunAgents/orchestration is cloud-backed and out of scope
                    // for this fork (DECLINED.md, #290) — there is no
                    // orchestration tab bar to query here, so this source
                    // never reports orchestration as available. The
                    // `orchestration_available`-gated behavior below is kept
                    // (and covered by tests that construct state directly)
                    // so it activates automatically if orchestration is ever
                    // reinstated as a local, non-cloud feature.
...
/// The current source preventing the normal session composer from owning
/// input.
///
/// This is a reduced counterpart of the pinned oracle's `BlockingInputSource`:
/// `Orchestration`, `Handoff`, and `GrokOAuth` are omitted because none of
/// those subsystems exist in this fork (RunAgents/orchestration is
/// cloud-backed and declined, see `DECLINED.md` #290; handoff is the
/// local-to-cloud cloud handoff flow; Grok subscription OAuth is declined,
/// #319 — API-key Grok credentials are supported). `AskQuestion` and
/// `Permission` mirror `crate::agent_block::TuiBlockingChild`, which already
/// covers those two locally.
#[derive(Clone)]
...
impl From<TuiBlockingChild> for BlockingInputSource {
    fn from(blocker: TuiBlockingChild) -> Self {
        match blocker {
            TuiBlockingChild::AskQuestion(view) => Self::AskQuestion(view),
            TuiBlockingChild::Permission(view) => Self::Permission(view),
        }
    }
}
        ...
        match self {
            ...
            Self::Block(state) => match &state.interaction {
                TuiInteractionState::Blocking(source) => match source {
                    ...
                    BlockingInputSource::AskQuestion(_) | BlockingInputSource::Permission(_) => {
