...
/// Shared CJK Han codepoint predicate: these characters have different glyph
/// shapes across ja / zh-Hans / zh-Hant / ko. Callers use this to bias the
/// DirectWrite / cosmic-text Han fallback toward a local font for the current
/// UI locale (e.g. ja-* -> Yu Gothic UI, ko-* -> Malgun Gothic,
/// zh-Hant -> Microsoft JhengHei UI).
///
/// Covers all Unified Ideographs blocks assigned as of Unicode 15.0 (up
/// through Extension G). When Unicode expands further, append here as
/// needed — this is the **single source of truth** for the CJK Han range in
/// this repo; callers (`crates/warpui/src/windowing/winit/fonts/windows.rs`
/// and `app/src/font_fallback.rs`) should not fork their own range.
