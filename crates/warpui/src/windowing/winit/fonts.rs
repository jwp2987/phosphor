...
impl FontDB {
    pub fn new() -> Self {
        ...
        // Warm-up is added for Windows only: PR #62 wired up locale-aware CJK
        // fallback only on the Windows path, and that regression
        // (zerx-lab/warp#68) also only showed up on Windows. macOS/Linux go
        // through their own fallback paths and don't need warm-up.
        #[cfg(target_os = "windows")]
...
/// Rebuilds the cosmic-text `FontSystem` in place for a new locale, reusing
/// the existing `fontdb::Database`, so font data isn't reloaded.
/// `cosmic_text::FontSystem` has no public `set_locale`, so here we replace
/// the whole struct via `std::mem::replace` and then re-attach the db.
///
/// Cost: `into_locale_and_db` only preserves `(locale, db)` across the
/// replacement. The new `FontSystem` rebuilds its instance-level caches from
/// scratch: `font_cache`, `monospace_font_ids`,
/// `per_script_monospace_font_ids`, `font_codepoint_support_info_cache`,
/// `font_matches_cache`, `shape_plan_cache`, and `shape_run_cache` when the
/// `shape-run-cache` feature is enabled. The constructor also re-runs
/// `cache_fonts` over the monospace font set; on Windows, which tends to have
/// many system fonts, the first rebuild's rendering can take anywhere from a
/// few hundred ms to around 1 s. The mmap-backed font file data itself is not
/// re-read.
///
/// This is acceptable because UI locale switching happens rarely (currently
/// driven by Settings -> Language and the "restart Zap" prompt), and the
/// user already expects a brief interruption. If low-latency hot reload is
/// needed later, the hook is in `app::i18n::set_locale`.
