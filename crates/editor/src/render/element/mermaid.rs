...
use crate::{
    ...
    render::{
        ...
        model::{viewport::ViewportItem, BlockItem, RenderState},
        BLOCK_FOOTER_HEIGHT,
    },
};
...
impl RenderableBlock for RenderableMermaidDiagram {
    ...
    fn paint(&mut self, model: &RenderState, ctx: &mut RenderContext, app: &AppContext) {
        ...
        // Place the footer (Raw/Rendered toggle) at a higher z-index for event handling, matching
        // `RenderableRunnableCommand`. Without this, the toggle would disappear once a block
        // renders as a diagram, leaving no way to switch back to Raw.
