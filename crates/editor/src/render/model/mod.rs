...
pub struct RenderLayoutOptions {
    ...
    /// Buffer start offsets (0-indexed) of Mermaid blocks the user has explicitly toggled to
    /// `Rendered`. Checked per-block in addition to `render_mermaid_diagrams`, so a block can be
    /// rendered as a diagram even when the latter is `false`.
...
impl<'a> RenderContentTreeRef<'a> {
    ...
    ///
    /// Ported from the pin (crates/editor/src/render/model/mod.rs); `BlockItem::Hidden`
    /// already exists, this is just a missing convenience query over it.
...
pub struct RenderState {
    ...
    /// The active layout mode for soft-wrap computation. For the TUI path (`CharCell`),
    /// this also carries all char-cell-specific state.
...
impl RenderState {
    ...
    /// [`CharCellState::hidden_line_ranges`]. CharCell mode consumes
    /// `styles.base_text.fixed_width_tab_size` for tab geometry; other style
    /// fields are retained for API compatibility but are unused, so callers
    /// may otherwise supply a minimal stub.
    pub fn new_tui(
        terminal_width: u16,
        styles: RichTextStyles,
    ...
    /// Set the offsets of Mermaid blocks the user has explicitly toggled to `Rendered`. Returns
    /// `true` when the set changed (the caller should trigger a relayout in that case).
    ...
    /// The char-cell state for the TUI rendering path, or `None` in the GUI (`Pixels`) path.
...
// ============ Char-cell (TUI) rendering support ============
...
impl CharCellState {
    ...
    /// Like [`Self::new`], but derives the char-cell tab stop width from
    /// `styles.base_text.fixed_width_tab_size` instead of the default.
    fn new_with_styles(
        terminal_width: u16,
        styles: &RichTextStyles,
        hidden_lines: Option<ModelHandle<HiddenLinesModel>>,
...
// ===== char-cell free functions (index math; TUI path) =====
