...
This skill is the canonical reference for Phosphor tab config TOML files.
...
That is the directory this running build actually reads, resolved for its channel and platform. Use it verbatim — do not derive a path yourself, and do not assume it is a `.warp` directory: on macOS it is a channel-specific dotfile directory in `$HOME` (`.phosphor` on the open-source build), and on Linux it is the XDG data directory (typically under `~/.local/share/`), where globbing the home directory for `.warp*` finds nothing at all. Do NOT use `~/Library/Application Support/` either. If the directory shown above is blank, Phosphor could not resolve it — ask the user where their tab configs live rather than guessing.
...
- When creating or locating tab configs, use the directory named at the top of this skill rather than deriving or hardcoding a base directory.
...
`{{autogenerated_branch_name}}` is a special-cased reserved template variable: if a tab config references it, Phosphor generates a unique worktree branch name on each open instead of prompting the user for that value.
...
### With parameters
Phosphor-generated worktree configs default to `{{worktrees_dir}}/<repo-name>/<worktree-name>` — again the resolved path for this build, not a fixed `.warp` location. When writing a reusable config by hand, derive the repo-name segment at shell runtime with `$(basename {{repo}})`. (The absolute directory above is substituted into this skill when it is loaded; `{{repo}}`, `{{branch_name}}` and `{{base_branch}}` below are tab config parameters and stay literal in the TOML you write.)
...
directory = "{{repo}}"
commands = [
  "git worktree add -b {{branch_name}} {{worktrees_dir}}/$(basename {{repo}})/{{branch_name}} {{base_branch}}",
  "cd {{worktrees_dir}}/$(basename {{repo}})/{{branch_name}}",
]
