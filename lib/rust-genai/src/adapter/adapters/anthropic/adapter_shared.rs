...
/// Whether the model name's Opus version is at or above `(target_major, target_minor)`.
///
/// Kept independent of `ant_model::AnthropicModel`'s capability matrix: this predicate
/// answers a narrower, Phosphor-specific question (1M-context header eligibility) with
/// its own criteria, not Anthropic's effort/thinking capability tiers.
fn is_opus_at_least(model_name: &str, target_major: u32, target_minor: u32) -> bool {
	let Some(version) = model_name.split_once("claude-opus-").map(|(_, rest)| rest) else {
		return false;
	};
	...
	match (major, minor) {
		...
		(Some(major), None) => (major, 0) >= (target_major, target_minor),
		_ => false,
	}
}
...
///
/// Criteria (aligned with opencode's `packages/console/.../anthropic.ts`):
/// - Model name contains `sonnet` (all Claude Sonnet 4+ models support 1M)
/// - Or is Opus 4.6 or higher (`claude-opus-4-6`, `claude-opus-4-7`, `claude-opus-5-x` ...)
///
/// Used for BYOP / direct / relay scenarios: without this header, some
/// relays (e.g. anyrouter) will directly 400 reject requests carrying a
/// model name like `claude-opus-4-7`, see zerx-lab/warp #21.
/// Anthropic officially accepts this header and still bills at the regular
/// rate when prompt < 200K tokens, so sending it by default is safe.
...
impl AnthropicAdapter {
	...
	pub(in crate::adapter::adapters) fn into_anthropic_request_parts(
		...
		for msg in chat_req.messages {
			...
			match msg.role {
				...
				ChatRole::Tool => {
					let mut values: Vec<Value> = Vec::new();
					// PHOSPHOR: parts that ride along with a tool-result turn but are not
					// themselves a `tool_result` (e.g. a computer-use screenshot delivered as
					// `Text`/`Binary` after the tool results of the same turn). Anthropic
					// requires `tool_result` blocks first in the turn answering a `tool_use`,
					// so these are appended after `values`, and — critically — after
					// cache_control has already been applied to `values` below, so a volatile
					// trailing part (a screenshot that differs every turn) can never steal the
					// prompt-cache breakpoint from the stable tool_result blocks.
					...
					for part in msg.content {
						match part {
							...
							ContentPart::Binary(binary) => {
								if let Some(part) = binary_to_anthropic_content_part(binary) {
									trailing.push(part);
								}
							}
	...
	pub(in crate::adapter::adapters) fn build_web_request_data(
		...
		// Without it, some relay gateways (anyrouter, etc.) will directly 400 reject the request.
		// Anthropic officially still bills at the regular rate when prompt < 200K, so sending it by default is safe.
		...
		// -- Merge extra_headers the user appended in ChatOptions (later writes override earlier ones)
		// The user can explicitly set a header with the same name (e.g. combining
		// multiple betas: `context-1m-...,files-api-...`) to override the
		// adapter's default value.
...
/// PHOSPHOR: convert a `Binary` content part to its Anthropic `image`/`document` JSON
/// shape, for use outside the `ChatRole::User` arm (specifically: a screenshot delivered
/// as a trailing part on a `ChatRole::Tool` turn). Mirrors the conversion the `User` arm
/// performs inline; kept as a standalone helper so that arm is left untouched.
fn binary_to_anthropic_content_part(binary: Binary) -> Option<Value> {
	let is_image = binary.is_image();
	let Binary {
		content_type, source, ..
	} = binary;
