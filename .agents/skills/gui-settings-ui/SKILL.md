...
description: GUI desktop app only. How to build a Settings page in the Phosphor client (app/src/settings_view) so its widgets and settings search behave correctly — picking a PageType, deciding whether a heading belongs in the page-title slot or inside a widget, gating a widget, and scoping search_terms per widget. Use when adding or editing a settings page, a SettingsWidget, or anything that affects settings search.
---

# gui-settings-ui

**Scope — GUI desktop app only.** This skill covers the settings modal in `app/src/settings_view/`, part of Phosphor's **GUI** desktop front-end. It does not apply to the headless TUI (`crates/warp_tui`). For general UI conventions see `warp-ui-guidelines`.

Settings pages look simple, so they get written by pattern-matching the nearest neighbor — and the nearest neighbor is often wrong. Two mistakes recur: a page heading that vanishes under search, and a single widget that swallows a whole page's worth of settings so none of them can be filtered. Read this before writing a settings page.

*Adapted from upstream `dc0c37a9a` (#14599). Every claim below was re-checked against this fork's tree; several upstream examples cited pages and widgets that do not exist here and were replaced with real local ones. Where this fork still has the bug the upstream skill describes, that is called out rather than papered over.*
...
PageType::new_monolith(MCPServersSettingsWidget::default(), title, /* is_dual_scrollable */ true)
```

- **`Uncategorized`** — a flat list of widgets. The common shape.
- **`Categorized`** — widgets grouped under `Category`s, each with a subheader and an optional subtitle. `render_page` draws a category's heading with `render_sub_header_with_description` when it has a subtitle and `render_sub_header` when it doesn't, and skips it entirely when the category title is empty.
- **`Monolith`** — the whole page is a single widget because its content can't be split for search. Here: Keybindings (`keybindings.rs`), About (`about_page/mod.rs`), MCP servers (`mcp_servers_page.rs`), Network (`network_page.rs`).
...
One more thing worth knowing before you go looking for a title stranded over zero rows: you won't find one. `SettingsView::filtered_pages` (`app/src/settings_view/mod.rs`) keeps a page only when `self.should_render_page(page, app) && match_data.is_truthy()`, so a fully non-matching page drops out of the sidebar and is never displayed. The state the title slot actually protects is the **partial** match — some widgets survive, some don't — and only widget-list pages can be in it.
...
- **Can change at runtime** — a setting the user toggles, an availability check that can flip mid-session. → **Use `should_render`.**

The reason is *when each is evaluated*. The build-time `if` runs once, when the page is constructed, and its result is frozen until something rebuilds the page (for AI subpages, only switching subpages does — see below). `should_render` is re-evaluated on every filter and render pass, so it tracks a value that changes while the settings page is open. Gate a runtime-changing value with a build-time `if` and the page goes stale; gate a static flag with `should_render` and you carry a widget around for nothing.

The real `should_render` users here are all the runtime kind: the slash-commands widget in `features_page.rs` on `AISettings::as_ref(app).is_any_ai_enabled(app)`, and the codebase-indexing widgets in `code_page.rs` on `codebase_indexing_settings_supported(app)`.
...
- **Single-topic page** — one heading names everything on it. Done right here: Knowledge (`ai_page.rs`), Scripting (`scripting_page.rs`), Privacy (`privacy_page.rs`). → **Title in the `PageType` slot.**
- **Multi-section page** — several independent sections, each with its own heading. Appearance, Features, and Warpify are `Categorized`; the Phosphor Agent and Agent profiles AI subpages are flat lists whose widgets render their own subheader-sized titles internally. → **Per-section headings live in widgets/categories** and correctly disappear with their rows. (For `Categorized`, `get_filtered` drops categories whose widgets all filtered out, so their subheaders vanish automatically — that's the behavior you want.)
- **Monolith page** — Keybindings, About, MCP servers, Network. There is no partial-match state: the sole widget either matches, and the whole page renders, or it doesn't, and the whole page renders empty and drops out of the sidebar. So a monolith can never strand an orphaned setting under a missing heading — it is **not** affected by bug class 1, but for that reason, *not* because its title is protected. Passing the title through the slot is still the tidier structure (MCP servers does), just don't expect it to keep the title on screen during a non-matching search; on a monolith it will not.

The two are not mutually exclusive: a page can name itself in the title slot **and** have per-section subheaders inside its widgets. Privacy does exactly that — `PageType::new_uncategorized(widgets, Some(crate::t_static!("settings-privacy-page-title")))` plus a `render_sub_header` call inside an individual widget. The rule is per heading, not per page.

Worked positive example: **Scripting** (`scripting_page.rs`) is a small page done right — two focused widgets (`WarpControlCliInstallWidget`, `LocalControlModeWidget`) with their own `search_terms`, and `PageType::new_uncategorized(widgets, Some("Scripting"))`. A small page is not automatically a mega-widget.
...
The fix is to move the heading into page chrome. Knowledge is the local example of the fixed shape — `AISettingsPageView::build_page` ends with:
...
**A header-only widget is always this bug.** If a widget renders nothing but a title, delete it and use the title slot.

**Still open in this tree.** The `ThirdPartyCLIAgents` subpage is single-topic — `build_page` gives it exactly one widget, `CLIAgentWidget` — and that widget's `render` opens with `build_sub_header(..)`, while the `title` expression above yields `None` for it. Any search term that doesn't match `CLIAgentWidget` erases the heading with the rows. Upstream fixed this by extending the title expression to that subpage; this fork has not. Don't copy the pattern, and if you are working in `ai_page.rs` anyway, fixing it is cheap.
...
Live example in this tree — `CLIAgentWidget` (`ai_page.rs`), one widget, one blob, seven settings:
...
The remedy is one widget per setting, each with terms scoped to just that setting, assembled by a small helper that returns the vector. Rules of thumb when splitting:

- **One widget ≈ one setting row** (or one tightly-coupled group that always shows and hides together).
- **Scope `search_terms` to that widget only.** Keep enough shared context that a page-level query still matches (every CLI-agent row should keep `"third party cli coding agent"`), then add the terms unique to the row.
- **Give each row the right gating mechanism** (see "Two ways to gate a widget"). A row that used to sit behind an `if … { … }` inside a mega-`render` becomes its own widget: if its condition is a static feature flag or platform check, gate it with an `if` in the page's build function and never create it; if it can change at runtime, give it `should_render`, and share the predicate through a small free function rather than duplicating the condition.
- **Move per-row state with the row.** Each `SwitchStateHandle` / `MouseStateHandle` moves to the widget that owns its control. Never create one inline while rendering (see `warp-ui-guidelines`, and the `review-pr-local` skill's rule about inline `MouseStateHandle::default()`).
- **Watch the widget ids.** `widget_id()` is `std::any::type_name::<Self>()`, so splitting a widget changes ids. `settings_widget_deeplink_target` in `app/src/settings_view/mod.rs` maps stable public slugs (`zap://settings?widget=<slug>`) onto them; only `global_hotkey` and `cli_agents` are linkable here. Splitting `CLIAgentWidget` must keep it as the first widget so `cli_agent_settings_widget_id()` — the target of the `cli_agents` deeplink — stays valid. If you rename or remove a widget that backs a deeplink, re-point the accessor.

## AI subpages rebuild their `PageType` — reapply the filter

AI subpages rebuild their `PageType` when the active subpage changes (`AISettingsPageView::set_active_subpage` → `build_page`). A fresh `PageType` starts with **every** widget in its filter, so a live search query is silently dropped unless it's reapplied. `SettingsView::reapply_search_filter_to_active_subpage` in `app/src/settings_view/mod.rs` exists for exactly this (upstream `2356ddab2`, #14116). If you add a code path that rebuilds a subpage's page while search may be active, call it.

Upstream has a `CodeSubpage` half of the same bug. This fork's Code settings page (`code_page.rs`) has no subpage architecture — `build_page` takes no subpage argument and always passes `None` for the title — so that half does not apply, as `reapply_search_filter_to_active_subpage`'s own doc comment records.

## Known anti-examples still in the tree

Useful to read, not to copy:

- **Warpify** (`warpify_page.rs`) — `PageType::new_categorized(categories, None)` where the first category is `Category::new("", vec![Box::new(TitleWidget::default())])` and `TitleWidget::render` reaches `render_page_title` through `render_top_of_page`. Single-topic page, title inside a widget: bug class 1.
- **Third party CLI agents** (`ai_page.rs`) — single-topic subpage, no title slot, heading drawn by `CLIAgentWidget::render`; and the same widget carries the mega `search_terms` blob. Both bug classes in one widget.
- **Zap Drive** (`warp_drive_page.rs`) — `PageType::new_uncategorized([WarpDriveToggleWidget], None)`, a bare toggle row with no page heading at all. Note before "fixing" it: Warp Drive is dropped cloud in this fork (see `DECLINED.md`, #267), so this page is a declined surface, not parity debt.

## How to verify a settings-page change

Verification is cheap here; do both.

1. **Unit-test the filter.** `app/src/settings_view/mod_tests.rs` has the harness — `StubWidget`, `stub_widgets_page`, and `visible_widget_count` (which reads `PageType::get_filtered` and panics on anything but an `Uncategorized` page). Assert that a term unique to one widget yields exactly one visible widget, and that clearing the query restores all of them. Follow the `${filename}_tests.rs` convention from AGENTS.md; run with `cargo nextest run -p warp settings_view`.
...
// Assuming a PageType title is always exempt from filtering. True for
// Uncategorized and Categorized; on a Monolith the title goes with the widget.
```
