...
- `warp-integration-test` — end-to-end GUI and TUI behavior, terminal and shell integration, settings and keybinding wiring.
- `computer_use` — when the real question is visual and someone needs to look at it.
...
Note that none of this licenses weakening or deleting a test to make a red suite go green — see `AGENTS.md` 5.6. Delete a test because it is redundant or change-detecting, never because it is failing.
