...
| **`ORACLE.md`** | **The oracle is PINNED.** Compare against Warp `2026.08.12` stable = `42effe840`, **never `warp/master`** — master is unreleased trunk moving 50-80 tests/day, so measuring against it produces a gap that never shrinks. Also states the re-pin policy (always the latest *stable*). **Take the pin from `ORACLE.md`, not from here** — the pin moved to `42effe840` on 2026-08-15 and this row still named the superseded `02b53fcd8` until 2026-08-17, i.e. the instruction file contradicted the authority it points at. |
